...
describe('Template for Dev Unit Test', function () {
  ...
  describe('Usage of tenant context with all components', () => {
    it('Basic charging station transaction', async () => {
      const tenantContextAll = await ContextProvider.defaultInstance.getTenantContext(
      ...
      const response = await chargingStationContext.startTransaction(
        1,
        user.tags[0].id,
        0,
        moment()
      ...
      const userCentralService = tenantContextAll.getUserCentralServerService(
