...
export default class BillingTestHelper {
  ...
  public async initialize(
    tenantContext = ContextDefinition.TENANT_CONTEXTS.TENANT_BILLING
  ): Promise<void> {
    this.tenantContext = await ContextProvider.defaultInstance.getTenantContext(tenantContext);
    this.adminUserContext = this.tenantContext.getUserContext(
    ...
    this.basicUserContext = this.tenantContext.getUserContext(
  ...
  public async makeCurrentUserContextReadyForBilling(forceSynchronization = true): Promise<void> {
    if (forceSynchronization) {
      await this.getCurrentUserService().billingApi.forceSynchronizeUser({
        id: this.getCurrentUserContext().id,
      });
    }
  ...
  public async assignPaymentMethod(
    user: BillingUser,
  ...
  public async initContext2TestConnectedAccounts(): Promise<void> {
    ...
    let response = await this.getCurrentUserService().chargingStationApi.readById(
      chargingStation.id
    );
  ...
  public async initChargingStationContext(): Promise<ChargingStationContext> {
    this.siteContext = this.tenantContext.getSiteContext(
      ContextDefinition.SITE_CONTEXTS.SITE_WITH_OTHER_USER_STOP_AUTHORIZATION
    );
    this.siteAreaContext = this.siteContext.getSiteAreaContext(
      ContextDefinition.SITE_AREA_CONTEXTS.WITH_ACL
    );
    this.chargingStationContext = this.siteAreaContext.getChargingStationContext(
  ...
  public async initChargingStationContext2TestChargingTime(): Promise<ChargingStationContext> {
    ...
    await this.createTariff4ChargingStation(
      'FF+CT',
  ...
  public async initChargingStationContext2TestCS3Phased(
    testMode = 'FF+E'
  ): Promise<ChargingStationContext> {
    this.siteContext = this.tenantContext.getSiteContext(
      ContextDefinition.SITE_CONTEXTS.SITE_BASIC
    );
    this.siteAreaContext = this.siteContext.getSiteAreaContext(
  ...
  public async initChargingStationContext2TestFastCharger(
    testMode = 'E',
  ...
  public async initChargingStationContext2TestDaysOfTheWeek(
    testMode = 'E'
  ): Promise<ChargingStationContext> {
    ...
    const timezone = Utils.getTimezone(
      this.chargingStationContext.getChargingStation().coordinates
    );
    let dimensions: PricingDimensions;
    let restrictions: PricingRestriction;
    if (testMode === 'TODAY') {
      ...
      restrictions = {
        ...
        daysOfWeek: [
          ...
          DayOfWeek.SATURDAY,
          DayOfWeek.SUNDAY,
        ].filter((day) => day !== moment().tz(timezone).isoWeekday()),
      };
    }
    await this.createTariff4ChargingStation(
      testMode,
  ...
  public checkTimezone(): void {
    ...
    assert(
      moment().tz(timezone).isoWeekday() === moment().add(2, 'hours').tz(timezone).isoWeekday(),
      'Timezone is set  to ' + timezone + ' - test execution can not work in that context'
    );
  }

  public async initChargingStationContext2TestTimeRestrictions(
    testMode = 'E',
    aParticularMoment: moment.Moment
  ): Promise<ChargingStationContext> {
    // Charging Station Context
    this.siteContext = this.tenantContext.getSiteContext(
      ContextDefinition.SITE_CONTEXTS.SITE_BASIC
  ...
  public async setBillingSystemValidCredentials(
    activateTransactionBilling = true,
    ...
    billingSettings.stripe.secretKey = await Cypher.encrypt(
      tenant,
  ...
  public async saveBillingSettings(billingSettings: BillingSettings): Promise<void> {
    ...
    const tenantBillingSettings = await this.adminUserService.settingApi.readByIdentifier({
      Identifier: 'billing',
    });
  ...
  public async checkInvoiceData(
    invoiceID: string,
    expectedStatus: BillingInvoiceStatus,
  ...
  public async checkTransactionBillingData(
    transactionId: number,
    expectedInvoiceStatus: BillingInvoiceStatus,
    expectedPrice: number = null
  ): Promise<BillingDataTransactionStop> {
    ...
    assert(
      billingDataStop?.invoiceStatus === expectedInvoiceStatus,
      `The invoice status should be ${expectedInvoiceStatus}`
    );
    if (expectedInvoiceStatus !== BillingInvoiceStatus.DRAFT) {
      assert(billingDataStop?.invoiceNumber, 'Invoice Number should be set');
    } else {
      assert(
        billingDataStop?.invoiceNumber === null,
        `Invoice Number should not yet been set - Invoice Number is: ${billingDataStop?.invoiceNumber}`
      );
    }
    if (expectedPrice) {
      // --------------------------------
      ...
      // --------------------------------
      const roundedPrice = Utils.createDecimal(
        transactionResponse.data.stop.roundedPrice as number
      );
      assert(
        roundedPrice.equals(expectedPrice),
        `The rounded price should be: ${expectedPrice} - actual value: ${roundedPrice.toNumber()}`
      );
      // ---------------------------
      ...
      assert(
        billedPrice.equals(expectedPrice),
        `The billed price should be: ${expectedPrice} - actual value: ${billedPrice.toNumber()}`
      );
    }
  ...
  public async dumpLastErrors(): Promise<void> {
    ...
    const loggedErrors = await LogStorage.getLogs(
  ...
  public async generateTransactionAndCheckBillingStatus(
    expectedBillingStatus: BillingInvoiceStatus
  ): Promise<number> {
  ...
  public async generateTransaction(
    ...
    expectedStartDate = new Date(),
    withSoftStopSimulation = false,
    withWrongStatusNotificationSequence = false
  ): Promise<number> {
    const meterStart = 0;
    ...
    if (
      expectedStatus === 'Accepted' &&
      startTransactionResponse.idTagInfo.status !== expectedStatus
    ) {
    ...
    await this.sendConsumptionMeterValue(
      connectorId,
      transactionId,
      currentTime,
      cumulated,
    ...
    assert(
      cumulated === meterStop,
      'Inconsistent meter values - cumulated energy should equal meterStop - ' + cumulated
    );
    ...
    if (expectedStatus === 'Accepted') {
      if (withSoftStopSimulation) {
        ...
        await this.sendStatusNotification(
          connectorId,
          stopDate.clone().add(29, 'minutes').toDate(),
          ChargePointStatus.FINISHING
        );
        await this.sendStatusNotification(
          connectorId,
          stopDate.clone().add(30, 'minutes').toDate(),
          ChargePointStatus.AVAILABLE
        );
        // SOFT STOP TRANSACTION
        const stopTransactionResponse = await this.chargingStationContext.softStopTransaction(
          transactionId
        );
        ...
        transaction = await TransactionStorage.getTransaction(tenant, transactionId, {
          withUser: true,
          withChargingStation: true,
        });
        ...
        if (
          expectedStatus === 'Accepted' &&
          stopTransactionResponse.idTagInfo.status !== expectedStatus
        ) {
          await this.dumpLastErrors();
        }
  ...
  public async sendConsumptionMeterValue(
    connectorId: number,
    transactionId: number,
  ...
  public async waitForAsyncTasks(): Promise<void> {
    let counter = 0,
      pending: DataResult<AsyncTask>,
      running: DataResult<AsyncTask>;
    await TestUtils.sleep(1000);
    while (counter++ <= 10) {
      // Get the number of pending tasks
      pending = await AsyncTaskStorage.getAsyncTasks(
        { status: AsyncTaskStatus.PENDING },
        Constants.DB_PARAMS_COUNT_ONLY
      );
      running = await AsyncTaskStorage.getAsyncTasks(
      ...
      // Give some time to the asyncTask to bill the transaction
      console.log(
        `Waiting for async tasks - pending tasks: ${pending.count} - running tasks: ${running.count}`
      );
      await TestUtils.sleep(1000);
    }
    if (!pending.count && !running.count) {
      ...
      console.warn(
        `Gave up after more than 10 seconds - pending tasks: ${pending.count} - running tasks: ${running.count}`
      );
    }
  }
  ...
  public async createTariff4ChargingStation(
    ...
    restrictions: PricingRestriction = null
  ): Promise<void> {
    // Set a default value
    ...
    const tariff: Partial<PricingDefinition> = {
      ...
      name: tariffName,
      description:
        'Tariff for CS ' + chargingStation.id + ' - ' + tariffName + ' - ' + connectorType,
      staticRestrictions: {
        connectorType,
  ...
  public async checkPricingDefinitionEndpoints(): Promise<void> {
    ...
    let response = await this.adminUserService.pricingApi.createPricingDefinition(
      tariffForSiteArea
    );
    assert(response?.data?.status === 'Success', 'The operation should succeed');
    assert(response?.data?.id, 'The ID should not be null');
    ...
    assert(response?.data?.id === pricingDefinitionId, 'The ID should be: ' + pricingDefinitionId);
    assert(
      response?.data?.entityName === siteArea.name,
      'The Site Area data should be retrieved as well'
    );
  }
  ...
  public async createActivatedAccount(): Promise<BillingAccount> {
    ...
    // ----------------------------------------------------
    response = await this.getCurrentUserService().billingApi.refreshBillingAccount({
      accountID,
    ...
    // --------------------------------------------------------------
    response = await this.getCurrentUserService().billingApi.activateBillingAccount({
      accountID,
      TenantID: this.getTenantID(),
    });
    if (response.status !== StatusCodes.OK) {
  ...
  public async getActivatedAccount(createIfMissing = true): Promise<BillingAccount> {
    if (!this.billingAccount) {
      ...
      const paging = TestConstants.DEFAULT_PAGING;
      const ordering = [{ field: '-createdOn' }];
      const response = await this.adminUserService.billingApi.readBillingAccounts(
        params,
        paging,
        ordering
      );
  ...
  public async getTransfers(status: BillingTransferStatus): Promise<any> {
    ...
    assert(
      accountID,
      'accountID should not be null - make sure the context has been initialized with ==> billingTestHelper.initContext2TestConnectedAccounts()'
    );
  ...
  public async checkTransferData(
    transferID: string,
  ...
  public async addFundsToBalance(
    amount: number,
    stripe_test_token = 'btok_us_verified',
    currency = 'usd'
