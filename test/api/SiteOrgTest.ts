  ...
  describe('With component Organization (utorg)', () => {
    ...
    describe('Where admin user', () => {
      ...
      it('Assets, Charging stations and transactions should be aligned with organizations entities', async () => {
        ...
        const transactionSite = (await testData.userService.siteApi.readById(transaction.siteID))
          .data;
        // Change the companyID of the Site
        transactionSite.companyID = testData.tenantContext.getContext().companies[0].id;
        // Update
        await testData.userService.updateEntity(testData.userService.siteApi, transactionSite);
        ...
        await testData.userService.siteAreaApi.assignChargingStations(testData.newSiteArea.id, [
          chargingStation.id,
        ]);
        ...
        testData.testAsset = (
          await testData.userService.assetApi.readById(testData.testAsset.id)
        ).data;
        expect(testData.testAsset.companyID).to.equal(
          testData.tenantContext.getContext().companies[1].id
        );
        // Get the assigned charging station
        chargingStation = (
          await testData.userService.chargingStationApi.readById(chargingStations.data.result[0].id)
        ).data;
        expect(chargingStation.companyID).to.equal(
          testData.tenantContext.getContext().companies[1].id
        );
      ...
      it('Should find the created site by id', async () => {
        ...
        await testData.userService.getEntityById(testData.userService.siteApi, testData.newSite);
      });
      ...
      it('Should be able to assign a user to a site', async () => {
        ...
        const userContext = await testData.tenantContext.getUserContext(
      ...
      it('Should be able to unassign a site to a user', async () => {
        await unassignSitesToUser(ContextDefinition.USER_CONTEXTS.BASIC_USER, [
          testData.newSite.id,
        ]);
        const res = await testData.userService.siteApi.readUsersForSite(testData.newSite.id);
      ...
      it('Should be able to assign site admin to a site', async () => {
        ...
        expect(res.data.result.map((site) => site.user.id)).to.contain(userContext.id);
        expect(
          res.data.result.filter((site) => site.siteAdmin === true).map((site) => site.user.id)
        ).to.contain(userContext.id);
      });
      ...
      it('Should be able to assign site owner to a site', async () => {
        ...
        expect(res.data.result.map((site) => site.user.id)).to.contain(userContext.id);
        expect(
          res.data.result.filter((site) => site.siteOwner === true).map((site) => site.user.id)
        ).to.contain(userContext.id);
      });
      ...
      it('Should be able to update the site', async () => {
        ...
        // Update
        await testData.userService.updateEntity(testData.userService.siteApi, testData.newSite);
      });
    ...
    describe('Where basic user', () => {
      ...
      it('Should not be able to assign a user to a site he is assigned to', async () => {
        const assignmentResult = await assignUserToSite(
          ContextDefinition.USER_CONTEXTS.DEFAULT_ADMIN,
          testData.createdSites[0]
        );
