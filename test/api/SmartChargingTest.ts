...
import {
  SapSmartChargingSetting,
  SettingDB,
  SmartChargingSetting,
...
class TestData {
  ...
  public static async setSmartChargingValidCredentials(testData): Promise<void> {
    ...
    sapSmartChargingSettings.password = await Cypher.encrypt(
      testData.tenantContext.getTenant(),
      sapSmartChargingSettings.password
    );
    ...
    (aCBufferFactor = 1 + sapSmartChargingSettings.limitBufferAC / 100),
      ...
      (smartChargingIntegration = await SmartChargingFactory.getSmartChargingImpl(
        testData.tenantContext.getTenant()
      ));
    (sapSmartChargingSettings.limitBufferDC = 10),
      (sapSmartChargingSettings.limitBufferAC = 5),
      await TestData.saveSmartChargingSettings(testData, sapSmartChargingSettings);
    smartChargingIntegrationWithDifferentBufferValues =
      await SmartChargingFactory.getSmartChargingImpl(testData.tenantContext.getTenant());
    sapSmartChargingSettings.stickyLimitation = false;
    await TestData.saveSmartChargingSettings(testData, sapSmartChargingSettings);
    smartChargingIntegrationWithoutStickyLimit = await SmartChargingFactory.getSmartChargingImpl(
      testData.tenantContext.getTenant()
    );
    sapSmartChargingSettings.prioritizationParametersActive = false;
    await TestData.saveSmartChargingSettings(testData, sapSmartChargingSettings);
    smartChargingIntegrationWithoutPriorities = await SmartChargingFactory.getSmartChargingImpl(
      testData.tenantContext.getTenant()
    );
  ...
  public static async saveSmartChargingSettings(
    testData,
    sapSmartChargingSettings: SapSmartChargingSetting
  ): Promise<void> {
    const tenantSmartChargingSettings = await testData.userService.settingApi.readByIdentifier({
      Identifier: 'smartCharging',
    });
  ...
  public static validateAndReturnChargingProfile(
    chargingProfiles: ChargingProfile[],
    transaction: Transaction
  ): ChargingProfile {
    const chargingProfile = chargingProfiles.find(
      (cp) => cp.profile.transactionId === transaction.id
    );
    expect(chargingProfile).containSubset({
      ...
      profile: {
        ...
        chargingSchedule: {
          chargingRateUnit: 'A',
          duration:
            chargingProfile.profile.chargingSchedule.chargingSchedulePeriod.length * 15 * 60,
        },
      },
    });
  ...
  public static async sendMeterValue(
    voltage: Voltage,
    amperagePerPhase: number,
    transaction: Transaction,
    chargingStationContext: ChargingStationContext,
...
describeif(testData.chargingSettingProvided)('Smart Charging Service', () => {
  ...
  describe('With component SmartCharging (utsmartcharging)', () => {
    beforeAll(async () => {
      testData.tenantContext = await ContextProvider.defaultInstance.getTenantContext(
        ContextDefinition.TENANT_CONTEXTS.TENANT_SMART_CHARGING
      );
      ...
      if (tenant.id) {
        ...
        throw new Error(
          `Unable to get Tenant ID for tenant : ${ContextDefinition.TENANT_CONTEXTS.TENANT_SMART_CHARGING}`
        );
      }
    });
    ...
    describe('Test for three phased site area (This is a sub site area of DC site area)', () => {
      beforeAll(() => {
        ...
        testData.siteAreaContext = testData.siteContext.getSiteAreaContext(
          ContextDefinition.SITE_AREA_CONTEXTS.WITH_SMART_CHARGING_THREE_PHASED
        );
        ...
        testData.chargingStationContext1 = testData.siteAreaContext.getChargingStationContext(
          ContextDefinition.CHARGING_STATION_CONTEXTS.ASSIGNED_OCPP16 +
            '-' +
      ...
      afterAll(async () => {
        ...
        await testData.chargingStationContext.setConnectorStatus(
          chargingStationConnector1Available
        );
        await testData.chargingStationContext.setConnectorStatus(
          chargingStationConnector2Available
        );
        await testData.chargingStationContext1.setConnectorStatus(
          chargingStationConnector1Available
        );
        await testData.chargingStationContext1.setConnectorStatus(
          chargingStationConnector2Available
      ...
      it('Test for one car charging', async () => {
        ...
        const transactionStartResponse = await testData.chargingStationContext.startTransaction(
          1,
          testData.userContext.tags[0].id,
          180,
          new Date()
        );
        // Get and store started transaction
        const transactionResponse = await testData.centralUserService.transactionApi.readById(
          transactionStartResponse.transactionId
        );
        ...
        // Call Smart Charging
        const chargingProfiles = await smartChargingIntegration.buildChargingProfiles(
          testData.siteAreaContext.getSiteArea()
        );
        // Validate Charging Schedule
        expect(
          TestData.validateAndReturnChargingProfile(chargingProfiles, transaction).profile
            .chargingSchedule.chargingSchedulePeriod
        ).containSubset(limit96);
      });
      ...
      it('Test for two cars charging', async () => {
        ...
        expect(
          TestData.validateAndReturnChargingProfile(chargingProfiles, transaction1).profile
            .chargingSchedule.chargingSchedulePeriod
        ).containSubset(limit96);
      });
      ...
      it('Test for three cars charging with lower site area limit and one car on a single phased station', async () => {
        ...
        const transactionStartResponse = await testData.chargingStationContext1.startTransaction(
          1,
          testData.userContext.tags[0].id,
          180,
          new Date()
        ...
        expect(
          TestData.validateAndReturnChargingProfile(chargingProfiles, transaction2).profile
            .chargingSchedule.chargingSchedulePeriod
        ).containSubset(limit0);
      });

      it('Test if single phased charging station is excluded from root fuse correctly', async () => {
        await smartChargingIntegration.buildChargingProfiles(
          testData.siteAreaContext.getSiteArea(),
        ...
        const log = await LogStorage.getLogs(
          testData.tenantContext.getTenant(),
          { actions: [ServerAction.SMART_CHARGING], search: 'currently being used' },
          Constants.DB_PARAMS_SINGLE_RECORD,
          null
        );
        const siteArea = testData.siteAreaContext.getSiteArea();
        const siteAreaLimitPerPhase = Utils.createDecimal(siteArea.maximumPower)
          .div(siteArea.voltage)
          .div(3)
          .toNumber();
        const connectorLimit = Utils.createDecimal(
          Utils.getChargingStationAmperage(
            testData.chargingStationContext1.getChargingStation(),
            null,
            1
          )
        ).toNumber();
        expect(log.result[0].detailedMessages).include(
          '"fusePhase1": ' + (siteAreaLimitPerPhase - connectorLimit)
        );
      ...
      it('Test for 3 single phased cars charging with lower site area limit and one car on a single phased station (meter value received)', async () => {
        ...
        await TestData.sendMeterValue(
          Voltage.VOLTAGE_230,
          32,
          transaction,
          testData.chargingStationContext,
          { csPhase1: false, csPhase2: true, csPhase3: false }
        );
        await TestData.sendMeterValue(
          Voltage.VOLTAGE_230,
          32,
          transaction1,
          testData.chargingStationContext,
          { csPhase1: false, csPhase2: false, csPhase3: true }
        );
        const chargingProfiles = await smartChargingIntegration.buildChargingProfiles(
          testData.siteAreaContext.getSiteArea()
        );
      ...
      it('Test for sticky limit - 1 three phased and 2 single phased cars charging and one car on a single phased station', async () => {
        ...
        await testData.userService.siteAreaApi.update(testData.siteAreaContext.getSiteArea());
        await testData.chargingStationContext.stopTransaction(
          transaction.id,
          testData.userContext.tags[0].id,
          180,
          new Date()
        ...
        await TestData.sendMeterValue(
          ...
          transaction,
          testData.chargingStationContext,
          { csPhase1: true, csPhase2: true, csPhase3: true }
        );
        await TestData.sendMeterValue(
          Voltage.VOLTAGE_230,
          13,
        ...
        await TestData.sendMeterValue(
          ...
          20,
          transaction2,
          testData.chargingStationContext1,
          { csPhase1: true, csPhase2: false, csPhase3: false }
        );
        ...
        let chargingProfile = TestData.validateAndReturnChargingProfile(
          chargingProfiles,
          transaction
        );
        ...
        await ChargingStationStorage.saveChargingProfile(
          testData.tenantContext.getTenant(),
          chargingProfile
        );
      ...
      it('Test if charging profiles are not returned, when they are already applied', async () => {
        ...
        const chargingProfile = TestData.validateAndReturnChargingProfile(
          chargingProfiles,
          transaction2
        );
        expect(chargingProfile.profile.chargingSchedule.chargingSchedulePeriod).containSubset([
      ...
      it('Test for sticky limit with different buffer value - 1 three phased and 2 single phased cars charging and one car on a single phased station', async () => {
        const chargingProfiles =
          await smartChargingIntegrationWithDifferentBufferValues.buildChargingProfiles(
            testData.siteAreaContext.getSiteArea()
          );
        // Charging Profiles should have limits according the sent meter values + buffer
        expect(
      ...
      it('Test for sticky limit disabled - 1 three phased and 2 single phased cars and one car on a single phased station', async () => {
        await TestData.sendMeterValue(
          Voltage.VOLTAGE_230,
          16,
          transaction,
        ...
        const chargingProfiles =
          await smartChargingIntegrationWithoutStickyLimit.buildChargingProfiles(
            testData.siteAreaContext.getSiteArea()
          );
        // Charging Profiles should have limits according the sent meter values + buffer
        expect(
      ...
      it('Test for sticky limit - check if cars are able to go up again, when using the buffer', async () => {
        ...
        await ChargingStationStorage.deleteChargingProfiles(
          testData.tenantContext.getTenant(),
          chargingProfiles[0].chargingStationID
        );
        await ChargingStationStorage.deleteChargingProfiles(
          testData.tenantContext.getTenant(),
          chargingProfiles[1].chargingStationID
        );
        await ChargingStationStorage.deleteChargingProfiles(
          testData.tenantContext.getTenant(),
          chargingProfiles[2].chargingStationID
        );
      });
      ...
      it('Test for sticky limit - 1 three phased and 2 single phased cars charging and one car on a single phased station with no consumption on two cars', async () => {
        ...
        expect(
          TestData.validateAndReturnChargingProfile(chargingProfiles, transaction).profile
            .chargingSchedule.chargingSchedulePeriod
        ).containSubset(limitMinThreePhased);
        expect(
          TestData.validateAndReturnChargingProfile(chargingProfiles, transaction1).profile
            .chargingSchedule.chargingSchedulePeriod
        ).containSubset([
        ...
        expect(
          TestData.validateAndReturnChargingProfile(chargingProfiles, transaction2).profile
            .chargingSchedule.chargingSchedulePeriod
        ).containSubset(limitMinSinglePhased);
      });

      it('Test if Charging Station is excluded, when transaction is not existing anymore', async () => {
        await testData.chargingStationContext1.stopTransaction(
          transaction2.id,
          transaction.tagID,
          200,
          new Date()
        ...
        const siteAreaLimitPerPhase = Utils.createDecimal(siteArea.maximumPower)
          ...
          .toNumber();
        const connectorLimit = Utils.createDecimal(
          Utils.getChargingStationAmperage(chargingStation, null, transaction2.connectorId)
        ).toNumber();
        expect(log.result[0].detailedMessages).include(
          '"fusePhase1": ' + (siteAreaLimitPerPhase - connectorLimit)
        );
      ...
      it('Test if Charging Stations are excluded, when transactions are not existing anymore', async () => {
        ...
        const connectorLimitSinglePhased = Utils.createDecimal(
          Utils.getChargingStationAmperage(
            testData.chargingStationContext1.getChargingStation(),
            null,
            transaction2.connectorId
          )
        ).toNumber();
        const chargingStationLimitThreePhased = Utils.createDecimal(
          Utils.getChargingStationAmperage(testData.chargingStationContext.getChargingStation())
        )
          .div(3)
          .toNumber();
        expect(log.result[0].detailedMessages).include(
          '"fusePhase1": ' +
            (siteAreaLimitPerPhase - connectorLimitSinglePhased - chargingStationLimitThreePhased)
        );
        expect(log.result[0].detailedMessages).include(
          '"fusePhase2": ' + (siteAreaLimitPerPhase - chargingStationLimitThreePhased)
        );
        expect(log.result[0].detailedMessages).include(
          '"fusePhase3": ' + (siteAreaLimitPerPhase - chargingStationLimitThreePhased)
        );
      });
    });
    ...
    describe('Test for single phased site area', () => {
      beforeAll(() => {
        ...
        testData.siteAreaContext = testData.siteContext.getSiteAreaContext(
          ContextDefinition.SITE_AREA_CONTEXTS.WITH_SMART_CHARGING_SINGLE_PHASED
        );
        testData.chargingStationContext = testData.siteAreaContext.getChargingStationContext(
          ContextDefinition.CHARGING_STATION_CONTEXTS.ASSIGNED_OCPP16
        );
      ...
      it('Test if one charging connector is excluded from root fuse when charging station is excluded', async () => {
        ...
        expect(log.result[0].detailedMessages).include(
          '"fusePhase1": ' +
            Utils.createDecimal(siteArea.maximumPower)
              .div(siteArea.voltage)
              .minus(
                Utils.getChargingStationAmperage(
    ...
    describe('Test for DC site area', () => {
      beforeAll(() => {
        ...
        testData.siteAreaContext = testData.siteContext.getSiteAreaContext(
          ContextDefinition.SITE_AREA_CONTEXTS.WITH_SMART_CHARGING_DC
        );
        testData.chargingStationContext = testData.siteAreaContext.getChargingStationContext(
          ContextDefinition.CHARGING_STATION_CONTEXTS.ASSIGNED_OCPP16
        );
      ...
      it('Test for one car charging', async () => {
        ...
        expect(
          chargingProfiles[0].profile.chargingSchedule.chargingSchedulePeriod[2].limit
        ).to.be.greaterThan(250);
        expect(
          chargingProfiles[0].profile.chargingSchedule.chargingSchedulePeriod[2].limit
        ).to.be.lessThanOrEqual(654);
      });
      ...
      it('Test if whole charge point is excluded from root fuse when charging station is excluded (one connector charging)', async () => {
        ...
        const chargePoint = chargingStation.chargePoints.find((cp) =>
          cp.connectorIDs.includes(transaction.connectorId)
        );
        const siteArea = testData.siteAreaContext.getSiteArea();
        const siteAreaLimitPerPhase = Utils.createDecimal(siteArea.maximumPower)
          .div(siteArea.voltage)
          .div(3)
          .toNumber();
        let connectorLimitPerPhase = Utils.createDecimal(
          Utils.getChargingStationAmperage(chargingStation)
        )
          .div(3)
          .toNumber();
        // Calculate efficiency
        connectorLimitPerPhase = Utils.createDecimal(connectorLimitPerPhase)
          .mul(100)
          .div(chargePoint.efficiency)
          .toNumber();
        expect(log.result[0].detailedMessages).include(
          '"fusePhase1": ' + (siteAreaLimitPerPhase - connectorLimitPerPhase)
        );
        expect(log.result[0].detailedMessages).include(
          '"fusePhase2": ' + (siteAreaLimitPerPhase - connectorLimitPerPhase)
        );
        expect(log.result[0].detailedMessages).include(
          '"fusePhase3": ' + (siteAreaLimitPerPhase - connectorLimitPerPhase)
        );
      });
      ...
      it('Test if only charge point is excluded from root fuse when charging station is excluded (two connectors charging)', async () => {
        ...
        const chargingStation = testData.chargingStationContext.getChargingStation();
        const chargePoint = chargingStation.chargePoints.find((cp) =>
          cp.connectorIDs.includes(transaction1.connectorId)
        );
        const siteArea = testData.siteAreaContext.getSiteArea();
        const siteAreaLimitPerPhase = Utils.createDecimal(siteArea.maximumPower)
          .div(siteArea.voltage)
  ...
  describe('Test for Site Area Tree (Three phased site area is child of DC Site Area)', () => {
    beforeAll(() => {
      ...
      testData.siteAreaContext1 = testData.siteContext.getSiteAreaContext(
        ContextDefinition.SITE_AREA_CONTEXTS.WITH_SMART_CHARGING_THREE_PHASED
      );
      testData.chargingStationContext = testData.siteAreaContext.getChargingStationContext(
        ContextDefinition.CHARGING_STATION_CONTEXTS.ASSIGNED_OCPP16
      );
      testData.chargingStationContext1 = testData.siteAreaContext1.getChargingStationContext(
        ContextDefinition.CHARGING_STATION_CONTEXTS.ASSIGNED_OCPP16
      );
      testData.chargingStationContext2 = testData.siteAreaContext1.getChargingStationContext(
        ContextDefinition.CHARGING_STATION_CONTEXTS.ASSIGNED_OCPP16 +
          '-' +
          `${ContextDefinition.SITE_CONTEXTS.SITE_BASIC}-${ContextDefinition.SITE_AREA_CONTEXTS.WITH_SMART_CHARGING_THREE_PHASED}` +
          '-' +
    ...
    it('Check for three cars charging on different site areas if enough power is available', async () => {
      ...
      await testData.userService.siteAreaApi.update(testData.siteAreaContext1.getSiteArea());
      let transactionStartResponse = await testData.chargingStationContext.startTransaction(
        1,
        testData.userContext.tags[0].id,
        180,
        new Date()
      );
      let transactionResponse = await testData.centralUserService.transactionApi.readById(
        transactionStartResponse.transactionId
      );
      transaction = transactionResponse.data;
      transactionStartResponse = await testData.chargingStationContext1.startTransaction(
        1,
        testData.userContext.tags[0].id,
        180,
        new Date()
      );
      transactionResponse = await testData.centralUserService.transactionApi.readById(
        transactionStartResponse.transactionId
      );
      transaction1 = transactionResponse.data;
      transactionStartResponse = await testData.chargingStationContext2.startTransaction(
        1,
        testData.userContext.tags[0].id,
        180,
        new Date()
      ...
      const chargingProfiles =
        await smartChargingIntegrationWithoutPriorities.buildChargingProfiles(
          testData.siteAreaContext.getSiteArea()
        );
    ...
    it('Check for three cars charging on different site areas if not enough power is available on sub site area', async () => {
      ...
      await TestData.sendMeterValue(
        ...
        32,
        transaction2,
        testData.chargingStationContext2,
        { csPhase1: true, csPhase2: true, csPhase3: true }
      );
      const chargingProfiles =
        await smartChargingIntegrationWithoutPriorities.buildChargingProfiles(
  ...
  describe('Test limit adjustments, when charging stations are excluded', () => {
    ...
    it('Check if one charging connector of three phased charging station will be excluded from smart charging on three phased site area', async () => {
      testData.siteAreaContext.getSiteArea().maximumPower =
        22080 +
        Utils.createDecimal(limitMinThreePhased[0].limit)
          .mul(testData.siteAreaContext.getSiteArea().voltage)
          .toNumber();
      await testData.userService.siteAreaApi.update(testData.siteAreaContext.getSiteArea());
      await testData.chargingStationContext.startTransaction(
        1,
        testData.userContext.tags[0].id,
        180,
        new Date()
      );
      await testData.chargingStationContext1.startTransaction(
        1,
        testData.userContext.tags[0].id,
        180,
        new Date()
    ...
    it('Check if two charging connectors will be excluded also on parent site area level', async () => {
      testData.siteAreaContext.getSiteArea().maximumPower = 100000;
      testData.siteAreaContext1.getSiteArea().maximumPower =
        22080 +
        Utils.createDecimal(limitMinThreePhased[0].limit)
          .mul(testData.siteAreaContext.getSiteArea().voltage)
          .toNumber();
  ...
  describe('Test for Asset management', () => {
    ...
    it('Check if parent site area is limited with building consumption on sub site area  ', async () => {
      testData.siteAreaContext.getSiteArea().maximumPower = 200000;
      await testData.userService.siteAreaApi.update(testData.siteAreaContext.getSiteArea());
    ...
    it('Check if charging station is limited with backup value from building', async () => {
      testData.newAsset.lastConsumption = {
        timestamp: new Date('2021-02-05T14:16:19.001Z'),
        value: 0,
      };
    ...
    it('Check if solar panel production is added to capacity', async () => {
      ...
      (testData.newAsset.staticValueWatt = 50000), (testData.newAsset.fluctuationPercent = 50);
  ...
  describe('Test for Priorities', () => {
    ...
    it('Check if session with earlier departure time is prioritized', async () => {
      testData.siteAreaContext.getSiteArea().maximumPower = 22080;
      testData.siteAreaContext.getSiteArea().smartCharging = true;
      await testData.userService.siteAreaApi.update(testData.siteAreaContext.getSiteArea());
      let transactionStartResponse = await testData.chargingStationContext.startTransaction(
      ...
      transactionStartResponse = await testData.chargingStationContext.startTransaction(
        2,
        testData.userContext.tags[0].id,
        180,
        new Date()
      ...
      let chargingProfiles = await smartChargingIntegration.buildChargingProfiles(
        testData.siteAreaContext.getSiteArea()
      );
      expect(
        TestData.validateAndReturnChargingProfile(chargingProfiles, transaction).profile
      ...
      chargingProfiles = await smartChargingIntegration.buildChargingProfiles(
        testData.siteAreaContext.getSiteArea()
      );
      expect(
        TestData.validateAndReturnChargingProfile(chargingProfiles, transaction).profile
    ...
    it('Check if session with less SOC is prioritized', async () => {
      testData.siteAreaContext.getSiteArea().maximumPower = 22080;
      testData.siteAreaContext.getSiteArea().smartCharging = true;
      await testData.userService.siteAreaApi.update(testData.siteAreaContext.getSiteArea());
      let transactionStartResponse = await testData.chargingStationContext.startTransaction(
    ...
    it('Check if session with higher target SOC is prioritized', async () => {
      testData.siteAreaContext.getSiteArea().maximumPower = 22080;
      testData.siteAreaContext.getSiteArea().smartCharging = true;
      await testData.userService.siteAreaApi.update(testData.siteAreaContext.getSiteArea());
      let transactionStartResponse = await testData.chargingStationContext.startTransaction(
