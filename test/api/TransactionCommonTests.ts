...
export default class TransactionCommonTests {
  ...
  public async testReadNonExistingTransaction() {
    const response = await this.transactionUserService.transactionApi.readById(
  ...
  public async testReadTransactionOfUser(allowed = true, transactionTag: string) {
    ...
    const transactionResponse = await this.transactionUserService.transactionApi.readById(
  ...
  public async testReadSomeTransactionsInError() {
    ...
    const transactions = await this.transactionUserService.transactionApi.readAllInError({
      ChargingStationID: this.chargingStationContext.getChargingStation().id,
      ErrorType: TransactionInErrorType.NO_CONSUMPTION,
    });
    expect(transactions.status).to.equal(StatusCodes.OK);
    expect(transactions.data.count).to.equal(2);
    expect(transactions.data.result).to.containSubset([
      ...
      values: [
        ...
        },
        {
          ...
          cumulatedConsumptionAmps: Utils.convertWattToAmp(
            ...
            null,
            connectorId,
            meterValues[1].value + meterValues[0].value
          ),
        },
      ],
    });
  ...
  public async testReadConsumptionStoppedTransactionWithoutMeterValues() {
    ...
    expect(stopTransactionResponse).to.be.transactionStatus('Accepted');
    const consumption = await this.transactionUserService.transactionApi.readAllConsumption({
      TransactionId: transactionId,
    });
  ...
  public async testReadActiveTransactionsWithoutActiveTransactions() {
    const response = await this.transactionUserService.transactionApi.readAllActive({
      ChargingStationID: this.chargingStationContext.getChargingStation().id,
    });
  ...
  public async testReadActiveTransactionsWithMultipleActiveTransactions() {
    ...
    const transactions = await this.transactionUserService.transactionApi.readAllActive({
      ChargingStationID: this.chargingStationContext.getChargingStation().id,
    });
    expect(transactions.status).to.equal(StatusCodes.OK);
    expect(transactions.data.count).to.equal(2);
  ...
  public async testDeleteNotExistingTransaction() {
    const response = await this.transactionUserService.transactionApi.delete(
      faker.datatype.number(100000)
    );
  ...
  public async testDeleteStartedTransaction(allowed = true) {
    ...
    expect(startTransactionResponse).to.be.transactionValid;
    const transactionId = startTransactionResponse.transactionId;
    const transactionDeleted = await this.transactionUserService.transactionApi.delete(
      transactionId
    );
    if (allowed) {
  ...
  public async testMultiDeleteNotFoundTransactions() {
    // Only non-existent
    const response = await this.transactionUserService.transactionApi.deleteMany([
      faker.datatype.number(100000),
      faker.datatype.number(100000),
    ]);
    expect(response.status).to.equal(StatusCodes.NOT_FOUND);
    ...
    expect(stopTransactionResponse).to.be.transactionStatus('Accepted');
    // Delete
    const transactionsDeleted = await this.transactionUserService.transactionApi.deleteMany([
      transactionId,
      faker.datatype.number(100000),
    ]);
  ...
  public async testMultiDeleteExistingTransactions(allowed = true) {
    ...
    expect(stopTransactionResponse).to.be.transactionStatus('Accepted');
    // Transaction 2
    const startTransactionResponse2 = await this.chargingStationContext.startTransaction(
      connectorId,
      tagId,
      meterStart,
      startDate.toDate()
    ...
    const stopTransactionResponse2 = await this.chargingStationContext.stopTransaction(
      transactionId2,
      tagId,
      meterStop,
      stopDate.toDate()
  ...
  public async testMultiDeleteValidTransactions() {
    ...
    expect(transactionsDeleted.data.inSuccess).to.equal(2);
    expect(transactionsDeleted.data.inError).to.equal(0);
    let transactionResponse = await this.transactionUserService.transactionApi.readById(
      transactionId
    );
    expect(transactionResponse.status).to.equal(StatusCodes.NOT_FOUND);
    transactionResponse = await this.transactionUserService.transactionApi.readById(
      secondTransactionId
    );
    expect(transactionResponse.status).to.equal(StatusCodes.NOT_FOUND);
  }
  ...
  public async testSendMailNotificationWhenStartingTransaction() {
    ...
    expect(
      await this.centralUserService.mailApi.isMailReceived(
        this.transactionUser.email,
        'transaction-started'
      )
    ).is.equal(true, 'transaction-started mail');
    expect(
      await this.centralUserService.mailApi.isMailReceived(
        this.transactionUser.email,
        'end-of-charge'
      )
    ).is.equal(true, 'end-of-charge mail');
    const stopTransactionResponse = await this.chargingStationContext.stopTransaction(
      transactionId,
      tagId,
      cumulated + 50,
  ...
  public async testIsAuthorizedOnStartedTransaction(
    allowed: boolean,
    canStop?: boolean,
    canRead?: boolean,
  ...
  public async testIsAuthorizedToStopTransaction(
    allowed: boolean,
    canStop?: boolean,
    transactionTag?: string
  ) {
  ...
  public async testSessionsAmountIncreaseByOne(params) {
    ...
    const startDate = moment().toDate();
    const stopDate = moment(startDate).add(1, 'hour');
    const transactionCompletedBeforeResponse =
      await this.centralUserService.transactionApi.readAllCompleted(params);
    const startTransactionResponse = await this.chargingStationContext.startTransaction(
      connectorId,
      tagId,
      meterStart,
    ...
    expect(stopTransactionResponse).to.be.transactionStatus('Accepted');
    const transactionCompletedAfterResponse =
      await this.centralUserService.transactionApi.readAllCompleted(params);
    expect(transactionCompletedAfterResponse.data.count).to.be.eq(
      transactionCompletedBeforeResponse.data.count + 1
    );
  }

  public async testExportTransactionsToRefund(params) {
    const response = await this.centralUserService.transactionApi.exportTransactionsToRefund(
      params
    );
    const transactionsToRefund = await this.centralUserService.transactionApi.readAllToRefund(
      params
    );
