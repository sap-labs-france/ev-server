...
import {
  BillingPlatformFeeStrategyFactory,
  BillingTransferFactory,
} from '../factories/BillingFactory';
  ...
  describe('Where the admin user', () => {
    describe('Connected Accounts', () => {
      ...
      it('should create an account in an IDLE state and list it', async () => {
        ...
        const response = await billingTestHelper
          ...
          .billingApi.readBillingAccounts({
            userID: billingTestHelper.getCurrentUserContext().id,
            ID: accountID,
          });
        expect(response.status).to.be.eq(StatusCodes.OK);
        expect(response.data.result.map((account: BillingAccount) => account.id)).to.include(
          accountID
        );
      });

      it('should not activate an inexistent account', async () => {
        const activationResponse = await billingTestHelper
          .getCurrentUserService()
          .billingApi.activateBillingAccount({
            accountID: '5ce249a1a39ae1c056c389bd',
      ...
      it('should create a company assigned to a account', async () => {
        ...
        companyResponse = await billingTestHelper
          .getCurrentUserService()
          .companyApi.readById(companyResponse.data.id);
        expect(companyResponse.data.accountData.accountID).to.eq(billingAccount.id);
        expect(companyResponse.data.accountData.platformFeeStrategy).to.deep.eq(
          platformFeeStrategy
        );
      });
      ...
      it('should assign an account to a site', async () => {
        const billingAccount = await billingTestHelper.getActivatedAccount();
        // Create a company
        const companyResponse = await billingTestHelper
          .getCurrentUserService()
          .companyApi.create(CompanyFactory.build());
        expect(companyResponse.status).to.be.eq(StatusCodes.OK);
        // Create a site
        const platformFeeStrategy = BillingPlatformFeeStrategyFactory.build();
        ...
        siteResponse = await billingTestHelper
          .getCurrentUserService()
          .siteApi.readById(siteResponse.data.id as string);
      ...
      it('should update a site to assign a account', async () => {
        ...
        siteResponse = await billingTestHelper
          .getCurrentUserService()
          .siteApi.readById(siteID as string);
        expect(siteResponse.data.accountData.accountID).to.eq(billingAccount.id);
        expect(siteResponse.data.accountData.platformFeeStrategy).to.deep.eq(platformFeeStrategy);
      });
      ...
      it('should not able to send account onboarding for an activated account', async () => {
        ...
        const response = await billingTestHelper
          .getCurrentUserService()
          .billingApi.onboardBillingAccount(billingAccount.id);
    ...
    describe('Transfers', () => {
      ...
      it('should list transfers', async () => {
        ...
        const transferID = await BillingStorage.saveTransfer(
          billingTestHelper.getTenant(),
          transfer
        );
        ...
        const transfersResponse = await billingTestHelper
          .getCurrentUserService()
          .billingApi.readTransfers({ ID: transferID });
      ...
      it('should finalize transfer', async () => {
        ...
        transfer.id = transferID;
        const finalizeResponse = await billingTestHelper
          .getCurrentUserService()
          .billingApi.finalizeTransfer(transferID);
        expect(finalizeResponse.status).to.be.eq(StatusCodes.OK);
        const finalizedTransfer = await BillingStorage.getTransfer(
          billingTestHelper.getTenant(),
          transferID
        );
      ...
      it('should send a transfer', async () => {
        ...
        const finalizeResponse = await billingTestHelper
          .getCurrentUserService()
          .billingApi.finalizeTransfer(transfer.id);
        expect(finalizeResponse.status).to.be.eq(StatusCodes.OK);
        if (FeatureToggles.isFeatureActive(Feature.BILLING_PLATFORM_USE_EXPRESS_ACCOUNT)) {
          ...
          const sendResponse = await billingTestHelper
            .getCurrentUserService()
            .billingApi.sendTransfer(transfer.id);
    ...
    describe('Invoicing', () => {
      ...
      describe('with Periodic Billing ON ', () => {
        beforeAll(async () => {
          ...
          await billingTestHelper.setBillingSystemValidCredentials(
            true,
            false /* immediateBillingAllowed OFF */
          );
        ...
        it('should create an invoice, and get transfers generated', async () => {
          ...
          const transactionID1 = await billingTestHelper.generateTransactionAndCheckBillingStatus(
            BillingInvoiceStatus.DRAFT
          );
          const transactionID2 = await billingTestHelper.generateTransactionAndCheckBillingStatus(
            BillingInvoiceStatus.DRAFT
          );
          ...
          const billingDataStop1 = await billingTestHelper.checkTransactionBillingData(
            transactionID1,
            BillingInvoiceStatus.PAID
          );
          const billingDataStop2 = await billingTestHelper.checkTransactionBillingData(
            transactionID2,
            BillingInvoiceStatus.PAID
          );
          // Check billing data
          assert(
            billingDataStop1.invoiceID === billingDataStop2.invoiceID,
            'The two transactions should be billed by a single invoice'
          );
          await billingTestHelper.checkInvoiceData(
            billingDataStop1.invoiceID,
            BillingInvoiceStatus.PAID,
            2,
            20.16
          );
          ...
          await billingTestHelper.checkTransferData(
            transferID,
            BillingTransferStatus.FINALIZED,
            2,
            20.16,
            1.68,
            18.48
      ...
      describe('with Immediate Billing ON ', () => {
        ...
        it('should create an invoice, pay for it immediately and get a transfer generated or updated', async () => {
          ...
          const transactionID = await billingTestHelper.generateTransactionAndCheckBillingStatus(
            BillingInvoiceStatus.PAID
          );
          // // Explicit call to dispatch collected funds
          await billingTestHelper.billingImpl.dispatchCollectedFunds({ forceOperation: true });
          // The transaction should now have a different status and know the final invoice number
          const billingDataStop = await billingTestHelper.checkTransactionBillingData(
            transactionID,
            BillingInvoiceStatus.PAID
          );
          // Check billing data
          await billingTestHelper.checkInvoiceData(
            billingDataStop.invoiceID,
            BillingInvoiceStatus.PAID,
            1,
            10.08
          );
  ...
  describe('Where basic user', () => {
    ...
    describe('Connected Accounts', () => {
      ...
      it('should not be able to read account', async () => {
        ...
        const response = await billingTestHelper
          .getCurrentUserService()
          .billingApi.readBillingAccount('62978713f146ea8cb3bf8a95'); // inexistent account
        expect(response.status).to.be.eq(StatusCodes.FORBIDDEN);
      });
      ...
      it('should not be able to send account onboarding', async () => {
        ...
        const response = await billingTestHelper
          .getCurrentUserService()
          .billingApi.onboardBillingAccount('62978713f146ea8cb3bf8a95'); // inexistent account
        expect(response.status).to.be.eq(StatusCodes.FORBIDDEN);
      });
    });
    ...
    describe('Transfers', () => {
      ...
      it('should not be able to finalize a transfer', async () => {
        const finalizeResponse = await billingTestHelper
          .getCurrentUserService()
          .billingApi.finalizeTransfer('5ce249a1a39ae1c056c389bd'); // inexistent transfer
      ...
      it('should not be able to send a transfer invoice', async () => {
        const sendResponse = await billingTestHelper
          .getCurrentUserService()
          .billingApi.sendTransfer('5ce249a1a39ae1c056c389bd'); // inexistent transfer
