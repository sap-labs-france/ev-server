...
describe('User', () => {
    ...
    describe('When admin user', () => {
      ...
      describe('Using various basic APIs', () => {
        it('Should have accepted the Eula', async () => {
          ...
          const response = await testData.userService._baseApi.send({
            ...
            url:
              '/v1/auth/' +
              RESTServerRoute.REST_END_USER_LICENSE_AGREEMENT_CHECK +
              `?Email=${testData.userContext.email}&Tenant=${
                testData.tenantContext.getTenant().subdomain
              }`,
            headers: {
        ...
        it('Should find the created user by id', async () => {
          ...
          await testData.userService.getEntityById(testData.userService.userApi, testData.newUser);
        });
        ...
        it('Should be able to update the user', async () => {
          ...
          // Update
          await testData.userService.updateEntity(testData.userService.userApi, {
            ...testData.newUser,
        ...
        it('Should be able to delete the created user', async () => {
          ...
          await testData.userService.deleteEntity(testData.userService.userApi, testData.newUser);
        });
      ...
      describe('Using function "readAllInError"', () => {
        it('Should not find an active user in error', async () => {
          ...
          await testData.userService.deleteEntity(testData.userService.userApi, user);
        });
  ...
  describe('With component Organization (utbilling)', () => {
    beforeAll(async () => {
      testData.tenantContext = await ContextProvider.defaultInstance.getTenantContext(
        ContextDefinition.TENANT_CONTEXTS.TENANT_BILLING
      );
      testData.centralUserContext = testData.tenantContext.getUserContext(
        ContextDefinition.USER_CONTEXTS.DEFAULT_ADMIN
      );
    ...
    describe('Using various basic APIs', () => {
      describe('When admin user', () => {
        ...
        it('Should be able to set freeAccess flag on basic user', async () => {
          ...
          // Let's create a user
          await testData.userService.createEntity(testData.userService.userApi, fakeUser);
          ...
          await testData.userService.updateEntity(testData.userService.userApi, fakeUser, false);
          ...
          await testData.userService.deleteEntity(testData.userService.userApi, {
  ...
  describe('Car component is active (utcar)', () => {
    beforeAll(async () => {
      testData.tenantContext = await ContextProvider.defaultInstance.getTenantContext(
        ContextDefinition.TENANT_CONTEXTS.TENANT_CAR
      );
      testData.centralUserContext = testData.tenantContext.getUserContext(
        ContextDefinition.USER_CONTEXTS.DEFAULT_ADMIN
      );
    ...
    describe('Get user session context', () => {
      ...
      beforeAll(async () => {
        carCatalogID = (
          await testData.centralUserService.carApi.readCarCatalogs(
            {},
      ...
      describe('Given a user with no tag and no car', () => {
        ...
        test('When getting his session context for (any connector), (any charging station) and (non null carID)', async () => {
          const randomCarID = (
            await testData.userService.carApi.create(Factory.car.build({ carCatalogID }))
          ).data.id;
          const response = await testData.userService.userApi.getUserSessionContext({
            userID: user.id,
            chargingStationID: testData.chargingStationContext.getChargingStation().id,
        ...
        test('When getting his session context for (any connector), (any charging station) and (non null tagID)', async () => {
          const randomTagID = (await testData.userService.tagApi.createTag(Factory.tag.build()))
            .data.id;
          const response = await testData.userService.userApi.getUserSessionContext({
            userID: user.id,
            chargingStationID: testData.chargingStationContext.getChargingStation().id,
      ...
      describe('Given a user with 2 cars and 2 tags', () => {
        ...
        beforeAll(async () => {
          ...
          const createDefaultCarID = await CarStorage.saveCar(
