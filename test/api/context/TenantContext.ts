...
export default class TenantContext {
  ...
  async initialize(tokenID: string = null, siteAreaID: string = null) {
    ...
    this.ocpp16 = new OCPPJsonService16(
      `${config.get('ocpp.json.scheme')}://${config.get('ocpp.json.host')}:${config.get(
        'ocpp.json.port'
      )}/OCPP16/${this.tenant.id}/${tokenID}`,
      this.ocppRequestHandler
    );
    this.ocpp15 = new OCPPJsonService15(
      `${config.get('ocpp.soap.scheme')}://${config.get('ocpp.soap.host')}:${config.get(
        'ocpp.soap.port'
      )}/OCPP15?TenantID=${this.tenant.id}%26Token=${tokenID}`
    );
  }
  ...
  async getOCPPServiceForContextCreation(
    ocppVersion: string,
  ...
  getSiteContext(siteName = null) {
    if (siteName) {
      return this.context.siteContexts
        .concat(this.context.createdSites)
        .find((siteContext) => siteContext.getSiteName() === siteName);
    }
  ...
  getChargingStation(chargingStationID) {
    ...
    return this.context.chargingStations.find(
  ...
  async cleanUpCreatedData() {
    ...
    for (const chargingStation of this.context.createdChargingStations) {
      ...
      // Delete CS
      await this.centralAdminServerService.deleteEntity(
        this.centralAdminServerService.chargingStationApi,
        chargingStation,
        false
      );
    }
    ...
    for (const siteArea of this.context.createdSiteAreas) {
      ...
      // Delete
      await this.getAdminCentralServerService().deleteEntity(
        this.getAdminCentralServerService().siteAreaApi,
    ...
    for (const company of this.context.createdCompanies) {
      await this.centralAdminServerService.deleteEntity(
        this.centralAdminServerService.companyApi,
        company
      );
    }
    for (const user of this.context.createdUsers) {
      await this.centralAdminServerService.deleteEntity(
        this.centralAdminServerService.userApi,
        user
      );
    }
    for (const site of this.context.createdSites) {
      ...
      // Delete
      await this.centralAdminServerService.deleteEntity(
        this.centralAdminServerService.siteApi,
        site.getSite()
      );
    }
  }
  ...
  getUserContext(params) {
    ...
    if (params.id || params.email) {
      return this.context.users.find(
        (user) => user.id === params.id || user.email === params.email
      );
    }
    return this.context.users.find((user) => {
      let conditionMet = null;
      for (const key in params) {
        const userContextDef = ContextDefinition.TENANT_USER_LIST.find(
        ...
        if (Utils.objectHasProperty(user, key)) {
          ...
          if (conditionMet !== null) {
            conditionMet =
              conditionMet &&
              (userContextDef ? params[key] === userContextDef.assignedToSite : false);
          } else {
          ...
          if (conditionMet !== null) {
            conditionMet =
              conditionMet &&
              (params[key] ? !Utils.isEmptyArray(user.tags) : Utils.isEmptyArray(user.tags));
          } else {
            conditionMet = params[key]
              ? !Utils.isEmptyArray(user.tags)
              : Utils.isEmptyArray(user.tags);
          }
        }
      }
  ...
  async createUser(user = Factory.user.build(), loggedUser = null) {
    const createdUser = await this.centralAdminServerService.createEntity(
      this.centralAdminServerService.userApi,
      user
    );
    ...
    if (!Utils.objectHasProperty(createdUser, 'centralServerService')) {
      createdUser.centralServerService = new CentralServerService(
  ...
  async createCompany(company = Factory.company.build(), loggedUser = null) {
    const createdCompany = await this.centralAdminServerService.createEntity(
      this.centralAdminServerService.companyApi,
      company
    );
  ...
  async createSite(
    ...
    const createdSite = await this.centralAdminServerService.createEntity(
      this.centralAdminServerService.siteApi,
      site
    );
  ...
  async createSiteArea(site, chargingStations, siteArea) {
    siteArea.siteID =
      site && site.id
        ? !siteArea.siteID || siteArea.siteID !== site.id
          ? site.id
          : siteArea.siteID
        : null;
    siteArea.chargeBoxIDs =
      Array.isArray(chargingStations) && Utils.isEmptyArray(siteArea.chargeBoxIDs)
        ? chargingStations.map((chargingStation) => chargingStation.id)
        : [];
    const createdSiteArea = await this.centralAdminServerService.createEntity(
      this.centralAdminServerService.siteAreaApi,
      siteArea
    );
  ...
  async createChargingStation(
    ocppVersion,
    chargingStation = Factory.chargingStation.build({
    ...
    let createdChargingStation = await this.getAdminCentralServerService().getEntityById(
      this.getAdminCentralServerService().chargingStationApi,
      chargingStation
    );
    ...
    for (let i = 0; i < (connectorsDef ? connectorsDef.length : 2); i++) {
      createdChargingStation.connectors[i] = {
        connectorId: i + 1,
        status:
          connectorsDef && connectorsDef[i].status
            ? connectorsDef[i].status
            : ChargePointStatus.AVAILABLE,
        errorCode:
          connectorsDef && connectorsDef[i].errorCode ? connectorsDef[i].errorCode : 'NoError',
        timestamp:
          connectorsDef && connectorsDef[i].timestamp
            ? connectorsDef[i].timestamp
        ...
        numberOfConnectedPhase:
          connectorsDef && connectorsDef[i].numberOfConnectedPhase
            ? connectorsDef[i].numberOfConnectedPhase
            : 3,
    ...
    expect(createdChargingStation.connectors[1].power).to.eql(
      22080,
      `Connector ID 2 of charging station ${createdChargingStation.id} must have 22080 W`
    );
    expect(createdChargingStation.connectors[1].amperage).to.eql(
      96,
      `Connector ID 2 of charging station ${createdChargingStation.id} must have 96 A`
    );
    expect(createdChargingStation.connectors[1].type).to.eql(
      'T2',
    ...
    // Charge Points
    expect(createdChargingStation.chargePoints.length).to.eql(
      1,
    ...
    expect(createdChargingStation.chargePoints[0].numberOfConnectedPhase).to.eql(
      3,
      `Charge Point ID 1 of charging station ${createdChargingStation.id} must have 3 phases`
    );
    expect(createdChargingStation.chargePoints[0].cannotChargeInParallel).to.eql(
      false,
      `Charge Point ID 1 of charging station ${createdChargingStation.id} cannot charge in parallel`
    );
    expect(createdChargingStation.chargePoints[0].sharePowerToAllConnectors).to.eql(
      false,
      `Charge Point ID 1 of charging station ${createdChargingStation.id} cannot share power`
    );
    expect(createdChargingStation.chargePoints[0].excludeFromPowerLimitation).to.eql(
      false,
      `Charge Point ID 1 of charging station ${createdChargingStation.id} cannot be excluded from power limitation`
    );
    expect(createdChargingStation.chargePoints[0].ocppParamForPowerLimitation).to.eql(
      'maxintensitysocket',
  ...
  async createSinglePhasedChargingStation(
    ocppVersion,
    chargingStation = Factory.chargingStation.buildChargingStationSinglePhased({
      id: faker.random.alphaNumeric(12),
    }),
    connectorsDef = null,
    siteArea: SiteArea = null
  ...
  async createChargingStationDC(
    ...
    expect(createdChargingStation.chargePoints[0].efficiency).to.eql(
      95,
  ...
  async getRegistrationToken(siteAreaID: string): Promise<RegistrationToken> {
    const registrationTokenResponse = await this.centralAdminServerService.registrationApi.readAll({
      SiteAreaID: siteAreaID,
    });
    expect(registrationTokenResponse.status).eq(StatusCodes.OK);
    expect(registrationTokenResponse.data).not.null;
  ...
  findSiteContextFromSiteArea(siteArea) {
    return this.getSiteContexts().find((context) =>
      context.siteAreas.find((tmpSiteArea) => siteArea.id === tmpSiteArea.id)
    );
  }

  findSiteContextFromChargingStation(chargingStation) {
    return this.getSiteContexts().find((context) =>
      context.chargingStations.find(
        (tmpChargingStation) => tmpChargingStation.id === chargingStation.id
      )
    );
  }
