...
export default class ContextProvider {
  ...
  async _init(): Promise<void> {
    if (!this.initialized) {
      // Read all tenants
      this.tenantEntities = (
        await this.superAdminCentralServerService.tenantApi.readAll(
          { WithComponents: true },
    ...
    try {
      ...
      if (tenantContextNames) {
        ...
        tenantContexts = tenantContextNames.map((tenantName) =>
          ContextDefinition.TENANT_CONTEXT_LIST.find((tenantContext) => {
  ...
  async _tenantEntityContext(tenantContextDef): Promise<TenantContext> {
    ...
    const tenantEntity = this.tenantEntities.find(
      (tenant) => tenant.name === tenantContextDef.tenantName
    );
    ...
    if (
      tenantEntity.components &&
      tenantEntity.components[TenantComponents.ORGANIZATION] &&
      tenantEntity.components[TenantComponents.ORGANIZATION].active
    ) {
      siteAreaList = (
        await defaultAdminCentralServiceService.siteAreaApi.readAll({}, { limit: 0, skip: 0 })
      ).data.result;
      siteList = (
        await defaultAdminCentralServiceService.siteApi.readAll({}, { limit: 0, skip: 0 })
      ).data.result;
      companyList = (
        await defaultAdminCentralServiceService.companyApi.readAll({}, { limit: 0, skip: 0 })
      ).data.result;
      chargingStationList = (
        await defaultAdminCentralServiceService.chargingStationApi.readAll(
          {},
          TestConstants.DEFAULT_PAGING
        )
      ).data.result;
    ...
    userList = (await defaultAdminCentralServiceService.userApi.readAll({}, { limit: 0, skip: 0 }))
      .data.result;
    tagList = (await defaultAdminCentralServiceService.tagApi.readTags({}, { limit: 0, skip: 0 }))
      .data.result;
    ...
    const newTenantContext = new TenantContext(
      ...
      tenantEntity,
      '',
      defaultAdminCentralServiceService,
      null
    );
    ...
    if (
      ...
      for (const siteContextDef of ContextDefinition.TENANT_SITE_LIST) {
        ...
        const siteAreas = siteAreaList.filter(
          (siteArea) => siteContext.getSite().id === siteArea.siteID
        );
        for (const siteArea of siteAreas) {
          const siteAreaContext = siteContext.addSiteArea(siteArea);
          const chargingStations = chargingStationList.filter(
            (chargingStation) => siteArea.id === chargingStation.siteAreaID
          );
    ...
    const chargingStations = chargingStationList.filter(
      (chargingStation) => !chargingStation.siteAreaID
    );
    for (const chargingStation of chargingStations) {
    ...
    const unregisteredChargingStation15 = await Factory.chargingStation.build({
      id: ContextDefinition.CHARGING_STATION_CONTEXTS.UNREGISTERED_OCPP15,
    ...
    const unregisteredChargingStation16 = await Factory.chargingStation.build({
      id: ContextDefinition.CHARGING_STATION_CONTEXTS.UNREGISTERED_OCPP16,
      ocppVersion: OCPPVersion.VERSION_16,
    });
    await newTenantContext.addChargingStation(unregisteredChargingStation16, registrationToken);
    const invalidChargingStation15 = await Factory.chargingStation.build({
      id: ContextDefinition.CHARGING_STATION_CONTEXTS.INVALID_IDENTIFIER_OCPP15,
      ocppVersion: OCPPVersion.VERSION_15,
    });
    await newTenantContext.addChargingStation(invalidChargingStation15);
    const invalidChargingStation16 = await Factory.chargingStation.build({
      id: ContextDefinition.CHARGING_STATION_CONTEXTS.INVALID_IDENTIFIER_OCPP16,
      ocppVersion: OCPPVersion.VERSION_16,
    });
    await newTenantContext.addChargingStation(invalidChargingStation16);
    const unknownChargingStation15 = await Factory.chargingStation.buildChargingStationUnknown({
      id: ContextDefinition.CHARGING_STATION_CONTEXTS.UNKNOWN_OCPP15,
      ocppVersion: OCPPVersion.VERSION_15,
    });
    await newTenantContext.addChargingStation(unknownChargingStation15);
    const unknownChargingStation16 = await Factory.chargingStation.buildChargingStationUnknown({
      id: ContextDefinition.CHARGING_STATION_CONTEXTS.UNKNOWN_OCPP16,
      ocppVersion: OCPPVersion.VERSION_16,
    });
  ...
  _getTenantContextDef(tenantContextName, checkValid = true) {
    const tenantContext = ContextDefinition.TENANT_CONTEXT_LIST.find(
      (context) => context.tenantName === tenantContextName
    );
  ...
  _getTenantContext(tenantContextName, checkValid = true) {
    ...
    return this.tenantsContexts.find(
      (tenantContext) => tenantContext.getTenant().name === tenantContextName
    );
  }
}
