...
export default class ContextProvider {
  ...
  async _init() {
    if (!this.initialized) {
      // Read all tenants
      this.tenantEntities = (await this.superAdminCentralServerService.tenantApi.readAll({}, { limit: Constants.DB_RECORD_COUNT_CEIL, skip: 0 })).data.result;
    }
  ...
  async _tenantEntityContext(tenantContextDef) {
    ...
    chargingStationList = (await defaultAdminCentralServiceService.chargingStationApi.readAll({}, { limit: Constants.DB_RECORD_COUNT_CEIL, skip: 0 })).data.result;
