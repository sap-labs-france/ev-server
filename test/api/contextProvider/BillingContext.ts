...
export default class BillingContext {
  ...
  public async createTestData() {
    ...
    for (let i = 0; i < 2; i++) {
      await billingImpl.createInvoice(this.tenantContext.getTenant().id, adminBillingUser, { description: 'TestAdmin' + (i + 1), amount: 1 });
      await billingImpl.createInvoice(this.tenantContext.getTenant().id, basicBillingUser, { description: 'TestBasic' + (i + 1), amount: 1 });
    }
  }
