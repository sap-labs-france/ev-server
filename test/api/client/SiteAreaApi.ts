...
export default class SiteAreaApi extends CrudApi {
  ...
  public async create(data) {
    const siteArea = await super.create(
      data,
      this.buildRestEndpointUrl(RESTServerRoute.REST_SITE_AREAS)
    );
    // Check User IDs
    if (data.chargeBoxIDs) {
      ...
      await super.create(
        ...
        this.buildRestEndpointUrl(RESTServerRoute.REST_SITE_AREA_ASSIGN_CHARGING_STATIONS, {
          id: siteArea.data.id,
        })
      );
    }
  ...
  public async update(data) {
    return super.update(
      data,
      this.buildRestEndpointUrl(RESTServerRoute.REST_SITE_AREA, { id: data.id })
    );
  }
  ...
  public async removeChargingStations(siteAreaID: string, chargingStationIDs: string[]) {
    return super.update(
      { chargingStationIDs },
      this.buildRestEndpointUrl(RESTServerRoute.REST_SITE_AREA_REMOVE_CHARGING_STATIONS, {
        id: siteAreaID,
      })
    );
  }
  ...
  public async assignAssets(siteAreaID: string, assetIDs: string[]) {
    return super.update(
      { assetIDs },
      this.buildRestEndpointUrl(RESTServerRoute.REST_SITE_AREA_ASSIGN_ASSETS, { id: siteAreaID })
    );
  }
  ...
  public async removeAssets(siteAreaID: string, assetIDs: string[]) {
    return super.update(
      { assetIDs },
      this.buildRestEndpointUrl(RESTServerRoute.REST_SITE_AREA_REMOVE_ASSETS, { id: siteAreaID })
    );
  }
}
