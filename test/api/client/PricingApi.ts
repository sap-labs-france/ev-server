...
export default class PricingApi extends CrudApi {
  ...
  public async readPricingDefinition(id: string) {
    return super.read({ ID: id }, this.buildRestEndpointUrl(ServerRoute.REST_PRICING_DEFINITION, { id }));
  }

  public async updatePricingDefinition(data: PricingDefinition) {
    return super.update(data, this.buildRestEndpointUrl(ServerRoute.REST_PRICING_DEFINITION, { id: data.id }));
  }

  public async createPricingDefinition(data: Partial<PricingDefinition>) {
    return super.create(data, this.buildRestEndpointUrl(ServerRoute.REST_PRICING_DEFINITIONS));
  }

  public async deletePricingDefinition(id: string) {
    return super.delete(id, this.buildRestEndpointUrl(ServerRoute.REST_PRICING_DEFINITION, { id }));
  }
