...
describe('OCPI Service Tests (utocpi)', () => {
  ...
  beforeAll(async () => {
    testData.tenantContext = await ContextProvider.defaultInstance.getTenantContext(
      ContextDefinition.TENANT_CONTEXTS.TENANT_OCPI
    );
  ...
  describe('Test /ocpi/cpo/2.1.1/locations', () => {
    ...
    describe('Access with paging', () => {
      ...
      it('should access url /ocpi/cpo/2.1.1/locations/ and have headers X-Limit and X-Total-Count', async () => {
        // Call
        const locationsResponse = await testData.cpoService.accessPath(
          'GET',
      ...
      it('should access url with paging /ocpi/cpo/2.1.1/locations/?offset=0&limit=20', async () => {
        ...
        const locationsResponse = await testData.cpoService.accessPath(
          'GET',
          '/ocpi/cpo/2.1.1/locations/?offset=0&limit=20'
        );
        expect(locationsResponse.data).to.not.be.empty;
        expect(locationsResponse.data).to.have.property('status_code', 1000);
        expect(locationsResponse.data).to.have.property('status_message', 'Success');
      ...
      it('should access url with paging /ocpi/cpo/2.1.1/locations/?offset=0&limit=1', async () => {
        ...
        const locationsResponse = await testData.cpoService.accessPath(
          'GET',
          '/ocpi/cpo/2.1.1/locations/?offset=0&limit=1'
        );
        expect(locationsResponse.data).to.not.be.empty;
        expect(locationsResponse.data).to.have.property('status_code', 1000);
        expect(locationsResponse.data).to.have.property('status_message', 'Success');
        expect(locationsResponse.data)
          .to.have.property('data')
          .to.be.an('array')
          .and.to.have.length(1);
        expect(locationsResponse.headers)
          .to.have.property('link')
          .to.match(/^<.*:\/\/.*:.*\/ocpi\/cpo\/.*\/locations\/\?offset=1&limit=1>; rel="next"/);
      });
    });
  });
  ...
  describe('Test single entity /ocpi/cpo/2.1.1/locations/...', () => {
    ...
    describe('Success cases', () => {
      ...
      it('should access single location entity /ocpi/cpo/2.1.1/locations/{locationId}', async () => {
        for (const location of response.data.data) {
          // Call
          const locationResponse = await testData.cpoService.accessPath(
            'GET',
            `/ocpi/cpo/2.1.1/locations/${location.id}`
          );
          // Check status
      ...
      it('should access single EVSE entity /ocpi/cpo/2.1.1/locations/{locationId}/{evseUid}', async () => {
        for (const location of response.data.data) {
          for (const evse of location.evses) {
            // Call
            const evseResponse = await testData.cpoService.accessPath(
              'GET',
              `/ocpi/cpo/2.1.1/locations/${location.id}/${evse.uid}`
            );
            // Check status
      ...
      it('should access single Connector entity /ocpi/cpo/2.1.1/locations/{locationId}/{evseUid}/{connectorId}', async () => {
        for (const location of response.data.data) {
          for (const evse of location.evses) {
            for (const connector of evse.connectors) {
              // Call
              const connectorResponse = await testData.cpoService.accessPath(
                'GET',
                `/ocpi/cpo/2.1.1/locations/${location.id}/${evse.uid}/${connector.id}`
              );
              // Check status
    ...
    describe('Failure cases', () => {
      ...
      it('should not find this non-existing location  /ocpi/cpo/2.1.1/locations/5abeba9e4bae1457eb565e67', async () => {
        ...
        const locationResponse = await testData.cpoService.accessPath(
          'GET',
          '/ocpi/cpo/2.1.1/locations/5abeba9e4bae1457eb565e67'
        );
        // Check status
        ...
        expect(locationResponse.data).to.have.property(
          'status_message',
          "Location ID '5abeba9e4bae1457eb565e67' not found"
        );
      });
      ...
      it('should not find this non-existing EVSE  /ocpi/cpo/2.1.1/locations/5ce249a2372f0b1c8caf9294/NonExistingSite', async () => {
        ...
        const locationResponse = await testData.cpoService.accessPath(
          'GET',
          '/ocpi/cpo/2.1.1/locations/5ce249a2372f0b1c8caf9294/NonExistingSite'
        );
        // Check status
        expect(locationResponse.status).to.be.eql(StatusCodes.NOT_FOUND);
        expect(locationResponse.data).to.have.property('timestamp');
        ...
        expect(locationResponse.data).to.have.property(
          'status_message',
          "EVSE UID not found 'NonExistingSite' in Location ID '5ce249a2372f0b1c8caf9294'"
        );
      });
      ...
      it('should not find this non-existing Connector  /ocpi/cpo/2.1.1/locations/5ce249a2372f0b1c8caf9294/cs-15-ut-site-withoutACL/0', async () => {
        ...
        const locationResponse = await testData.cpoService.accessPath(
          'GET',
          '/ocpi/cpo/2.1.1/locations/5ce249a2372f0b1c8caf9294/cs-15-ut-site-withoutACL/0'
        );
        // Check status
        expect(locationResponse.status).to.be.eql(StatusCodes.NOT_FOUND);
        expect(locationResponse.data).to.have.property('timestamp');
        ...
        expect(locationResponse.data).to.have.property(
          'status_message',
          "EVSE Connector ID '0' not found on Charging Station ID 'cs-15-ut-site-withoutACL' and Location ID '5ce249a2372f0b1c8caf9294'"
        );
      });
    });
  });
    ...
    describe('Success cases', () => {
      it('Should create a new ocpiEndpoint', async () => {
        ...
        testData.newOcpiEndpoint = await testData.centralUserService.createEntity(
          testData.centralUserService.ocpiEndpointApi,
          Factory.ocpiEndpoint.build()
        );
      });
