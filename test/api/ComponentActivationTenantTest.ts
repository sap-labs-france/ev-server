...
class TestData {
  ...
  public connectUser(): void {
    this.centralService = new CentralServerService(
...
describe('Tenant Settings', () => {
  ...
  describe('Success cases', () => {
    ...
    it('OCPI : Check that the setting has been created in the tenant after activation', async () => {
      // Fill in the data
      testData.data = {
        id: testData.credentials.tenantId,
        name: ContextDefinition.TENANT_CONTEXTS.TENANT_WITH_NO_COMPONENTS,
      ...
      expect(settings.data.result[1]).to.be.validatedSetting(
        TenantComponents.OCPI,
        RoamingSettingsType.OCPI
      );
    });

    it('Pricing/Simple : Check that the setting has been created in the tenant after activation', async () => {
      // Fill in the data
      testData.data = {
        id: testData.credentials.tenantId,
        name: ContextDefinition.TENANT_CONTEXTS.TENANT_WITH_NO_COMPONENTS,
      ...
      expect(settings.data.result[1]).to.be.validatedSetting(
        ...
        PricingSettingsType.SIMPLE
      );
    });

    it('Billing : Check that the setting has been created in the tenant after activation', async () => {
      // Fill in the data
      testData.data = {
        id: testData.credentials.tenantId,
        name: ContextDefinition.TENANT_CONTEXTS.TENANT_WITH_NO_COMPONENTS,
      ...
      expect(settings.data.result[0]).to.be.validatedSetting(
        TenantComponents.BILLING,
        BillingSettingsType.STRIPE
      );
      expect(settings.data.result[2]).to.be.validatedSetting(
        TenantComponents.PRICING,
        PricingSettingsType.SIMPLE
      );
    });

    it('Billing accounts : Check that the setting has been created in the tenant after activation', async () => {
      // Fill in the data
      testData.data = {
        id: testData.credentials.tenantId,
        name: ContextDefinition.TENANT_CONTEXTS.TENANT_WITH_NO_COMPONENTS,
    ...
    it('Refund : Check that the setting has been created in the tenant after activation', async () => {
      ...
      expect(settings.data.result[2]).to.be.validatedSetting(
        ...
        RefundSettingsType.CONCUR
      );
    });

    it('SmartCharging : Check that the setting has been created in the tenant after activation', async () => {
    ...
    it('Analytics : Check that the setting has been created in the tenant after activation', async () => {
      // Fill in the data
      ...
      expect(settings.data.count).to.equal(3);
      expect(settings.data.result[0]).to.be.validatedSetting(
        TenantComponents.ANALYTICS,
        AnalyticsSettingsType.SAC
      );
    });

    it('Asset : Check that the setting has been created in the tenant after activation', async () => {
