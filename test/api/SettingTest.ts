...
describe('Setting tests', function() {
  ...
  before(async function() {
    ...
    const response = await testData.superCentralService.tenantApi.readAll({ 'Search' : CONTEXTS.TENANT_CONTEXTS.TENANT_WITH_ALL_COMPONENTS }, { limit: Constants.UNLIMITED, skip: 0 });
  ...
  after(async function() {
    ...
    testData.data = JSON.parse(`{"id":"${testData.credentials.tenantId}","name": "${CONTEXTS.TENANT_CONTEXTS.TENANT_WITH_ALL_COMPONENTS}","email": "${testData.credentials.email}","subdomain":"utall","components":{"ocpi":{"active":true,"type":"gireve"},"organization":{"active":true,"type":null},"pricing":{"active":true,"type":"simple"},"refund":{"active":true,"type":"concur"},"statistics":{"active":true,"type":null},"analytics":{"active":true,"type":null}}}`);
