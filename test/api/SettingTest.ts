...
async function updatePricingWithSensitiveDataAndCheckResultSuccessful(): Promise<void> {
  const crtPricingData = await testData.centralService.settingApi.readByIdentifier({
    Identifier: 'refund',
  });
  ...
  const update = await testData.centralService.updateEntity(
    testData.centralService.settingApi,
    testData.data
  );
  expect(update.status).to.equal(StatusCodes.OK);
}

function getCryptoTestSettings(
  originalCryptoObject: SettingDB,
  newKeyProperties: CryptoKeyProperties
) {
  return JSON.parse(`{
...
async function getCurrentCryptoDataAndCheckResultSuccessful(): Promise<SettingDB> {
  const crtCryptoData = await testData.centralService.settingApi.readByIdentifier({
    Identifier: 'crypto',
  });
...
async function resetCryptoSettingToDefault(): Promise<void> {
  // Aes-256-gcm
...
async function updateCryptoSettingsAndCheckResultSuccessful(
  crtData,
  blockCypher,
  blockSize,
  operationMode
...
describe('Setting', () => {
  ...
  beforeAll(async () => {
    ...
    testData.centralService = new CentralServerService(
      ...
      { email: config.get('admin.username'), password: config.get('admin.password') }
    );
    testData.credentials.email = config.get('admin.username');
    // Retrieve the tenant id from the name
    const response = await testData.superCentralService.tenantApi.readAll(
      { Search: ContextDefinition.TENANT_CONTEXTS.TENANT_WITH_ALL_COMPONENTS },
      TestConstants.DEFAULT_PAGING
    );
    testData.credentials.tenantId = response ? response.data.result[0].id : '';
    initialTenant = (
      await testData.superCentralService.tenantApi.readById(testData.credentials.tenantId)
    ).data;
  ...
  afterAll(async () => {
    ...
    const res = await testData.superCentralService.updateEntity(
      testData.centralService.tenantApi,
      initialTenant
    );
  ...
  describe('Success cases (utall)', () => {
    it('Check that retrieving refund settings filtered by identifier returns just one result', async () => {
      ...
      const read = await testData.centralService.settingApi.readByIdentifier({
        Identifier: 'refund',
      });
    ...
    describe('Crypto settings update tests', () => {
      ...
      it('Check crypto settings update - change key', async () => {
        const crtCryptoData = await getCurrentCryptoDataAndCheckResultSuccessful();
        await updateCryptoSettingsAndCheckResultSuccessful(
          crtCryptoData,
          crtCryptoData.content.crypto.keyProperties.blockCypher,
          crtCryptoData.content.crypto.keyProperties.blockSize,
          crtCryptoData.content.crypto.keyProperties.operationMode
        );

        // Get setting with sensitive data after crypto setting update
        const readSettingAfter = await testData.centralService.settingApi.readAll(
          { Identifier: 'refund' },
          TestConstants.DEFAULT_PAGING
        );
  ...
  describe('Settings authorization tests', () => {
    describe('With admin user', () => {
      it('Should be able to read all settings', async () => {
        const read = await testData.centralService.settingApi.readAll(
          {},
          TestConstants.DEFAULT_PAGING
        );
        expect(read.status).to.equal(StatusCodes.OK);
    ...
    describe('Settings authorization tests - basic user', () => {
      ...
      it('Should not be able to read all settings', async () => {
        try {
          const read = await testData.userService.settingApi.readAll(
            {},
            TestConstants.DEFAULT_PAGING
          );
