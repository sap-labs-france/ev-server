...
export default class PrometheusMonitoringServer extends MonitoringServer {
  ...
  public constructor(monitoringConfig: MonitoringConfiguration) {
    ...
    if (process.env.K8S) {
      this.createGaugeMetric(
        Constants.WEB_SOCKET_QUEUED_REQUEST,
        'The number of web sockets that are queued'
      );
      this.createGaugeMetric(
        Constants.WEB_SOCKET_RUNNING_REQUEST,
        'The number of web sockets that are running'
      );
      this.createGaugeMetric(
        Constants.WEB_SOCKET_RUNNING_REQUEST_RESPONSE,
        'The number of web sockets request + response that are running'
      );
      this.createGaugeMetric(Constants.WEB_SOCKET_CURRENT_REQUEST, 'JSON WS Requests in cache');
      this.createGaugeMetric(
        Constants.WEB_SOCKET_OCPP_CONNECTIONS_COUNT,
        'number of json web sockets'
      );
      this.createGaugeMetric(
        Constants.MONGODB_CONNECTION_READY,
        'The number of connection that are ready'
      );
      this.createGaugeMetric(
        Constants.MONGODB_CONNECTION_CREATED,
        'The number of connection created'
      );
      this.createGaugeMetric(
        Constants.MONGODB_CONNECTION_CLOSED,
        'The number of connection closed'
      );
    }
    // Create HTTP Server
    this.expressApplication = ExpressUtils.initApplication();
    // Handle requests
    this.expressApplication.use('/metrics', (req: Request, res: Response, next: NextFunction) => {
      // Trace Request
      Logging.traceExpressRequest(req, res, next, ServerAction.MONITORING)
        .then(() => {
          // Process
  ...
  public start(): void {
    ...
    ServerUtils.startHttpServer(
      ...
      ServerUtils.createHttpServer(this.monitoringConfig, this.expressApplication),
      MODULE_NAME,
      ServerType.MONITORING_SERVER
    );
  }

  public getAvgClearableMetric(
    prefix: string,
    metricName: string,
    suffix: number,
    metricHelp: string,
  ...
  public getCountAvgClearableMetric(
    prefix: string,
    metricName: string,
    suffix: number,
    ...
    let metric: CountAvgGaugeClearableMetric = this.mapAvgGaugeClearableMetric.get(
      keyCount
    ) as CountAvgGaugeClearableMetric;
    if (metric) {
      return metric;
    }
    metric = new CountAvgGaugeClearableMetric(
      this.clientRegistry,
      keyAvg,
      keyCount,
      metricAvgHelp,
  ...
  public getCounterClearableMetric(
    prefix: string,
    metricName: string,
    metricHelp: string,
