...
export default class RemotePushNotificationTask implements NotificationTask {
  ...
  async sendPreparingSessionsAreStarted(data: PreparingSessionsAreStartedNotification, user: User, tenantID: string, severity: NotificationSeverity): Promise<void> {
    // Set the locale
    I18nManager.switchLocale(user.locale);
    // Get Message Text
    const title = i18n.t('notifications.preparingSessionsAreStarted.title');
    const body = i18n.t('notifications.preparingSessionsAreStarted.body',
