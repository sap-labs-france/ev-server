...
import {
  OICPAuthorizeStartCpoReceive,
  OICPAuthorizeStartCpoSend,
  OICPAuthorizeStopCpoReceive,
  OICPAuthorizeStopCpoSend,
} from '../../types/oicp/OICPAuthorize';
import {
  OICPChargingNotification,
  OICPErrorClass,
  OICPStatusCode,
} from '../../types/oicp/OICPStatusCode';
import {
  OICPChargingNotificationEndCpoSend,
  OICPChargingNotificationErrorCpoSend,
  OICPChargingNotificationProgressCpoSend,
  OICPChargingNotificationStartCpoSend,
} from '../../types/oicp/OICPChargingNotifications';
import {
  OICPDefaultTagId,
  OICPIdentification,
  OICPSessionID,
...
import {
  ...
  OICPOperatorEvseData,
  OICPOperatorEvseStatus,
...
export default class CpoOICPClient extends OICPClient {
  ...
  public async updateSession(transaction: Transaction): Promise<void> {
    ...
    transaction.oicpData.session.kwh = Utils.createDecimal(transaction.currentTotalConsumptionWh)
      .div(1000)
      .toNumber();
    transaction.oicpData.session.last_updated = transaction.currentTimestamp;
    transaction.oicpData.session.total_cost =
      transaction.currentCumulatedPrice > 0 ? transaction.currentCumulatedPrice : 0;
  ...
  public async stopSession(transaction: Transaction): Promise<void> {
    ...
    transaction.oicpData.session.kwh = Utils.createDecimal(transaction.stop.totalConsumptionWh)
      .div(1000)
      .toNumber();
    transaction.oicpData.session.total_cost =
      transaction.stop.roundedPrice > 0 ? transaction.stop.roundedPrice : 0;
  ...
  public async sendEVSEs(partial = false, actionType?: OICPActionType): Promise<OICPResult> {
    ...
    do {
      ...
      sites = (
        await SiteStorage.getSites(
          this.tenant,
          { public: true },
          { skip: currentSiteSkip, limit: Constants.DB_RECORD_COUNT_DEFAULT }
        )
      ).result;
          ...
          do {
            ...
            chargingStations = (
              await ChargingStationStorage.getChargingStations(
                this.tenant,
                { siteIDs: [site.id], public: true, withSiteArea: true },
                { skip: currentChargingStationSkip, limit: Constants.DB_RECORD_COUNT_DEFAULT }
              )
            ).result;
            if (!Utils.isEmptyArray(chargingStations)) {
              // Convert (public) charging stations to OICP EVSEs
              const evses = await OICPUtils.convertChargingStationsToEVSEs(
                this.tenant,
                site,
                chargingStations,
                options
              ...
              if (!partial) {
                ...
                chargeBoxIDsToProcessFromInput = evsesToProcess.map(
                  (evse) => evse.ChargingStationID
                );
              } else {
                ...
                chargeBoxIDsToProcess.push(
                  ...(await this.getChargeBoxIDsWithNewStatusNotifications())
                );
                // Remove duplicates
              ...
              if (evsesToProcess) {
                ...
                do {
                  ...
                  const evseIDBatch = chargeBoxIDsToProcessFromInput.splice(
                    0,
                    OICPBatchSize.EVSE_DATA
                  );
                  try {
                    ...
                    result.logs.push(
                      `Failed to update the EVSEs from tenant '${this.tenant.id}': ${String(
                        error.message
                      )}`
                    );
                  }
    ...
    if (result.failure > 0) {
      NotificationHandler.sendOICPPatchChargingStationsError(this.tenant, {
    ...
    // Save
    const executionDurationSecs = Utils.createDecimal(new Date().getTime())
      .minus(startTime)
      .div(1000)
      .toNumber();
    await OICPEndpointStorage.saveOicpEndpoint(this.tenant, this.oicpEndpoint);
    await Logging.logOicpResult(
      this.tenant.id,
      ServerAction.OICP_PUSH_EVSE_DATA,
      MODULE_NAME,
      'sendEVSEs',
      result,
  ...
  public async sendEVSEStatuses(partial = false, actionType?: OICPActionType): Promise<OICPResult> {
        ...
        for (const site of sites) {
          ...
          do {
            ...
            if (!Utils.isEmptyArray(chargingStations)) {
              ...
              const evseStatuses = OICPUtils.convertChargingStationsToEvseStatuses(
                chargingStations,
                options
              );
              ...
              if (!partial) {
                ...
                chargeBoxIDsToProcessFromInput = evseStatusesToProcess.map(
                  (evseStatus) => evseStatus.ChargingStationID
                );
              } else {
                let chargeBoxIDsToProcess = [];
                // Get ChargingStation in Failure from previous run
              ...
              if (!Utils.isEmptyArray(evseStatusesToProcess)) {
                try {
                  ...
                  result.logs.push(
                    `Failed to update the EVSE Statuses from tenant '${this.tenant.id}': ${String(
                      error.message
                    )}`
                  );
                }
                if (result.failure > 0) {
                  // Send notification to admins
                  NotificationHandler.sendOICPPatchChargingStationsStatusesError(this.tenant, {
                    evseDashboardURL: Utils.buildEvseURL(this.tenant.subdomain),
                  }).catch((error) => {
                    Logging.logPromiseError(error, this.tenant?.id);
                  });
    ...
    await Logging.logOicpResult(
      this.tenant.id,
      ServerAction.OICP_PUSH_EVSE_STATUSES,
      MODULE_NAME,
      'sendEVSEStatuses',
      result,
  ...
  public async pushEvseData(
    evses: OICPEvseDataRecord[],
    actionType: OICPActionType
  ): Promise<OICPAcknowledgment> {
    this.axiosInstance.defaults.httpsAgent = await this.getHttpsAgent(
      ServerAction.OICP_CREATE_AXIOS_INSTANCE
    );
  ...
  public async pushEvseStatus(
    evseStatuses: OICPEvseStatusRecord[],
    actionType: OICPActionType
  ): Promise<OICPAcknowledgment> {
    this.axiosInstance.defaults.httpsAgent = await this.getHttpsAgent(
      ServerAction.OICP_CREATE_AXIOS_INSTANCE
    ...
    if (!pushEvseStatusResponse?.Result || pushEvseStatusResponse?.Result !== true) {
      await Logging.logError({
        tenantID: this.tenant.id,
        action: ServerAction.OICP_PUSH_EVSE_STATUSES,
        message: this.buildOICPChargingNotificationErrorMessage(
          pushEvseStatusResponse,
          requestError
        ),
        module: MODULE_NAME,
        method: 'pushEvseStatus',
  ...
  public async authorizeStart(
    tagID: string,
    transactionId?: number
  ): Promise<OICPAuthorizeStartCpoReceive> {
    this.axiosInstance.defaults.httpsAgent = await this.getHttpsAgent(
      ServerAction.OICP_CREATE_AXIOS_INSTANCE
    );
  ...
  public async pushCdr(transaction: Transaction): Promise<OICPAcknowledgment> {
    ...
    if (!Utils.isEmptyArray(transaction.oicpData.session.meterValueInBetween)) {
      cdr.MeterValueInBetween = {
        meterValues: transaction.oicpData.session.meterValueInBetween.map((wattHour) =>
          Utils.convertWattHourToKiloWattHour(wattHour, 3)
        ),
      }; // Optional
    }
    cdr.ConsumedEnergy = Utils.convertWattHourToKiloWattHour(
  ...
  public async sendChargingNotificationStart(transaction: Transaction): Promise<void> {
    ...
    const fullUrl = this.getEndpointUrl(
      'notifications',
      ServerAction.OICP_SEND_CHARGING_NOTIFICATION_START
    );
    // Build payload
  ...
  public async sendChargingNotificationProgress(transaction: Transaction): Promise<void> {
    if (this.checkProgressUpdateInterval(transaction)) {
      ...
      payload.ChargingDuration = Utils.createDecimal(transaction.currentTimestamp.getTime())
        .minus(transaction.timestamp.getTime())
        .toNumber(); // Optional Duration in milliseconds (Integer). Charging Duration = EventOccurred - Charging Duration. Same as transaction.currentTotalDurationSecs * 1000?
      payload.SessionStart = transaction.oicpData.session.start_datetime; // Optional
      payload.ConsumedEnergyProgress = Utils.convertWattHourToKiloWattHour(
        transaction.currentTotalConsumptionWh,
        3
      ); // In kW.h Optional
      payload.MeterValueStart = Utils.convertWattHourToKiloWattHour(transaction.meterStart, 3); // Optional
      payload.OperatorID = this.getOperatorID(
        ServerAction.OICP_SEND_CHARGING_NOTIFICATION_PROGRESS
      ); // Optional
      payload.PartnerProductID; // Optional
      // Call Hubject
  ...
  public async sendChargingNotificationEnd(transaction: Transaction): Promise<OICPAcknowledgment> {
    ...
    if (!transaction.oicpData) {
      throw new BackendError({
        ...
        message: `OICP data does not exists on Session ID '${transaction.id}'`,
        module: MODULE_NAME,
        method: 'sendChargingNotificationEnd',
      });
    }
    if (!transaction.oicpData.session) {
      throw new BackendError({
    ...
    const fullUrl = this.getEndpointUrl(
      'notifications',
      ServerAction.OICP_SEND_CHARGING_NOTIFICATION_END
    );
    // Build payload
    ...
    payload.ConsumedEnergy = Utils.convertWattHourToKiloWattHour(
      transaction.stop.totalConsumptionWh,
      3
    );
  ...
  public async sendChargingNotificationError(
    transaction: Transaction,
    error: OICPErrorClass,
    ...
    if (!transaction.oicpData) {
      throw new BackendError({
        ...
        message: `OICP data does not exists on Session ID '${transaction.id}'`,
        module: MODULE_NAME,
        method: 'sendChargingNotificationError',
      });
    }
    if (!transaction.oicpData.session) {
      throw new BackendError({
    ...
    const fullUrl = this.getEndpointUrl(
      'notifications',
      ServerAction.OICP_SEND_CHARGING_NOTIFICATION_ERROR
    );
    // Build payload
  ...
  public async ping(): Promise<any> {
    ...
    try {
      ...
      if (!response.Result || !(response.StatusCode.Code === OICPStatusCode.Code000)) {
        ...
        pingResult.statusText = `Invalid response from POST ${this.getEndpointUrl(
          'evses',
          ServerAction.OICP_PUSH_EVSE_DATA
        )}`;
      } else {
  ...
  private getChargeBoxIDsInFailure(): string[] {
    if (
      this.oicpEndpoint.lastPatchJobResult &&
      this.oicpEndpoint.lastPatchJobResult.chargeBoxIDsInFailure
    ) {
  ...
  private checkProgressUpdateInterval(transaction: Transaction): boolean {
    ...
    if (transaction.oicpData.session.last_progress_notification) {
      ...
      const lastProgressUpdateTime =
        transaction.oicpData.session.last_progress_notification.getTime();
      lastProgressUpdate = (currentTime - lastProgressUpdateTime) / 1000; // Difference in seconds
    }
    if (
      lastProgressUpdate >= Constants.OICP_PROGRESS_NOTIFICATION_MAX_INTERVAL ||
  ...
  private async getChargeBoxIDsWithNewStatusNotifications(): Promise<string[]> {
    // Get last job
    const lastPatchJobOn = this.oicpEndpoint.lastPatchJobOn
    ...
    if (statusNotificationsResult.count > 0) {
      return statusNotificationsResult.result.map(
        (statusNotification) => statusNotification.chargeBoxID
      );
    }
    return [];
  }

  private buildOICPChargingNotificationErrorMessage(
    notificationProgressResponse: OICPAcknowledgment | OICPAuthorizeStopCpoReceive,
    requestError: Error
  ): string {
    let errorMessage = '';
    if (notificationProgressResponse) {
      errorMessage =
        notificationProgressResponse?.StatusCode?.AdditionalInfo?.length > 0
          ? notificationProgressResponse?.StatusCode?.AdditionalInfo
          : notificationProgressResponse?.StatusCode?.Description;
    }
    if (requestError) {
