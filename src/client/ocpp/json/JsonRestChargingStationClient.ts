...
export default class JsonRestChargingStationClient extends ChargingStationClient {
  ...
  private requests: {
    [messageUID: string]: {
      resolve?: (result: Record<string, unknown> | string) => void;
      reject?: (error: Error | Record<string, unknown>) => void;
      command: Command;
    };
  };
  ...
  private async openConnection(request: OCPPOutgoingRequest): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.wsConnection.onmessage = async (message) => {
          try {
            ...
            const [
              ...
              commandPayload,
              errorDetails,
            ]: OCPPIncomingRequest = JSON.parse(message.data) as OCPPIncomingRequest;
            ...
            await Logging.logException(
              error as Error,
              ServerAction.WS_CLIENT_MESSAGE,
              MODULE_NAME,
              'onMessage',
              this.tenantID
            );
        ...
        reject(
          new Error(
            `Unexpected error on opening Web Socket connection: ${error.message as string}'`
          )
        );
      }
    });
  ...
  private async sendMessage(request: OCPPOutgoingRequest): Promise<any> {
    ...
    if (
      Date.now() - this.chargingStation.lastSeen.getTime() >
      ...
      throw new Error(
        `Charging station is not connected to the server - request '${request[2]}' has been aborted`
      );
    }
