...
export default class SoapChargingStationClient extends ChargingStationClient {
  ...
  public async reset(params: OCPPResetCommandParam): Promise<OCPPResetCommandResult> {
    ...
    // Log
    Logging.logChargingStationClientSendAction(MODULE_NAME, this.tenantID, this.chargingStation.id, ServerAction.RESET,
      [params, { headers: this.client.getSoapHeaders() }]);
    // Execute
    ...
    // Log
    Logging.logChargingStationClientReceiveAction(MODULE_NAME, this.tenantID, this.chargingStation.id, ServerAction.RESET, [
      { result },
      { envelope }
    ]);
    return result;
