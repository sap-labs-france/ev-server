...
export default class CpoOCPIClient extends OCPIClient {
  ...
  public async pullTokens(partial = false): Promise<OCPIResult> {
    ...
    const emspUsers = (
      await UserStorage.getUsers(this.tenant, { issuer: false }, Constants.DB_PARAMS_MAX_LIMIT)
    ).result;
          ...
          async (token) => {
            try {
              ...
              const email = OCPIUtils.buildEmspEmailFromEmspToken(
                token,
                this.ocpiEndpoint.countryCode,
                this.ocpiEndpoint.partyId
              );
              ...
              await OCPIUtilsService.updateCreateTagWithEmspToken(
                ...
                emspTag,
                emspUser,
                ServerAction.OCPI_CPO_GET_TOKENS
              );
              result.success++;
            } catch (error) {
              result.failure++;
              ...
              result.logs.push(
                `Failed to update Issuer '${token.issuer}' - ID '${token.uid}': ${
    ...
    await Logging.logOcpiResult(
      ...
      `{{inSuccess}} token(s) were successfully pulled in ${executionDurationSecs}s ${
        partial ? 'from ' + momentFrom.format() : ''
      }`,
  ...
  public async authorizeToken(
    ...
    // Get tokens endpoint url
    const tokensUrl = `${this.getEndpointUrl('tokens', ServerAction.OCPI_CPO_AUTHORIZE_TOKEN)}/${
      token.uid
    }/authorize`;
    // Build payload
    ...
    // Call IOP
    const response = await this.axiosInstance.post(tokensUrl, locationReference, {
      headers: {
        Authorization: `Token ${this.ocpiEndpoint.token}`,
  ...
  public async startSession(
    ...
    // Get tokens endpoint url
    const sessionsUrl = `${this.getEndpointUrl(
      'sessions',
      ServerAction.OCPI_CPO_PUSH_SESSIONS
    )}/${this.getLocalCountryCode(ServerAction.OCPI_CPO_PUSH_SESSIONS)}/${this.getLocalPartyID(
      ServerAction.OCPI_CPO_PUSH_SESSIONS
    )}/${transaction.id.toString()}`;
    const site = await SiteStorage.getSite(this.tenant, chargingStation.siteID);
    const ocpiLocation: OCPILocation = this.convertChargingStationToOCPILocation(
      this.tenant,
      site,
      chargingStation,
    ...
    await Logging.logInfo({
      ...
      message: `${Utils.buildConnectorInfo(
        transaction.connectorId,
        transaction.id
      )} OCPI Transaction has been started successfully`,
      module: MODULE_NAME,
      method: 'startSession',
  ...
  public async updateSession(transaction: Transaction): Promise<void> {
    ...
    const sessionsUrl = `${this.getEndpointUrl(
      ...
      ServerAction.OCPI_CPO_PUSH_SESSIONS
    )}/${transaction.ocpiData.session.id}`;
    // Update transaction
    transaction.ocpiData.session.kwh = Utils.createDecimal(transaction.currentTotalConsumptionWh)
      .div(1000)
      .toNumber();
    ...
    transaction.ocpiData.session.charging_periods = await this.buildChargingPeriods(
      this.tenant,
      transaction
    );
    ...
    // Call IOP
    const response = await this.axiosInstance.patch(sessionsUrl, session, {
      headers: {
        Authorization: `Token ${this.ocpiEndpoint.token}`,
        'Content-Type': 'application/json',
      },
    ...
    await Logging.logInfo({
      ...
      message: `${Utils.buildConnectorInfo(
        transaction.connectorId,
        transaction.id
      )} OCPI Transaction has been patched successfully`,
      module: MODULE_NAME,
      method: 'updateSession',
  ...
  public async stopSession(transaction: Transaction): Promise<void> {
    if (!transaction.ocpiData?.session) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} OCPI Transaction does not exists`,
        module: MODULE_NAME,
        method: 'stopSession',
      });
    }
    if (!transaction.stop) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} Transaction has not yet been stopped`,
        module: MODULE_NAME,
        method: 'stopSession',
      });
    }
    // Get tokens endpoint url
    const tokensUrl = `${this.getEndpointUrl(
      'sessions',
      ServerAction.OCPI_CPO_PUSH_SESSIONS
    )}/${this.getLocalCountryCode(ServerAction.OCPI_CPO_PUSH_SESSIONS)}/${this.getLocalPartyID(
      ServerAction.OCPI_CPO_PUSH_SESSIONS
    )}/${transaction.ocpiData.session.id}`;
    transaction.ocpiData.session.kwh = Utils.createDecimal(transaction.stop.totalConsumptionWh)
      .div(1000)
      .toNumber();
    transaction.ocpiData.session.total_cost = 0; // Never calculate the cost of OCPI transaction
    ...
    await Logging.logInfo({
      ...
      message: `${Utils.buildConnectorInfo(
        transaction.connectorId,
        transaction.id
      )} OCPI Transaction has been stopped successfully`,
      module: MODULE_NAME,
      method: 'stopSession',
  ...
  public async pushCdr(transaction: Transaction): Promise<boolean> {
    ...
    if (transaction.ocpiData.cdr?.id) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} OCPI CDR has already been sent for this Transaction`,
        module: MODULE_NAME,
        method: 'postCdr',
      });
    }
    ...
    transaction.ocpiData.cdr = {
      ...
      total_parking_time: Utils.createDecimal(transaction.stop.totalInactivitySecs)
        .plus(transaction.stop.extraInactivitySecs)
        .div(3600)
        .toNumber(), // In hours
    ...
    if (
      !Utils.isEmptyArray(transaction.ocpiData.cdr.charging_periods) &&
      ...
      await Logging.logInfo({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} OCPI CDR has been sent successfully`,
        module: MODULE_NAME,
        method: 'postCdr',
        detailedMessages: {
    ...
    await Logging.logWarning({
      ...
      message: `${Utils.buildConnectorInfo(
        transaction.connectorId,
        transaction.id
      )} OCPI CDR has no consumption and will be be ignored`,
      module: MODULE_NAME,
      method: 'postCdr',
  ...
  public async patchChargingStationStatus(
    chargingStation: ChargingStation,
    status?: OCPIEvseStatus
  ): Promise<void> {
    const results: any[] = [];
    for (const connector of chargingStation.connectors) {
      const result = await this.patchChargingStationConnectorStatus(
        chargingStation,
        connector,
        status
      );
  ...
  public async patchChargingStationConnectorStatus(
    chargingStation: ChargingStation,
    connector: Connector,
    status?: OCPIEvseStatus
    ...
    if (!chargingStation.siteAreaID) {
      throw new BackendError({
        ...
        module: MODULE_NAME,
        method: 'patchChargingStationStatus',
      });
    }
        ...
        async (transaction) => {
          ...
          if (transaction.stop && transaction.stop.timestamp) {
            try {
              ...
              result.objectIDsInFailure.push(String(transaction.id));
              result.logs.push(
                `Failed to check OCPI Transaction ID '${transaction.ocpiData.session.id}': ${
                  error.message as string
                }`
              );
            }
        ...
        async (location) => {
          ...
          if (location) {
            try {
              ...
              result.objectIDsInFailure.push(String(location.id));
              result.logs.push(
                `Failed to check the Location '${location.name}' with ID '${location.id}': ${
                  error.message as string
                }`
              );
            }
  ...
  public async checkCdrs(): Promise<OCPIResult> {
      ...
      await Promise.map(
        ...
        async (transaction) => {
          ...
          try {
            ...
            result.objectIDsInFailure.push(String(transaction.id));
            result.logs.push(
              `${Utils.buildConnectorInfo(
                transaction.connectorId,
                transaction.id
              )} Failed to check CDR: ${error.message as string}`
            );
          }
        },
        { concurrency: Constants.OCPI_MAX_PARALLEL_REQUESTS }
    ...
    await Logging.logOcpiResult(
      this.tenant.id,
      ServerAction.OCPI_CPO_CHECK_CDRS,
      MODULE_NAME,
      'checkCdrs',
      result,
  ...
  public async pushChargingStationStatuses(partial = false): Promise<OCPIResult> {
    ...
    if (partial) {
      ...
      chargeBoxIDsToProcess.push(
        ...(await this.getChargeBoxIDsWithNewStatusNotifications(momentFrom.toDate()))
      );
      // Remove duplicates
        ...
        do {
          ...
          if (partial) {
            if (!Utils.isEmptyArray(chargeBoxIDsToProcess)) {
              evses = await OCPIUtilsService.getCpoEvsesFromSite(
                this.tenant,
                location.id,
                options,
              ...
              async (evse) => {
                ...
                if (location.id && evse?.uid) {
                  try {
                    ...
                    result.logs.push(
                      `Update status failed on Location '${location.name}' with ID '${
                        location.id
                      }', Charging Station ID '${evse.evse_id}': ${error.message as string}`
                    );
                  }
                  if (result.failure > 0) {
                    // Send notification to admins
                    NotificationHandler.sendOCPIPatchChargingStationsStatusesError(this.tenant, {
  ...
  private async checkAndCreateEMSPUsersFromTokens(
    tokens: OCPIToken[],
    emspUsers: Map<string, User>
  ) {
    if (!Utils.isEmptyArray(tokens)) {
  ...
  private async getChargeBoxIDsWithNewStatusNotifications(
  ...
  private async patchEVSEStatus(
    chargingStation: { id: string; siteID: string; siteAreaID: string; companyID: string },
    locationId: string,
    evseUID: string,
    newStatus: OCPIEvseStatus
  ): Promise<AxiosResponse<any>> {
    ...
    // Call IOP
    const response = await this.axiosInstance.patch(fullUrl, evseStatus, {
      headers: {
        Authorization: `Token ${this.ocpiEndpoint.token}`,
        'Content-Type': 'application/json',
      },
  ...
  private async checkCdr(transaction: Transaction): Promise<boolean> {
      ...
      if (cdr) {
        ...
        await Logging.logInfo({
          ...
          message: `${Utils.buildConnectorInfo(
            transaction.connectorId,
            transaction.id
          )} CDR has been checked successfully`,
          module: MODULE_NAME,
          method: 'checkCdr',
    ...
    await Logging.logError({
      ...
      message: `${Utils.buildConnectorInfo(
        transaction.connectorId,
        transaction.id
      )} Failed to check CDR '${transaction.ocpiData.session.id}' at ${cdrsUrl}/${
        transaction.ocpiData.cdr.id
      }`,
      module: MODULE_NAME,
      method: 'checkCdr',
  ...
  private async checkSession(transaction: Transaction): Promise<boolean> {
    ...
    const sessionsUrl = `${this.getEndpointUrl(
      'sessions',
      ServerAction.OCPI_CPO_CHECK_SESSIONS
    )}/${this.getLocalCountryCode(ServerAction.OCPI_CPO_CHECK_SESSIONS)}/${this.getLocalPartyID(
      ServerAction.OCPI_CPO_CHECK_SESSIONS
    )}/${transaction.ocpiData.session.id}`;
      ...
      if (session) {
        await Logging.logInfo({
          ...
          message: `${Utils.buildConnectorInfo(
            transaction.connectorId,
            transaction.id
          )} Transaction has been checked successfully`,
          module: MODULE_NAME,
          method: 'checkSession',
    ...
    await Logging.logError({
      ...
      message: `${Utils.buildConnectorInfo(
        transaction.connectorId,
        transaction.id
      )} Failed to check Transaction at ${sessionsUrl}`,
      module: MODULE_NAME,
      method: 'checkSession',
      detailedMessages: { response: response.data, transaction },
    });
  ...
  private async buildChargingPeriods(
    tenant: Tenant,
    transaction: Transaction
  ): Promise<OCPIChargingPeriod[]> {
    if (!transaction || !transaction.timestamp) {
      return [];
    }
    const chargingPeriods: OCPIChargingPeriod[] = [];
    const consumptions = await ConsumptionStorage.getOptimizedTransactionConsumptions(tenant, {
    ...
    if (consumptions) {
      ...
      const consumption: number = transaction.stop
      ...
      const inactivity: number = transaction.stop
        ? transaction.stop.totalInactivitySecs
        : transaction.currentTotalInactivitySecs;
      if (inactivity > 0) {
        const inactivityStart = transaction.stop
  ...
  private convertChargingStationToOCPILocation(
    tenant: Tenant,
    site: Site,
    chargingStation: ChargingStation,
    connectorID: number,
    ...
    const ocpiLocation: OCPILocation = {
      ...
      country: countries.getAlpha3Code(
        ...
        CountriesList.countries[countryID].languages[0]
      ),
      coordinates: {
  ...
  private buildChargingPeriod(consumption: Consumption): OCPIChargingPeriod {
    ...
    if (consumption.consumptionWh > 0) {
      chargingPeriod.dimensions.push({
        type: CdrDimensionType.ENERGY,
        volume: Utils.truncTo(
          Utils.createDecimal(consumption.consumptionWh).div(1000).toNumber(),
          3
        ),
      });
    } else {
