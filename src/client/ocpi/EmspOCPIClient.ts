...
export default class EmspOCPIClient extends OCPIClient {
  ...
  public async pushTokens(partial = false): Promise<OCPIResult> {
    ...
    do {
      ...
      tokens = await OCPIUtilsService.getEmspTokensFromTags(
        this.tenant,
        Constants.DB_RECORD_COUNT_DEFAULT,
        currentSkip,
    ...
    await Logging.logOcpiResult(
      this.tenant.id,
      ServerAction.OCPI_EMSP_UPDATE_TOKENS,
      MODULE_NAME,
      'sendTokens',
      result,
      ...
      if (!Utils.isEmptyArray(locations)) {
        ...
        for (const location of locations) {
          try {
            ...
            // Get Site Area
            const foundSiteArea = await SiteAreaStorage.getSiteAreaByOcpiLocationUid(
              this.tenant,
              location.id
            );
            // Process Site Area
            const siteArea = await OCPIUtils.updateCreateSiteAreaWithEmspLocation(
              this.tenant,
              location,
              site,
              foundSiteArea
            ...
            result.logs.push(
              `Failed to update Location '${location.name}' with ID '${location.id}': ${
  ...
  public async pullSessions(partial = false): Promise<OCPIResult> {
    ...
    do {
      ...
      if (!Utils.isEmptyArray(sessions)) {
        await Promise.map(
          sessions,
          async (session: OCPISession) => {
            try {
              await OCPIUtilsService.processEmspTransactionFromSession(
                this.tenant,
                session,
                ServerAction.OCPI_EMSP_GET_SESSIONS
              );
              result.success++;
            } catch (error) {
              result.failure++;
    ...
    do {
      ...
      if (!Utils.isEmptyArray(cdrs)) {
        await Promise.map(
          ...
          async (cdr: OCPICdr) => {
            try {
              await OCPIUtilsService.processEmspCdr(
                this.tenant,
                cdr,
                ServerAction.OCPI_EMSP_GET_CDRS
              );
              result.success++;
            } catch (error) {
              result.failure++;
              result.logs.push(`Failed to update CDR ID '${cdr.id}': ${error.message as string}`);
            }
          },
          { concurrency: Constants.OCPI_MAX_PARALLEL_REQUESTS }
        );
  ...
  public async remoteStartSession(
    ...
    const commandUrl =
      this.getEndpointUrl('commands', ServerAction.OCPI_EMSP_START_SESSION) +
      '/' +
    ...
    if (!tag || !tag.issuer || !tag.active) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          connectorID
        )} OCPI Remote Start Session is not available for Tag ID '${tagID}'`,
        module: MODULE_NAME,
    ...
    if (!tag.user || !tag.user.issuer) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          connectorID
        )} OCPI Remote Start Session is not available for user with Tag ID '${tagID}'`,
        module: MODULE_NAME,
        method: 'remoteStartSession',
    ...
    // Call IOP
    const response = await this.axiosInstance.post(commandUrl, remoteStart, {
      headers: {
        Authorization: `Token ${this.ocpiEndpoint.token}`,
        'Content-Type': 'application/json',
      },
    ...
    await Logging.logDebug({
      ...
      message: `${Utils.buildConnectorInfo(
        connectorID
      )} OCPI Remote Start session response status '${response.status}'`,
      module: MODULE_NAME,
      method: 'remoteStartSession',
  ...
  public async remoteStopSession(transactionID: number): Promise<OCPICommandResponse> {
    ...
    if (transaction.issuer) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          ...
          transaction.id
        )} Transaction belongs to a local organization`,
        module: MODULE_NAME,
        method: 'remoteStopSession',
        detailedMessages: {
    ...
    await Logging.logDebug({
      ...
      message: `${Utils.buildConnectorInfo(
        transaction.connectorId,
        transaction.id
      )} OCPI Remote Stop response status '${response.status}'`,
      module: MODULE_NAME,
      method: 'remoteStopSession',
