...
export default class LockingManager {
  public static createExclusiveLock(
    tenantID: string,
    entity: LockEntity,
    key: string,
    lockValiditySecs = 600
  ): Lock {
    ...
    try {
      ...
      Utils.isDevelopmentEnv() &&
        Logging.logConsoleInfo(
          `Try to acquire the lock entity '${lock.entity}' ('${lock.key}') of type '${lock.type}' in Tenant ID '${lock.tenantID}'`
        );
      switch (lock.type) {
        ...
        default:
          throw new BackendError({
            ...
            module: MODULE_NAME,
            method: 'acquire',
            message: `Cannot acquire a lock entity '${lock.entity}' ('${
              lock.key
            }') with an unknown type '${lock.type as string}'`,
            detailedMessages: { lock, timeoutSecs, retry },
          });
      }
      await Logging.logDebug({
      ...
      Utils.isDevelopmentEnv() &&
        Logging.logConsoleInfo(
          `Acquired successfully the lock entity '${lock.entity}' ('${lock.key}') of type '${lock.type}' in Tenant ID '${lock.tenantID}'`
        );
      return true;
    } catch (error) {
      ...
      Utils.isDevelopmentEnv() &&
        Logging.logConsoleError(
          `Cannot acquire the lock entity '${lock.entity}' ('${lock.key}') of type '${lock.type}' in Tenant ID '${lock.tenantID}'`
        );
      return false;
    }
  }
  ...
  public static async release(lock: Lock): Promise<boolean> {
    ...
    await Logging.logDebug({
      ...
      method: 'release',
      action: ServerAction.LOCKING,
      message: `Released successfully the lock entity '${lock.entity}' ('${lock.key}') of type '${
        lock.type
      }' after ${Math.round(Date.now() - lock.timestamp.getTime()) / 1000} secs`,
      detailedMessages: { lock },
    });
    Utils.isDevelopmentEnv() &&
      Logging.logConsoleDebug(
        `Released the lock entity '${lock.entity}' ('${lock.key}') of type '${
          lock.type
        }' in Tenant ID '${lock.tenantID}' after ${
          Math.round(Date.now() - lock.timestamp.getTime()) / 1000
        } secs`
      );
    return true;
  }

  private static createLock(
    tenantID: string,
    entity: LockEntity,
    key: string,
    type: LockType = LockType.EXCLUSIVE,
    lockValiditySecs?: number
  ): Lock {
    if (!tenantID) {
      throw new BackendError({
        action: ServerAction.LOCKING,
        ...
        try {
          ...
          Utils.isDevelopmentEnv() &&
            Logging.logConsoleWarning(
              `>> Lock failed, Wait for ${Constants.LOCK_WAIT_MILLIS}ms, lock '${lock.tenantID}~${lock.entity}~${lock.key}`
            );
  ...
  private static async checkAndReleaseExpiredLock(lock: Lock): Promise<boolean> {
    ...
    if (lockInDB?.expirationDate && lockInDB.expirationDate.getTime() < new Date().getTime()) {
      try {
        ...
        Utils.isDevelopmentEnv() &&
          Logging.logConsoleWarning(
            `The lock '${lock.entity}' ('${lock.key}') of type '${lock.type}' in Tenant ID ${lock.tenantID} has expired and was released successfully`
          );
        return true;
      } catch (error) {
        ...
        Utils.isDevelopmentEnv() &&
          Logging.logConsoleError(
            `The lock '${lock.entity}' ('${lock.key}') of type '${lock.type}' in Tenant ID ${lock.tenantID} has expired and cannot be released`
          );
      }
    }
    return false;
