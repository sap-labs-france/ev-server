...
export default class PerformanceStorage {
  public static async savePerformanceRecord(
    ...
    if (PERFS_ENABLED) {
      ...
      if (
        !performanceRecord.tenantSubdomain ||
        performanceRecord.tenantSubdomain === Constants.DEFAULT_TENANT_ID
      ) {
        delete performanceRecord.tenantSubdomain;
      }
      // Validate
      performanceRecord =
        PerformanceValidatorStorage.getInstance().validatePerformance(performanceRecord);
      // Insert
      const result = await global.database
        .getCollection<any>(Constants.DEFAULT_TENANT_ID, 'performances')
  ...
  public static async updatePerformanceRecord(
    performanceRecord: PerformanceRecord,
    labelValues: LabelValues<string>
  ): Promise<void> {
    if (PERFS_ENABLED) {
      // Validate
      const performanceRecordMDB =
        PerformanceValidatorStorage.getInstance().validatePerformance(performanceRecord);
      // Convert to ObjectID
      DatabaseUtils.switchIDToMongoDBID(performanceRecordMDB);
      // Update
      const ret = await global.database
        .getCollection<PerformanceRecord>(Constants.DEFAULT_TENANT_ID, 'performances')
        .findOneAndUpdate(
  ...
  public static async deletePerformanceRecords(params?: {
    deleteUpToDate: Date;
  ...
  private static savePrometheusMetric(
    performanceRecord: PerformanceRecord,
    labelValues: LabelValues<string>
  ) {
    ...
    try {
      if (performanceRecord.durationMs) {
        if (
          performanceRecord.group === PerformanceRecordGroup.MONGO_DB ||
          performanceRecord.group === PerformanceRecordGroup.NOTIFICATION
        ) {
          const durationMetric = global.monitoringServer.getCountAvgClearableMetric(
            grafanaGroup,
            'DurationMs',
            hashCode,
          ...
          const durationMetric = global.monitoringServer.getAvgClearableMetric(
            grafanaGroup,
            'DurationMs',
            hashCode,
            'duration in milliseconds',
      ...
      throw new Error(
        `Failed to save performance metrics - group: '${grafanaGroup}' - hashCode: '${hashCode}', labels: '${JSON.stringify(
          labels
