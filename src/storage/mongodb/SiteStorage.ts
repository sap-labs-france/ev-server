...
export default class SiteStorage {
  ...
  public static async getSites(tenantID: string,
    ...
    if (params.companyID) {
      ...
      const companyIDs = params.companyID.split('|');
      if(companyIDs.length > 1) {
        filters.companyID = { $in: companyIDs.map((companyID) => {
    ...
    if (sitesMDB && sitesMDB.length > 0) {
      ...
      for (const siteMDB of sitesMDB) {
        ...
        if (params.withAvailableChargers) {
          ...
          // Get te chargers
          const chargingStations = await ChargingStationStorage.getChargingStations(tenantID, { siteID: siteMDB.id }, Constants.MAX_DB_RECORD_COUNT, 0);
