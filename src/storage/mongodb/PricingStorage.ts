...
export default class PricingStorage {
  public static async savePricingDefinition(
    tenant: Tenant,
    ...
    const pricingDefinitionMDB = {
      _id: pricingDefinition.id
        ? DatabaseUtils.convertToObjectID(pricingDefinition.id)
        : new ObjectId(),
      entityID,
    ...
    await global.database
      .getCollection<any>(tenant.id, 'pricingdefinitions')
      .findOneAndUpdate(
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'savePricingDefinition',
      uniqueTimerID,
      pricingDefinitionMDB
    );
    return pricingDefinitionMDB._id.toString();
  }

  public static async deletePricingDefinition(
    tenant: Tenant,
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'deletePricingDefinition',
      uniqueTimerID,
      { id: pricingDefinitionID }
    );
  }

  public static async getPricingDefinition(
    tenant: Tenant,
    id: string,
    params: {
      entityID?: string;
    ...
    const pricingDefinitionMDB = await PricingStorage.getPricingDefinitions(
      tenant,
      {
  ...
  public static async getPricingDefinitions(
    tenant: Tenant,
    params: {
    ...
    dbParams: DbParams,
    projectFields?: string[]
  ): Promise<DataResult<PricingDefinition>> {
    const uniqueTimerID = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (!Utils.isEmptyArray(params.pricingDefinitionIDs)) {
      filters._id = {
        $in: params.pricingDefinitionIDs.map((pricingDefinitionID) =>
          DatabaseUtils.convertToObjectID(pricingDefinitionID)
        ),
      };
    }
    // Context IDs
    ...
    // Count Records
    const pricingDefinitionsCountMDB = await global.database
      .getCollection<any>(tenant.id, 'pricingdefinitions')
    ...
    // Read DB
    const pricingDefinitions = (await global.database
      .getCollection<any>(tenant.id, 'pricingdefinitions')
