...
class LoggingStorage {
    static async deleteLogs(tenant, deleteUpToDate){
        // Build filter
        const filters = {};
        ...
        // Delete Logs
        const result = await global.database.getCollection(tenant, 'logs')
        .deleteMany(filters);
        // Return the result
        return result.result;
    }

    static async deleteSecurityLogs(tenant, deleteUpToDate){
        // Build filter
        const filters = {};
    ...
    static async saveLog(tenant, logToSave){
        ...
        // Insert
        await global.database.getCollection(tenant, 'logs').insertOne(log);
    }

    static async getLog(tenant, id){
        // Read DB
        const loggingMDB = await global.database.getCollection(tenant, 'logs')
    ...
    static async getLogs(tenant, params = {}, limit, skip, sort){
        // Check Limit
        ...
        // Count Records
        const loggingsCountMDB = await global.database.getCollection(tenant, 'logs')
        ...
        // Read DB
        const loggingsMDB = await global.database.getCollection(tenant, 'logs')
