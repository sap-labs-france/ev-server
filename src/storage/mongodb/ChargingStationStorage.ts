...
export default class ChargingStationStorage {
  public static async getChargingStationTemplates(
    ...
    // Query Templates
    const chargingStationTemplatesMDB = (await global.database
  ...
  public static async deleteChargingStationTemplates(): Promise<void> {
    ...
    await global.database
      .getCollection<any>(Constants.DEFAULT_TENANT_ID, 'chargingstationtemplates')
      .deleteMany({ qa: { $not: { $eq: true } } });
    await Logging.traceDatabaseRequestEnd(
      Constants.DEFAULT_TENANT_OBJECT,
      MODULE_NAME,
      'deleteChargingStationTemplates',
      startTime,
      { qa: { $not: { $eq: true } } }
    );
  }

  public static async getChargingStation(
    tenant: Tenant,
    id: string = Constants.UNKNOWN_STRING_ID,
    params: {
    ...
    const chargingStationsMDB = await ChargingStationStorage.getChargingStations(
      tenant,
      {
  ...
  public static async getChargingProfile(
    tenant: Tenant,
    id: string = Constants.UNKNOWN_STRING_ID,
    params: { siteIDs?: string[]; withSiteArea?: boolean } = {},
    projectFields?: string[]
  ): Promise<ChargingProfile> {
    const chargingProfilesMDB = await ChargingStationStorage.getChargingProfiles(
      tenant,
      {
  ...
  public static async getChargingStationByOcpiLocationEvseUid(
    tenant: Tenant,
    ocpiLocationID: string = Constants.UNKNOWN_STRING_ID,
  ...
  public static async getChargingStationByOicpEvseID(
    tenant: Tenant,
    oicpEvseID: string = Constants.UNKNOWN_STRING_ID,
    projectFields?: string[]
  ): Promise<ChargingStation> {
    const chargingStationsMDB = await ChargingStationStorage.getChargingStations(
      tenant,
  ...
  public static async getChargingStations(
    tenant: Tenant,
    params: {
      search?: string;
      chargingStationIDs?: string[];
      chargingStationSerialNumbers?: string[];
      siteAreaIDs?: string[];
    ...
    dbParams: DbParams,
    projectFields?: string[]
  ): Promise<ChargingStationDataResult> {
    const startTime = Logging.traceDatabaseRequestStart();
    ...
    if (!Utils.isEmptyArray(params.companyIDs)) {
      // Query by companyID
      filters.companyID = {
        $in: params.companyIDs.map((id) => DatabaseUtils.convertToObjectID(id)),
      };
    }
    ...
    // Count Records
    const chargingStationsCountMDB = (await global.database
    ...
    if (params.withUser) {
      DatabaseUtils.pushArrayLookupInAggregation(
        'connectors',
        DatabaseUtils.pushUserLookupInAggregation.bind(this),
        {
          tenantID: tenant.id,
          aggregation: aggregation,
          localField: 'connectors.currentUserID',
          foreignField: '_id',
          asField: 'connectors.user',
    ...
    // Read DB
    const chargingStationsMDB = (await global.database
      .getCollection<any>(tenant.id, 'chargingstations')
  ...
  public static async getChargingStationsInError(
    tenant: Tenant,
    params: { search?: string; siteIDs?: string[]; siteAreaIDs: string[]; errorType?: string[] },
    dbParams: DbParams,
    projectFields?: string[]
  ): Promise<ChargingStationInErrorDataResult> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
  ...
  public static async saveChargingStation(
    tenant: Tenant,
    chargingStationToSave: ChargingStation
  ): Promise<string> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Remove old field
    ...
    const chargingStationMDB = {
      ...
      excludeFromSmartCharging: Utils.convertToBoolean(
        chargingStationToSave.excludeFromSmartCharging
      ),
      ...
      connectors: chargingStationToSave.connectors
        ? chargingStationToSave.connectors.map((connector) =>
            ChargingStationStorage.filterConnectorMDB(connector)
          )
        : [],
      backupConnectors: chargingStationToSave.backupConnectors
        ? chargingStationToSave.backupConnectors.map((backupConnector) =>
            ChargingStationStorage.filterConnectorMDB(backupConnector)
          )
        : [],
      chargePoints: chargingStationToSave.chargePoints
        ? chargingStationToSave.chargePoints.map((chargePoint) =>
            ChargingStationStorage.filterChargePointMDB(chargePoint)
          )
        : [],
      coordinates: Utils.hasValidGpsCoordinates(chargingStationToSave.coordinates)
        ? chargingStationToSave.coordinates.map((coordinate) => Utils.convertToFloat(coordinate))
        : [],
  ...
  public static async saveChargingStationConnectors(
    tenant: Tenant,
    id: string,
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveChargingStationConnectors',
      startTime,
      connectors
    );
  }

  public static async saveChargingStationOicpData(
    tenant: Tenant,
    id: string,
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveChargingStationOicpData',
      startTime,
      oicpData
    );
  }

  public static async saveChargingStationRuntimeData(
    tenant: Tenant,
    id: string,
    runtimeData: {
      lastSeen?: Date;
    ...
    await global.database
      .getCollection<any>(tenant.id, 'chargingstations')
      .findOneAndUpdate({ _id: id }, { $set: runtimeDataMDB }, { upsert: true });
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveChargingStationRuntimeData',
      startTime,
      runtimeData
    );
  }

  public static async saveChargingStationOcpiData(
    tenant: Tenant,
    id: string,
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveChargingStationOcpiData',
      startTime,
      ocpiData
    );
  }

  public static async saveChargingStationRemoteAuthorizations(
    tenant: Tenant,
    id: string,
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveChargingStationRemoteAuthorizations',
      startTime,
      remoteAuthorizations
    );
  }

  public static async saveChargingStationFirmwareStatus(
    tenant: Tenant,
    id: string,
    ...
    await global.database
      .getCollection<any>(tenant.id, 'chargingstations')
      .findOneAndUpdate({ _id: id }, { $set: { firmwareUpdateStatus } }, { upsert: true });
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveChargingStationFirmwareStatus',
      startTime,
  ...
  public static async deleteChargingStation(tenant: Tenant, id: string): Promise<void> {
    ...
    await global.database
      .getCollection<any>(tenant.id, 'configurations')
    ...
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'deleteChargingStation', startTime, {
      id,
    });
  }

  public static async getOcppParameterValue(
    tenant: Tenant,
    chargeBoxID: string,
    paramName: string
  ): Promise<string> {
  ...
  public static async saveOcppParameters(
    tenant: Tenant,
    parameters: ChargingStationOcppParameters
  ): Promise<void> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Modify
  ...
  public static async getOcppParameters(
    tenant: Tenant,
    id: string
  ): Promise<DataResult<OcppParameter>> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Read DB
    const parametersMDB = (await global.database
      .getCollection<any>(tenant.id, 'configurations')
  ...
  public static async getChargingProfiles(
    tenant: Tenant,
    params: {
      search?: string;
      chargingStationIDs?: string[];
      ...
      profilePurposeType?: ChargingProfilePurposeType;
    ...
    dbParams: DbParams,
    projectFields?: string[]
  ): Promise<ChargingProfileDataResult> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Clone before updating the values
    dbParams = Utils.cloneObject(dbParams);
    ...
    // Count Records
    const chargingProfilesCountMDB = (await global.database
      .getCollection<any>(tenant.id, 'chargingprofiles')
    ...
    DatabaseUtils.projectFields(aggregation, projectFields);
    // Read DB
    const chargingProfilesMDB = (await global.database
      .getCollection<any>(tenant.id, 'chargingprofiles')
      .aggregate<any>(aggregation, DatabaseUtils.buildAggregateOptions())
  ...
  public static async saveChargingProfile(
    tenant: Tenant,
    chargingProfileToSave: ChargingProfile
  ): Promise<string> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (chargingProfileToSave.id) {
      ...
      chargingProfileFilter._id = Utils.hash(
        `${chargingProfileToSave.chargingStationID}~${chargingProfileToSave.connectorID}~${chargingProfileToSave.profile.chargingProfileId}`
      );
    }
    // Properties to save
    ...
    await global.database
      .getCollection<any>(tenant.id, 'chargingprofiles')
      .findOneAndUpdate(chargingProfileFilter, { $set: chargingProfileMDB }, { upsert: true });
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveChargingProfile',
  ...
  public static async deleteChargingProfile(tenant: Tenant, id: string): Promise<void> {
    ...
    await global.database
      .getCollection<any>(tenant.id, 'chargingprofiles')
      .findOneAndDelete({ _id: id });
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'deleteChargingProfile', startTime, {
      id,
    });
  }

  public static async deleteChargingProfiles(
    tenant: Tenant,
  ...
  public static async updateChargingStationsWithOrganizationIDs(
    tenant: Tenant,
    companyID: string,
    siteID: string,
    siteAreaID?: string
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'updateChargingStationsWithOrganizationIDs',
      startTime,
    ...
    switch (errorType) {
        ...
        return [
          {
            $match: {
              $or: [
                { maximumPower: { $exists: false } },
                { maximumPower: { $lte: 0 } },
                { maximumPower: null },
                { chargePointModel: { $exists: false } },
                { chargePointModel: { $eq: '' } },
                { chargePointVendor: { $exists: false } },
                { chargePointVendor: { $eq: '' } },
                { powerLimitUnit: { $exists: false } },
                { powerLimitUnit: null },
                {
                  powerLimitUnit: {
                    $nin: [ChargingRateUnitType.AMPERE, ChargingRateUnitType.WATT],
                  },
                },
                { chargingStationURL: { $exists: false } },
                { chargingStationURL: null },
                { chargingStationURL: { $eq: '' } },
                { 'connectors.type': { $exists: false } },
                ...
                { 'connectors.type': { $eq: '' } },
                {
                  'connectors.type': {
                    $nin: [
                      ...
                      ConnectorType.TYPE_1_CCS,
                      ConnectorType.TYPE_2,
                      ConnectorType.TYPE_3C,
                    ],
                  },
                },
              ],
      ...
      case ChargingStationInErrorType.CONNECTION_BROKEN: {
        const inactiveDate = new Date(
          new Date().getTime() -
            Configuration.getChargingStationConfig().maxLastSeenIntervalSecs * 1000
        );
        return [
        ...
        return [
          {
            $match: {
              $or: [
                { 'connectors.errorCode': { $ne: 'NoError' } },
                { 'connectors.status': { $eq: ChargePointStatus.FAULTED } },
              ],
            },
          },
    ...
    if (chargePoint) {
      return {
        ...
        numberOfConnectedPhase: chargePoint.numberOfConnectedPhase
          ? Utils.convertToInt(chargePoint.numberOfConnectedPhase)
          : null,
        ...
        connectorIDs: chargePoint.connectorIDs.map((connectorID) =>
