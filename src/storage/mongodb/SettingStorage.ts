...
export default class SettingStorage {
  public static async getSetting(
    ...
    const settingMDB = await SettingStorage.getSettings(
      tenant,
      { settingID: id },
  ...
  public static async getSettingByIdentifier(
    tenant: Tenant,
    identifier: string = Constants.UNKNOWN_STRING_ID,
    projectFields?: string[]
  ): Promise<SettingDB> {
  ...
  public static async saveSettings(
    tenant: Tenant,
    settingToSave: Partial<SettingDB>
  ): Promise<string> {
    ...
    await global.database
      .getCollection<any>(tenant.id, 'settings')
      .findOneAndUpdate(
        settingFilter,
        { $set: settingMDB },
  ...
  public static async saveCarConnectorSettings(
    tenant: Tenant,
    carConnectorSettingToSave: CarConnectorSettings
  ): Promise<void> {
  ...
  public static async getPricingSettings(
    tenant: Tenant,
    limit?: number,
    skip?: number,
    dateFrom?: Date,
    ...
    if (settings && settings.count > 0 && settings.result[0].content) {
      ...
      if (config.simple) {
        ...
        pricingSettings.simple = {
          ...
          last_updated: settings.result[0].lastChangedOn
            ? Utils.convertToDate(settings.result[0].lastChangedOn)
            : null,
        };
      }
    }
      ...
      if (config.sapSmartCharging) {
        ...
        smartChargingSettings.sapSmartCharging = {
          optimizerUrl: config.sapSmartCharging.optimizerUrl
            ? config.sapSmartCharging.optimizerUrl
            : '',
          ...
          stickyLimitation: config.sapSmartCharging.stickyLimitation
            ? config.sapSmartCharging.stickyLimitation
            : false,
          limitBufferDC: config.sapSmartCharging.limitBufferDC
            ? config.sapSmartCharging.limitBufferDC
            : 0,
          limitBufferAC: config.sapSmartCharging.limitBufferAC
            ? config.sapSmartCharging.limitBufferAC
            : 0,
          prioritizationParametersActive:
            config.sapSmartCharging.prioritizationParametersActive ?? false,
        };
      }
    }
  ...
  public static async getSettings(
    ...
    // Count Records
    const settingsCountMDB = (await global.database
      .getCollection<any>(tenant.id, 'settings')
    ...
    // Read DB
    const settingsMDB = (await global.database
      .getCollection<any>(tenant.id, 'settings')
  ...
  public static async saveUserSettings(
    tenant: Tenant,
    userSettingToSave: UserSettings
  ): Promise<void> {
    // Build internal structure
    const settingsToSave = {
  ...
  public static async saveCryptoSettings(
    tenant: Tenant,
    cryptoSettingsToSave: CryptoSettings
  ): Promise<void> {
    // Build internal structure
    const settingsToSave = {
  ...
  public static async getBillingSetting(
    tenant: Tenant,
    projectedFields?: string[]
  ): Promise<BillingSettings> {
    // Get BILLING Settings by Identifier
    const setting = await SettingStorage.getSettingByIdentifier(
      tenant,
  ...
  public static async saveBillingSetting(
    tenant: Tenant,
