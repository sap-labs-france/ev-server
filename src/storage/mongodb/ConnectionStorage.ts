...
export default class ConnectionStorage {
  public static async saveConnection(
    ...
    const connectionMDB: any = {
      _id: !connectionToSave.id
        ? new ObjectId()
        : DatabaseUtils.convertToObjectID(connectionToSave.id),
  ...
  public static async getConnectionByConnectorIdAndUserId(
    tenant: Tenant,
    connectorId: string,
    userId: string,
    ...
    // Exec
    const connections = (await global.database
      .getCollection<any>(tenant.id, 'connections')
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'getConnectionByConnectorIdAndUserId',
      startTime,
      aggregation,
      connections
    );
  ...
  public static async getConnectionsByUserId(
    tenant: Tenant,
    userID: string,
    projectFields?: string[]
  ): Promise<DataResult<Connection>> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    const aggregation = [];
    aggregation.push({
    ...
    DatabaseUtils.projectFields(aggregation, projectFields);
    // Get connections
    const connectionsMDB = (await global.database
      .getCollection<any>(tenant.id, 'connections')
  ...
  public static async deleteConnectionById(tenant: Tenant, id: string): Promise<void> {
    ...
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'deleteConnectionById', startTime, {
      id,
    });
  }
