...
export default class SiteAreaStorage {
  public static async updateEntitiesWithOrganizationIDs(
    tenant: Tenant,
    companyID: string,
    siteID: string,
    siteAreaID: string
    ...
    let updated = await ChargingStationStorage.updateChargingStationsWithOrganizationIDs(
      tenant,
      companyID,
      siteID,
      siteAreaID
    ...
    // Update Transactions
    updated += await TransactionStorage.updateTransactionsWithOrganizationIDs(
      tenant,
      companyID,
      siteID,
      siteAreaID
    ...
    // Update Assets
    updated += await AssetStorage.updateAssetsWithOrganizationIDs(
      tenant,
      companyID,
      siteID,
      siteAreaID
    ...
    // Update Consumptions
    updated += await ConsumptionStorage.updateConsumptionsWithOrganizationIDs(
      tenant,
      siteID,
      siteAreaID
    );
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'updateEntitiesWithOrganizationIDs',
      startTime,
      { companyID, siteID, siteAreaID }
    );
    return updated;
  }

  public static async attachSiteAreaChildrenToNewParent(
    tenant: Tenant,
    oldParentSiteAreaID: string,
    newParentSiteAreaID: string
  ): Promise<number> {
    const startTime = Logging.traceDatabaseRequestStart();
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'attachSiteAreaChildrenToNewParent',
      startTime,
      { oldParentSiteAreaID, newParentSiteAreaID }
    );
    return result.modifiedCount;
  }

  public static async updateSmartCharging(
    tenant: Tenant,
    siteAreaID: string,
    ...
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'updateSmartCharging', startTime, {
      siteAreaID,
      smartCharging,
    });
  }

  public static async updateSiteID(
    tenant: Tenant,
    siteAreaID: string,
    siteID: string
  ): Promise<void> {
    ...
    await Logging.traceDatabaseRequestEnd(tenant, MODULE_NAME, 'updateSiteID', startTime, {
      siteAreaID,
      siteID,
    });
  }

  public static async addAssetsToSiteArea(
    tenant: Tenant,
    siteAreaID: string,
    siteID: string,
    companyID: string,
  ...
  public static async removeAssetsFromSiteArea(
    tenant: Tenant,
    siteAreaID: string,
    assetIDs: string[]
  ): Promise<void> {
  ...
  public static async getSiteAreaImage(tenant: Tenant, id: string): Promise<Image> {
    ...
    DatabaseUtils.checkTenantObject(tenant);
    // Read DB
    const siteAreaImageMDB = await global.database
      .getCollection<{ _id: ObjectId; image: string }>(tenant.id, 'siteareaimages')
    ...
    return {
      id: id,
      image: siteAreaImageMDB ? siteAreaImageMDB.image : null,
    };
  }

  public static async getSiteAreaByOcpiLocationUid(
    tenant: Tenant,
    ocpiLocationID: string = Constants.UNKNOWN_STRING_ID,
    projectFields?: string[]
  ): Promise<SiteArea> {
    const siteAreaMDB = await SiteAreaStorage.getSiteAreas(
      tenant,
      {
        ocpiLocationID,
        withSite: true,
    ...
    if (siteAreaMDB.count > 1) {
      await Logging.logWarning({
        ...
        module: MODULE_NAME,
        method: 'getSiteAreaByOcpiLocationUid',
  ...
  public static async getSiteArea(
    tenant: Tenant,
  ...
  public static async saveSiteArea(
    tenant: Tenant,
    siteAreaToSave: SiteArea,
    saveImage = false
  ): Promise<string> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (siteAreaToSave.address) {
      siteAreaMDB.address = {
        ...
        coordinates: Utils.hasValidGpsCoordinates(siteAreaToSave.address.coordinates)
          ? siteAreaToSave.address.coordinates.map((coordinate) => Utils.convertToFloat(coordinate))
          : [],
      };
    }
    if (siteAreaToSave.smartChargingSessionParameters) {
      siteAreaMDB.smartChargingSessionParameters = {
        ...
        carStateOfCharge: siteAreaToSave.smartChargingSessionParameters.carStateOfCharge
          ? Utils.convertToInt(siteAreaToSave.smartChargingSessionParameters.carStateOfCharge)
          : null,
        targetStateOfCharge: siteAreaToSave.smartChargingSessionParameters.targetStateOfCharge
          ? Utils.convertToInt(siteAreaToSave.smartChargingSessionParameters.targetStateOfCharge)
          : null,
      };
    }
    ...
    await global.database
      .getCollection<any>(tenant.id, 'siteareas')
      .findOneAndUpdate(
        { _id: siteAreaMDB._id },
        { $set: siteAreaMDB },
    ...
    if (saveImage) {
      await SiteAreaStorage.saveSiteAreaImage(
        tenant,
        siteAreaMDB._id.toString(),
        siteAreaToSave.image
      );
    }
    await Logging.traceDatabaseRequestEnd(
      tenant,
  ...
  public static async saveSiteAreaOcpiData(
    tenant: Tenant,
    id: string,
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'saveSiteAreaOcpiData',
      startTime,
      ocpiData
    );
  }

  public static async getSiteAreas(
    tenant: Tenant,
    params: {
      siteAreaIDs?: string[];
      search?: string;
      siteIDs?: string[];
      parentSiteAreaIDs?: string[];
      companyIDs?: string[];
      withSite?: boolean;
      withParentSiteArea?: boolean;
      issuer?: boolean;
      name?: string;
      withChargingStations?: boolean;
      withOnlyChargingStations?: boolean;
      withAvailableChargingStations?: boolean;
      chargingStationConnectorStatuses?: ChargePointStatus[];
      ...
      excludeSiteAreaIDs?: string[];
    } = {},
    dbParams: DbParams,
    projectFields?: string[]
  ): Promise<DataResult<SiteArea>> {
    const startTime = Logging.traceDatabaseRequestStart();
    let withChargingStations =
      params.withChargingStations ||
      params.withOnlyChargingStations ||
      params.withAvailableChargingStations;
    DatabaseUtils.checkTenantObject(tenant);
    ...
    if (!Utils.isEmptyArray(params.excludeSiteAreaIDs)) {
      filters._id = {
        $nin: params.excludeSiteAreaIDs.map((siteAreaID) =>
          DatabaseUtils.convertToObjectID(siteAreaID)
        ),
      };
    }
    // Parent Site Area
    if (!Utils.isEmptyArray(params.parentSiteAreaIDs)) {
      filters.parentSiteAreaID = {
        $in: params.parentSiteAreaIDs.map((parentSiteAreaID) =>
          DatabaseUtils.convertToObjectID(parentSiteAreaID)
        ),
      };
    }
    // Site
    ...
    if (params.withNoParentSiteArea) {
      const noParentFilter = {
        $or: [{ parentSiteAreaID: { $exists: false } }, { parentSiteAreaID: { $eq: null } }],
      };
      if (filters.$and) {
    ...
    if (!Utils.isEmptyArray(params.chargingStationConnectorStatuses)) {
      ...
      // Filter
      DatabaseUtils.push2ArraysFilterInAggregation(
        aggregation,
        'chargingStations',
        'chargingStations.id',
    ...
    // Count Records
    const siteAreasCountMDB = (await global.database
      .getCollection<any>(tenant.id, 'siteareas')
    ...
    if (params.withParentSiteArea) {
      DatabaseUtils.pushSiteAreaLookupInAggregation({
        tenantID: tenant.id,
        aggregation,
        localField: 'parentSiteAreaID',
        foreignField: '_id',
        asField: 'parentSiteArea',
        oneToOneCardinality: true,
      });
    }
    // Charging Stations
    ...
    // Read DB
    const siteAreasMDB = (await global.database
      .getCollection<any>(tenant.id, 'siteareas')
  ...
  public static async addChargingStationsToSiteArea(
    tenant: Tenant,
    siteAreaID: string,
    siteID: string,
    companyID: string,
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'addChargingStationsToSiteArea',
      startTime,
      chargingStationIDs
    );
  }

  public static async removeChargingStationsFromSiteArea(
    tenant: Tenant,
    siteAreaID: string,
    chargingStationIDs: string[]
  ): Promise<void> {
    ...
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'removeChargingStationsFromSiteArea',
      startTime,
      chargingStationIDs
    );
  }
  ...
  public static async deleteSiteAreas(tenant: Tenant, siteAreaIDs: string[]): Promise<void> {
    ...
    await global.database
      .getCollection<any>(tenant.id, 'siteareas')
      .deleteMany({ _id: { $in: siteAreaIDs.map((ID) => DatabaseUtils.convertToObjectID(ID)) } });
    // Delete Image
    await global.database
      .getCollection<any>(tenant.id, 'sitesareaimages')
      .deleteMany({ _id: { $in: siteAreaIDs.map((ID) => DatabaseUtils.convertToObjectID(ID)) } });
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
  ...
  public static async deleteSiteAreasFromSites(tenant: Tenant, siteIDs: string[]): Promise<void> {
    ...
    const siteareas: string[] = (
      await global.database
        .getCollection<any>(tenant.id, 'siteareas')
        .find({ siteID: { $in: siteIDs.map((id) => DatabaseUtils.convertToObjectID(id)) } })
        .project({ _id: 1 })
        .toArray()
    ).map((idWrapper): string => idWrapper._id.toString());
    // Delete site areas
    await SiteAreaStorage.deleteSiteAreas(tenant, siteareas);
    await Logging.traceDatabaseRequestEnd(
      tenant,
      MODULE_NAME,
      'deleteSiteAreasFromSites',
      startTime,
      siteIDs
    );
  }

  private static async saveSiteAreaImage(
    tenant: Tenant,
    siteAreaID: string,
    siteAreaImageToSave: string
  ): Promise<void> {
    const startTime = Logging.traceDatabaseRequestStart();
    DatabaseUtils.checkTenantObject(tenant);
    // Modify
    await global.database
      .getCollection<any>(tenant.id, 'siteareaimages')
      .findOneAndUpdate(
        { _id: DatabaseUtils.convertToObjectID(siteAreaID) },
