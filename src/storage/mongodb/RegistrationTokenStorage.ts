...
export default class RegistrationTokenStorage {
  public static async saveRegistrationToken(
    tenant: Tenant,
    ...
    const registrationTokenMDB = {
      _id: registrationToken.id
        ? DatabaseUtils.convertToObjectID(registrationToken.id)
        : new ObjectId(),
      description: registrationToken.description,
      siteAreaID: registrationToken.siteAreaID
        ? DatabaseUtils.convertToObjectID(registrationToken.siteAreaID)
        : null,
    ...
    await global.database
      .getCollection<any>(tenant.id, 'registrationtokens')
      .findOneAndUpdate(
    ...
    await Logging.traceDatabaseRequestEnd(
      ...
      startTime,
      registrationTokenMDB
    );
    return registrationTokenMDB._id.toString();
  }

  public static async getRegistrationTokens(
    tenant: Tenant,
    params: {
      search?: string;
      tokenIDs?: string[];
    ...
    // Search
    if (params.search) {
      filters.$or = [{ description: { $regex: params.search, $options: 'i' } }];
    ...
    // Count Records
    const registrationTokensCountMDB = (await global.database
      .getCollection<any>(tenant.id, 'registrationtokens')
    ...
    // Read DB
    const registrationTokens = (await global.database
      .getCollection<any>(tenant.id, 'registrationtokens')
  ...
  public static async getRegistrationToken(
    tenant: Tenant,
