...
export default class OwnUserDynamicAuthorizationFilter extends DynamicAuthorizationFilter {
  public processFilter(
    ...
    ) as OwnUserDynamicAuthorizationDataSource;
    ...
    if (userID) {
      ...
      if (
        ...
        // Override
        authorizationFilters.filters.userIDs = filteredUserIDs.filter((user) =>
          authorizationFilters.filters.userIDs.includes(user)
        );
        // Check auth
