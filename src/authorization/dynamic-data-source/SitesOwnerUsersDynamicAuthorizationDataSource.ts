import {
  ...
  SitesOwnerUsersDynamicAuthorizationDataSourceData,
...
export default class SitesOwnerUsersDynamicAuthorizationDataSource extends DynamicAuthorizationDataSource<SitesOwnerUsersDynamicAuthorizationDataSourceData> {
  ...
  private async getSitesOwnerUserSiteIDs(): Promise<{
    siteIDs: string[];
    tagIDs: string[];
    userID: string;
  }> {
    // Get the Site IDs of the Sites for which the user is Site Owner
    const sites = await UserStorage.getUserSites(
      this.tenant,
      {
    ...
    return {
      ...
      tagIDs: tags.result.map((tag) => tag.id),
