...
export class CommonUtilsService {
  public static async isAuthorizedOnChargingStation(
    tenant: Tenant,
    ...
    return CommonUtilsService.isTagIDAuthorizedOnChargingStation(
      tenant,
      chargingStation,
      null,
      tagID,
  ...
  public static async isAuthorizedToStartTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    tagID: string,
  ...
  public static async isAuthorizedToStopTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    tagID: string,
    action: ServerAction,
    authAction?: Action
  ): Promise<{ user: User; tag: Tag; alternateUser: User; alternateTag }> {
  ...
  private static async isTagIDAuthorizedOnChargingStation(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    tagID: string,
    ...
    // Get Authorized Tag
    const tag = await Authorizations.checkAndGetAuthorizedTag(
      action,
      tenant,
      chargingStation,
      tagID
    ...
    if (!tag) {
      // Check OICP first
      const user = await Authorizations.checkAndGetOICPAuthorizedUser(
        action,
        tenant,
        transaction,
        tagID
    ...
    // Get Authorized User
    const user = await CommonUtilsService.checkAndGetAuthorizedUserFromTag(
      action,
      tenant,
      chargingStation,
      transaction,
    ...
    // Check OCPI
    if (user && !user.issuer) {
      await Authorizations.checkOCPIAuthorizedUser(
        action,
        tenant,
        chargingStation,
        transaction,
  ...
  private static async checkAndGetAuthorizedUserFromTag(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    ...
    if (user.status !== UserStatus.ACTIVE) {
      throw new BackendError({
        ...LoggingHelper.getChargingStationProperties(chargingStation),
        action: action,
        message: `User with Tag ID '${tag.id}' is not Active ('${Utils.getStatusDescription(
          user.status
        )}')`,
        module: MODULE_NAME,
