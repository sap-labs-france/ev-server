...
export default class OCPPValidator extends SchemaValidator {
  ...
  private bootNotificationRequest: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/boot-notification-request.json`,
      'utf8'
    )
  );
  private authorizeRequest: Schema = JSON.parse(
    fs.readFileSync(`${global.appRoot}/assets/server/ocpp/schemas/authorize-request.json`, 'utf8')
  );
  private statusNotificationRequest: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/status-notification-request.json`,
      'utf8'
    )
  );
  private startTransactionRequest: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/start-transaction-request.json`,
      'utf8'
    )
  );
  private stopTransactionRequest16: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/stop-transaction-request-16.json`,
      'utf8'
    )
  );
  private stopTransactionRequest15: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/stop-transaction-request-15.json`,
      'utf8'
    )
  );
  private diagnosticsStatusNotificationRequest: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/diagnostics-status-notification-request.json`,
      'utf8'
    )
  );
  private heartbeatRequest: Schema = JSON.parse(
    fs.readFileSync(`${global.appRoot}/assets/server/ocpp/schemas/heartbeat-request.json`, 'utf8')
  );
  private firmwareStatusNotificationRequest: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/firmware-status-notification-request.json`,
      'utf8'
    )
  );
  private dataTransferRequest: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/data-transfert-request.json`,
      'utf8'
    )
  );
  private meterValueRequest: Schema = JSON.parse(
    fs.readFileSync(
      `${global.appRoot}/assets/server/ocpp/schemas/meter-values-request.json`,
      'utf8'
    )
  );
  ...
  public validateStatusNotification(
    ...
    if (
      ...
      new Date(statusNotification.timestamp).getFullYear() === new Date(0).getFullYear()
    ) {
  ...
  public validateBootNotification(
    headers: OCPPHeader,
    ...
    if (!headers.chargeBoxIdentity) {
      throw new BackendError({
        ...
        module: MODULE_NAME,
        method: 'validateBootNotification',
  ...
  public validateDiagnosticsStatusNotification(
  ...
  public validateStartTransaction(
    chargingStation: ChargingStation,
  ...
  public validateStopTransaction(
    chargingStation: ChargingStation,
  ...
  private cleanUpTagID(tagID: string): string {
    ...
    if (tagID && typeof tagID === 'string' && tagID.toLowerCase().includes(';')) {
      tagID = tagID.split(';')[1];
    }
    return tagID;
  }
