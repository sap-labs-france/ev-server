...
export default class JsonOCPPServer extends OCPPServer {
      ...
      .ws('/*', {
        ...
        close: async (ws: WebSocket, code: number, message: ArrayBuffer) => {
          ...
          await this.logWSConnectionClosed(
            ...
            code,
            `${WebSocketAction.CLOSE} > WS Connection ID '${
              wsWrapper.guid
            }' closed by charging station with code '${code}', reason: '${
              !Utils.isNullOrEmptyString(reason) ? reason : 'No reason given'
            }'`
          );
          // Remove connection
          await this.removeWSWrapper(
  ...
  public async getChargingStationClient(
    tenant: Tenant,
  ...
  private async onUpgrade(
    ...
    try {
      ...
      const message = `${
        WebSocketAction.UPGRADE
      } > New WS Connection with URL '${url}' failed with error: ${error.message as string}`;
      res.writeStatus('500');
      res.end(message);
  ...
  private async onOpen(ws: uWS.WebSocket) {
    ...
    await this.acquireLockForWSRequest(
    ...
    try {
      ...
      await this.closeWebSocket(
        ...
        `${WebSocketAction.OPEN} > WS Connection ID '${
          wsWrapper.guid
        }' has been rejected and closed by server due to an exception: ${error.message as string}`
      );
    } finally {
      this.runningWSMessages--;
  ...
  private async checkAndStoreWSOpenedConnection(
    ...
    if (!wsWrapper.closed) {
      ...
      const message = `${WebSocketAction.OPEN} > WS Connection ID '${
        wsWrapper.guid
      }' has been accepted in ${Utils.computeTimeDurationSecs(timeStart)} secs`;
      ...
      await this.logWSConnectionClosed(
        ...
        `${WebSocketAction.OPEN} > WS Connection ID '${
          wsWrapper.guid
        }' has been closed during initialization in ${Utils.computeTimeDurationSecs(
          timeStart
        )} secs ('${wsWrapper.url}')`
      );
    }
  ...
  private async checkAndCloseIdenticalOpenedWSConnection(
    wsWrapper: WSWrapper,
    wsConnection: WSConnection
  ): Promise<void> {
    // Get connection
    const existingWSConnection = this.getWSConnectionFromProtocolAndID(
      wsConnection.getWS().protocol,
      wsConnection.getID()
    );
  ...
  private async acquireLockForWSRequest(
  ...
  private releaseLockForWSMessageRequest(
    wsWrapper: WSWrapper,
    ocppMessageType?: OCPPMessageType
  ): void {
    // Only lock requests, not responses
  ...
  private async onMessage(ws: uWS.WebSocket, message: string, isBinary: boolean): Promise<void> {
    ...
    try {
      ...
      const logMessage = `${WebSocketAction.MESSAGE} > WS Connection ID '${
        wsWrapper.guid
      }' got error while processing WS Message: ${error.message as string}`;
      if (wsWrapper?.tenantID) {
        await Logging.logError({
          ...LoggingHelper.getWSWrapperProperties(wsWrapper),
  ...
  private async checkWSConnectionFromOnMessage(wsWrapper: WSWrapper) {
    ...
    const wsExistingConnection = this.getWSConnectionFromProtocolAndID(
      wsWrapper.protocol,
      wsWrapper.key
    );
  ...
  private async logWSConnectionClosed(
    wsWrapper: WSWrapper,
    action: ServerAction,
    code: number,
    message: string
  ...
  private async waitForWSLockToRelease(
    wsAction: WebSocketAction,
    action: ServerAction,
    wsWrapper: WSWrapper
  ): Promise<boolean> {
      ...
      while (true) {
        ...
        if (!this.runningWSRequestsMessages[wsWrapper.url]) {
          await Logging.logInfo({
            ...
            module: MODULE_NAME,
            method: 'waitForWSLockToRelease',
            message: `${wsAction} > WS Connection ID '${
              wsWrapper.guid
            }' - Lock has been acquired successfully after ${numberOfTrials} trial(s) and ${Utils.computeTimeDurationSecs(
              timeStart
            )} secs`,
            detailedMessages: { wsWrapper: this.getWSWrapperData(wsWrapper) },
          });
        ...
        if (numberOfTrials >= maxNumberOfTrials) {
          ...
          await Logging.logError({
            ...
            message: `${wsAction} > WS Connection ID '${
              wsWrapper.guid
            }' - Cannot acquire the lock after ${numberOfTrials} trial(s) and ${Utils.computeTimeDurationSecs(
              timeStart
            )} secs - Lock will be forced to be released`,
            detailedMessages: { wsWrapper: this.getWSWrapperData(wsWrapper) },
          });
          // Free the lock
          this.waitingWSMessages--;
    ...
    try {
      ...
      if (wsWrapper.nbrPingFailed >= Constants.WS_MAX_NBR_OF_FAILED_PINGS) {
        ...
        await Logging.logWarning({
          ...
          module: MODULE_NAME,
          method: 'pingWebSocket',
          message: `${WebSocketAction.PING} > Failed to ping the WS Connection ID '${
            wsWrapper.guid
          }' after ${wsWrapper.nbrPingFailed} trial(s) (${
    ...
    if (!wsWrapper.closed) {
      try {
        ...
        await Logging.logError({
          ...
          message: `${wsAction} > Failed to close WS Connection ID '${wsWrapper.guid}': ${
  ...
  private getWSConnectionFromProtocolAndID(
    protocol: WSServerProtocol,
  ...
  private async removeWSWrapper(
    ...
    if (wsWrapper.protocol === WSServerProtocol.OCPP16) {
      await this.removeWSConnection(
        wsAction,
        action,
        wsWrapper.wsConnection,
        this.jsonWSConnections
      );
    }
    if (wsWrapper.protocol === WSServerProtocol.REST) {
      await this.removeWSConnection(
    ...
    setInterval(() => {
      try {
        ...
        Logging.logDebug({
          ...
          message: `${this.jsonWSConnections.size} WS connections, ${
            this.jsonRestWSConnections.size
          } REST connections, ${this.runningWSMessages} Messages, ${
            Object.keys(this.runningWSRequestsMessages).length
          } Requests, ${this.waitingWSMessages} queued WS Message(s)`,
          detailedMessages: [
        ...
        if (Utils.isMonitoringEnabled()) {
          global.monitoringServer
            .getGauge(Constants.WEB_SOCKET_RUNNING_REQUEST_RESPONSE)
            .set(this.runningWSMessages);
          global.monitoringServer
            .getGauge(Constants.WEB_SOCKET_OCPP_CONNECTIONS_COUNT)
            .set(this.jsonWSConnections.size);
          global.monitoringServer
            .getGauge(Constants.WEB_SOCKET_REST_CONNECTIONS_COUNT)
            .set(this.jsonRestWSConnections.size);
          global.monitoringServer
            .getGauge(Constants.WEB_SOCKET_CURRENT_REQUEST)
            .set(numberOfCurrentRequests);
          global.monitoringServer
            .getGauge(Constants.WEB_SOCKET_RUNNING_REQUEST)
            .set(Object.keys(this.runningWSRequestsMessages).length);
          global.monitoringServer
            .getGauge(Constants.WEB_SOCKET_QUEUED_REQUEST)
            .set(this.waitingWSMessages);
        }
        if (this.isDebug()) {
          Logging.logConsoleDebug('=====================================');
          Logging.logConsoleDebug(`** ${this.jsonWSConnections.size} JSON Connection(s)`);
          Logging.logConsoleDebug(
            `** ${numberOfCurrentRequests} JSON WS Requests in cache with a size of ${
              sizeOfCurrentRequestsBytes / 1000
            } kB`
          );
          Logging.logConsoleDebug(`** ${this.jsonRestWSConnections.size} REST Connection(s)`);
          Logging.logConsoleDebug(
            `** ${Object.keys(this.runningWSRequestsMessages).length} running WS Requests`
          );
          Logging.logConsoleDebug(
            `** ${this.runningWSMessages} running WS Messages (Requests + Responses)`
          );
  ...
  private checkAndCleanupAllWebSockets() {
    setInterval(() => {
      ...
      this.checkAndCleanupWebSockets(this.jsonWSConnections, 'CS').catch(() => {
        /* Intentional */
      });
      // Check Rest connections
      this.checkAndCleanupWebSockets(this.jsonRestWSConnections, 'REST').catch(() => {
        /* Intentional */
      });
    }, Configuration.getChargingStationConfig().pingIntervalOCPPJSecs * 1000);
  }

  private async checkAndCleanupWebSockets(
    wsConnections: Map<string, WSConnection>,
    type: 'CS' | 'REST'
  ) {
    ...
    if (!Utils.isEmptyArray(wsConnectionKeys)) {
      ...
      if (validConnections.length || invalidConnections.length) {
        const message = `Total of ${
          wsConnectionKeys.length
        } ${type} WS connection(s) pinged in ${Utils.computeTimeDurationSecs(timeStart)} secs: ${
  ...
  private getWSWrapperData(wsWrapper: WSWrapper): Record<string, any> {
    return {
      ...
      durationSecs: Utils.computeTimeDurationSecs(
        new Date(wsWrapper.firstConnectionDate).getTime()
      ),
