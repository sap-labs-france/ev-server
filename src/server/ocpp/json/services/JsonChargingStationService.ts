...
export default class JsonChargingStationService {
  ...
  public async handleBootNotification(headers: OCPPHeader, payload: OCPPBootNotificationRequest): Promise<OCPPBootNotificationResponse> {
    // Forward
    const result: OCPPBootNotificationResponse = await this.handle(OCPP16Command.BOOT_NOTIFICATION, headers, payload);
    // Return the response
    return {
  ...
  public async handleHeartbeat(headers: OCPPHeader, payload: OCPPHeartbeatRequest): Promise<OCPPHeartbeatResponse> {
    // Forward
    const result: OCPPHeartbeatResponse = await this.handle(OCPP16Command.HEARTBEAT, headers, payload);
    // Return the response
    return {
      currentTime: result.currentTime
    };
  ...
  public async handleStatusNotification(headers: OCPPHeader, payload: OCPPStatusNotificationRequest): Promise<OCPPStatusNotificationResponse> {
    // Forward
    await this.handle(OCPP16Command.STATUS_NOTIFICATION, headers, payload);
    // Return the response
    return {};
  }
  ...
  public async handleMeterValues(headers: OCPPHeader, payload: OCPPMeterValuesRequest): Promise<OCPPMeterValuesResponse> {
    // Forward
    await this.handle(OCPP16Command.METERVALUES, headers, payload);
    // Return the response
    return {};
  }
  ...
  public async handleAuthorize(headers: OCPPHeader, payload: OCPPAuthorizeRequest): Promise<OCPPAuthorizeResponse> {
    // Forward
    const result: OCPPAuthorizeResponse = await this.handle(OCPP16Command.AUTHORIZE, headers, payload);
    // Return the response
    return {
      idTagInfo: {
  ...
  public async handleDiagnosticsStatusNotification(headers: OCPPHeader, payload: OCPPDiagnosticsStatusNotificationRequest): Promise<OCPPDiagnosticsStatusNotificationResponse> {
    // Forward
    await this.handle(OCPP16Command.DIAGNOSTICS_STATUS_NOTIFICATION, headers, payload);
    // Return the response
    return {};
  }
  ...
  public async handleFirmwareStatusNotification(headers: OCPPHeader, payload: OCPPFirmwareStatusNotificationRequest): Promise<OCPPFirmwareStatusNotificationResponse> {
    // Forward
    await this.handle(OCPP16Command.FIRMWARE_STATUS_NOTIFICATION, headers, payload);
    // Return the response
    return {};
  }
  ...
  public async handleStartTransaction(headers: OCPPHeader, payload: OCPPStartTransactionRequest): Promise<OCPPStartTransactionResponse> {
    // Forward
    const result: OCPPStartTransactionResponse = await this.handle(OCPP16Command.START_TRANSACTION, headers, payload);
    // Return the response
    return {
  ...
  public async handleDataTransfer(headers: OCPPHeader, payload: OCPPDataTransferRequest): Promise<OCPPDataTransferResponse> {
    // Forward
    const result: OCPPDataTransferResponse = await this.handle(OCPP16Command.DATA_TRANSFER, headers, payload);
    // Return the response
    return {
      status: result.status
    };
  ...
  public async handleStopTransaction(headers: OCPPHeader, payload: OCPPStopTransactionRequest): Promise<OCPPStopTransactionResponse> {
    // Forward
    const result: OCPPStopTransactionResponse = await this.handle(OCPP16Command.STOP_TRANSACTION, headers, payload);
    // Return the response
    return {
      idTagInfo: {
        status: result.idTagInfo.status
  ...
  public async handleGet15118EVCertificate(headers: OCPPHeader, payload: OCPPGet15118EVCertificateRequest): Promise<OCPPGet15118EVCertificateResponse> {
    // Forward
    const result: OCPPGet15118EVCertificateResponse = await this.handle(OCPP16Command.GET_15118_EV_CERTIFICATE, headers, payload);
    // Return the response
    return {
      status: result.status,
  ...
  private async handle(command: OCPP16Command, headers: OCPPHeader, payload) {
    try {
      // Handle
      return await this.chargingStationService['handle' + command](headers, payload);
    } catch (error) {
      Logging.logException(error, command as unknown as ServerAction, headers.chargeBoxIdentity, MODULE_NAME, command, headers.tenantID);
      throw error;
    }
  }
}
