...
import ChargingStation, {
  ...
  ChargingStationTemplateConnector,
  Command,
  Connector,
...
import {
  ...
  OCPPPhase,
  OCPPReadingContext,
  OCPPStopTransactionRequestExtended,
  OCPPUnitOfMeasure,
  OCPPValueFormat,
...
export default class OCPPUtils {
  ...
  public static async ensureChargingStationHasValidConnectionToken(
    action: ServerAction,
    tenant: Tenant,
    ...
    if (!tokenID) {
      throw new BackendError({
        ...
        chargingStationID,
        module: MODULE_NAME,
        method: 'ensureChargingStationHasValidConnectionToken',
  ...
  public static async buildAndPriceExtraConsumptionInactivity(
    ...
    const lastConsumption = await OCPPUtils.buildExtraConsumptionInactivity(
      tenant,
      lastTransaction
    );
    if (lastConsumption) {
      ...
      if (lastConsumption?.toPrice) {
        await PricingFacade.processEndTransaction(
          tenant,
          lastTransaction,
          chargingStation,
          lastConsumption,
      ...
      lastTransaction.stop.totalDurationSecs = moment
        .duration(moment(lastTransaction.stop.timestamp).diff(lastTransaction.timestamp))
        .asSeconds();
  ...
  public static async buildExtraConsumptionInactivity(
    tenant: Tenant,
    ...
    if (extraInactivitySecs > 0) {
      ...
      const lastConsumption = await ConsumptionStorage.getLastTransactionConsumption(tenant, {
      ...
      if (lastConsumption) {
        ...
        lastConsumption.endedAt = moment(transaction.stop.timestamp)
          .add(extraInactivitySecs, 's')
          .toDate();
        lastConsumption.inactivitySecs = extraInactivitySecs;
        lastConsumption.totalDurationSecs = Utils.createDecimal(lastConsumption.totalDurationSecs)
          .add(extraInactivitySecs)
          .toNumber();
        lastConsumption.totalInactivitySecs = Utils.createDecimal(
          lastConsumption.totalInactivitySecs
        )
          .add(extraInactivitySecs)
          .toNumber();
        // Set inactivity
  ...
  public static assertConsistencyInConsumption(
    chargingStation: ChargingStation,
    connectorID: number,
    ...
    if (
      consumption.instantWattsL1 > 0 ||
      consumption.instantWattsL2 > 0 ||
      consumption.instantWattsL3 > 0
    ) {
      ...
      const totalWatts = Utils.createDecimal(consumption.instantWattsL1)
        .plus(consumption.instantWattsL2)
        .plus(consumption.instantWattsL3)
        .toNumber();
      // Tolerance ± 10%
    ...
    if (
      consumption.instantAmpsL1 > 0 ||
      consumption.instantAmpsL2 > 0 ||
      consumption.instantAmpsL3 > 0
    ) {
      ...
      const totalAmps = Utils.createDecimal(consumption.instantAmpsL1)
        .plus(consumption.instantAmpsL2)
        .plus(consumption.instantAmpsL3)
        .toNumber();
      // Tolerance ± 10%
    ...
    if (!consumption.instantWatts) {
      ...
      if (consumption.instantAmps > 0) {
        if (consumption.instantVolts > 0) {
          consumption.instantWatts = Utils.createDecimal(consumption.instantVolts)
            .mul(consumption.instantAmps)
            .toNumber();
        } else {
          consumption.instantWatts = Utils.convertAmpToWatt(
            chargingStation,
            null,
            connectorID,
        ...
        const diffSecs =
          moment(consumption.endedAt).diff(consumption.startedAt, 'milliseconds') / 1000;
        // Consumption is always provided
        const sampleMultiplierWhToWatt =
          diffSecs > 0 ? Utils.createDecimal(3600).div(diffSecs).toNumber() : 0;
        consumption.instantWatts = Utils.createDecimal(consumption.consumptionWh)
          .mul(sampleMultiplierWhToWatt)
          .toNumber();
      }
    }
    ...
    if (!consumption.instantAmps) {
      ...
      if (consumption.instantWatts > 0) {
        if (consumption.instantVolts > 0) {
          consumption.instantAmps = Utils.createDecimal(consumption.instantWatts)
            .div(consumption.instantVolts)
            .toNumber();
        } else {
          consumption.instantAmps = Utils.convertWattToAmp(
            chargingStation,
            null,
            connectorID,
    ...
    if (
      !consumption.instantWattsL1 &&
      !consumption.instantWattsL2 &&
      !consumption.instantWattsL3 &&
      (consumption.instantAmpsL1 > 0 ||
        consumption.instantAmpsL2 > 0 ||
        consumption.instantAmpsL3 > 0)
    ) {
      if (consumption.instantVoltsL1 > 0) {
        consumption.instantWattsL1 = Utils.createDecimal(consumption.instantAmpsL1)
          .mul(consumption.instantVoltsL1)
          .toNumber();
      } else {
        consumption.instantWattsL1 = Utils.convertAmpToWatt(
          chargingStation,
          null,
          connectorID,
          consumption.instantAmpsL1
        );
      }
      if (consumption.instantVoltsL2 > 0) {
        consumption.instantWattsL2 = Utils.createDecimal(consumption.instantAmpsL2)
          .mul(consumption.instantVoltsL2)
          .toNumber();
      } else {
        consumption.instantWattsL2 = Utils.convertAmpToWatt(
          chargingStation,
          null,
          connectorID,
          consumption.instantAmpsL2
        );
      }
      if (consumption.instantVoltsL3 > 0) {
        consumption.instantWattsL3 = Utils.createDecimal(consumption.instantAmpsL3)
          .mul(consumption.instantVoltsL3)
          .toNumber();
      } else {
        consumption.instantWattsL3 = Utils.convertAmpToWatt(
          chargingStation,
          null,
          connectorID,
          consumption.instantAmpsL3
        );
      }
    }
    // Fill Power per Phase
    if (
      !consumption.instantWattsDC &&
      consumption.instantAmpsDC > 0 &&
      consumption.instantVoltsDC > 0
    ) {
      consumption.instantWattsDC = Utils.createDecimal(consumption.instantAmpsDC)
        .mul(consumption.instantVoltsDC)
        .toNumber();
    }
  }

  public static updateTransactionWithConsumption(
    chargingStation: ChargingStation,
    transaction: Transaction,
    consumption: Consumption
  ): void {
    ...
    transaction.currentTotalConsumptionWh = Utils.convertToFloat(
      consumption.cumulatedConsumptionWh
    );
    ...
    transaction.currentTotalDurationSecs = moment
      .duration(
        moment(
          transaction.lastConsumption ? transaction.lastConsumption.timestamp : new Date()
        ).diff(moment(transaction.timestamp))
      )
      .asSeconds();
  ...
  public static async rebuildTransactionSimplePricing(
    tenant: Tenant,
    transaction: Transaction,
    pricePerkWh?: number
  ): Promise<void> {
    if (!transaction) {
      throw new BackendError({
        ...
        module: MODULE_NAME,
        method: 'rebuildTransactionSimplePricing',
    ...
    const transactionSimplePricePerkWh =
      pricePerkWh > 0
        ? pricePerkWh
        : Utils.roundTo(transaction.stop.price / (transaction.stop.totalConsumptionWh / 1000), 2);
    // Get the consumptions
    const consumptionDataResult = await ConsumptionStorage.getTransactionConsumptions(tenant, {
      transactionId: transaction.id,
    });
    ...
    for (const consumption of consumptions) {
      // Update the price
      consumption.amount = Utils.computeSimplePrice(
        transactionSimplePricePerkWh,
        consumption.consumptionWh
      );
      consumption.roundedAmount = Utils.truncTo(consumption.amount, 2);
      transaction.currentCumulatedPrice = Utils.createDecimal(transaction.currentCumulatedPrice)
        .plus(consumption.amount)
        .toNumber();
  ...
  public static updateTransactionWithStopTransaction(
    transaction: Transaction,
    chargingStation: ChargingStation,
    stopTransaction: OCPPStopTransactionRequestExtended,
    user: User,
    alternateUser: User,
    tagId: string,
    isSoftStop: boolean
  ...
  public static createTransactionStopMeterValues(
    chargingStation: ChargingStation,
    transaction: Transaction,
    stopTransaction: OCPPStopTransactionRequestExtended
  ): OCPPNormalizedMeterValue[] {
    ...
    if (transaction.currentInstantVolts > 0 || transaction.currentInstantVoltsDC > 0) {
      stopMeterValues.push({
        id: (id++).toString(),
        ...meterValueBasedProps,
        value: transaction.currentInstantVolts
      ...
      stopMeterValues.push({
        ...
        value: transaction.currentInstantAmps
          ? transaction.currentInstantAmps /
            Utils.getNumberOfConnectedPhases(chargingStation, null, transaction.connectorId)
    ...
    if (transaction.currentInstantWatts > 0 || transaction.currentInstantWattsDC > 0) {
      stopMeterValues.push({
        id: (id++).toString(),
        ...meterValueBasedProps,
        value: transaction.currentInstantWatts
  ...
  public static async createConsumptionsFromMeterValues(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    meterValues: OCPPNormalizedMeterValue[]
  ): Promise<Consumption[]> {
    // Build consumptions
    ...
    for (const meterValue of meterValues) {
      ...
      if (OCPPUtils.isValidMeterValue(meterValue)) {
        ...
        if (
          transaction.lastConsumption?.timestamp &&
          ...
          await Logging.logError({
            ...
            tenantID: tenant.id,
            module: MODULE_NAME,
            method: 'createConsumptionsFromMeterValues',
        ...
        const consumption = await this.createConsumptionFromMeterValue(
          tenant,
          chargingStation,
          transaction,
    ...
    // Add missing info
    for (const consumption of consumptions) {
      OCPPUtils.assertConsistencyInConsumption(
        chargingStation,
        transaction.connectorId,
        consumption
      );
  ...
  public static async createFirstConsumption(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction
  ): Promise<Consumption> {
    const lastConsumption: { value: number; timestamp: Date } = {
    ...
    return OCPPUtils.createConsumptionFromMeterValue(
      tenant,
      chargingStation,
      transaction,
      lastConsumption,
  ...
  public static async createConsumptionFromMeterValue(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    lastConsumption: { value: number; timestamp: Date },
    meterValue: OCPPNormalizedMeterValue
  ): Promise<Consumption> {
    ...
    if (OCPPUtils.isValidMeterValue(meterValue)) {
      ...
      if (OCPPUtils.isSocMeterValue(meterValue)) {
        ...
        const powerInMeterValueWatts =
          meterValue.attribute?.unit === OCPPUnitOfMeasure.KILO_WATT
            ? Utils.createDecimal(powerInMeterValue).mul(1000).toNumber()
            : powerInMeterValue;
        ...
        const durationSecs = Utils.createDecimal(
          moment(meterValue.timestamp).diff(lastConsumption.timestamp, 'milliseconds')
        )
          .div(1000)
          .toNumber();
        // Handle current Connector limitation
        await OCPPUtils.addConnectorLimitationToConsumption(
          tenant,
          chargingStation,
          transaction.connectorId,
          consumption
        ...
        // Convert to Wh
        const meterValueWh =
          meterValue.attribute.unit === OCPPUnitOfMeasure.KILO_WATT_HOUR
            ? Utils.createDecimal(Utils.convertToFloat(meterValue.value)).mul(1000).toNumber()
            : Utils.convertToFloat(meterValue.value);
        // Check if valid Consumption
        if (meterValueWh > lastConsumption.value) {
          // Compute consumption
          consumption.consumptionWh = Utils.createDecimal(meterValueWh)
            .minus(lastConsumption.value)
            .toNumber();
          consumption.consumptionAmps = Utils.convertWattToAmp(
            chargingStation,
            null,
            transaction.connectorId,
            consumption.consumptionWh
          ...
          transaction.currentTotalConsumptionWh = Utils.createDecimal(
            ...
            .plus(consumption.consumptionWh)
            .toNumber();
          ...
          if (
            ...
            consumption.limitAmps >=
              StaticLimitAmps.MIN_LIMIT_PER_PHASE *
                Utils.getNumberOfConnectedPhases(chargingStation, null, transaction.connectorId)
          ) {
            // Update inactivity
        ...
        consumption.totalDurationSecs = !transaction.stop
          ? moment
              .duration(moment(meterValue.timestamp).diff(moment(transaction.timestamp)))
              .asSeconds()
          : moment
  ...
  public static async addSiteLimitationToConsumption(
    tenant: Tenant,
    siteArea: SiteArea,
    consumption: Consumption
  ): Promise<void> {
    if (Utils.isTenantComponentActive(tenant, TenantComponents.ORGANIZATION) && siteArea) {
      ...
      if (siteArea && siteArea.maximumPower) {
        ...
        consumption.limitSiteAreaAmps = Utils.createDecimal(siteArea.maximumPower)
          .div(siteArea.voltage)
          .toNumber();
        consumption.limitSiteAreaSource = SiteAreaLimitSource.SITE_AREA;
      } else {
        // Compute it for Charging Stations
        const chargingStationsOfSiteArea = await ChargingStationStorage.getChargingStations(
          tenant,
        ...
        for (const chargingStationOfSiteArea of chargingStationsOfSiteArea.result) {
          if (Utils.objectHasProperty(chargingStationOfSiteArea, 'connectors')) {
            for (const connector of chargingStationOfSiteArea.connectors) {
              consumption.limitSiteAreaWatts = Utils.createDecimal(consumption.limitSiteAreaWatts)
                .plus(connector.power)
                .toNumber();
            }
          }
        }
        consumption.limitSiteAreaAmps = Math.round(
          consumption.limitSiteAreaWatts / siteArea.voltage
        );
  ...
  public static async addConnectorLimitationToConsumption(
    tenant: Tenant,
    chargingStation: ChargingStation,
    connectorID: number,
    consumption: Consumption
  ...
  public static async enrichChargingStationConnectorWithTemplate(
    ...
    if (chargingStation.manualConfiguration) {
      ...
      await Logging.logWarning({
        ...
        module: MODULE_NAME,
        method: 'enrichChargingStationConnectorWithTemplate',
    ...
    if (chargingStationTemplate) {
      ...
      if (!Utils.isEmptyArray(chargingStationTemplate.template.technical?.connectors)) {
        ...
        if (!Utils.objectHasProperty(connector, 'phaseAssignmentToGrid')) {
          await OCPPUtils.setConnectorPhaseAssignment(
            tenant,
            chargingStation,
            connector,
            numberOfPhases
  ...
  public static async checkAndApplyTemplateToChargingStation(
    tenant: Tenant,
    chargingStation: ChargingStation,
    applyOcppParameters = true
  ): Promise<TemplateUpdateResult> {
    // Apply Template
    const chargingStationTemplateUpdateResult = await OCPPUtils.enrichChargingStationWithTemplate(
      tenant,
      chargingStation
    );
  ...
  public static async applyTemplateOcppParametersToChargingStation(
    tenant: Tenant,
    chargingStation: ChargingStation
    ...
    await Logging.logInfo({
      ...
      action: ServerAction.UPDATE_CHARGING_STATION_WITH_TEMPLATE,
      module: MODULE_NAME,
      method: 'applyTemplateOcppParametersToChargingStation',
    ...
    let result = await Utils.executePromiseWithTimeout<OCPPChangeConfigurationResponse>(
      ...
      OCPPCommon.requestAndSaveChargingStationOcppParameters(tenant, chargingStation),
    ...
    if (result.status !== OCPPConfigurationStatus.ACCEPTED) {
      await Logging.logError({
        ...
        method: 'applyTemplateOcppParametersToChargingStation',
        message:
          "Cannot request Charging Station's OCPP Parameters: Cannot apply Template's OCPP Parameters",
      });
    } else {
      // Update the OCPP Parameters from the template
      result = await Utils.executePromiseWithTimeout<OCPPChangeConfigurationResponse>(
        Constants.DELAY_CHANGE_CONFIGURATION_EXECUTION_MILLIS,
        OCPPUtils.updateChargingStationOcppParametersWithTemplate(tenant, chargingStation),
        `Time out error (${Constants.DELAY_CHANGE_CONFIGURATION_EXECUTION_MILLIS} ms): Cannot update Charging Station with Template's OCPP Parameters`
      );
      if (result.status === OCPPConfigurationStatus.ACCEPTED) {
        await Logging.logInfo({
  ...
  public static async clearAndDeleteChargingProfilesForSiteArea(
    tenant: Tenant,
    siteArea: SiteArea,
    params?: { profilePurposeType?: ChargingProfilePurposeType; transactionId?: number }
        ...
        for (const chargingProfile of chargingProfiles.result) {
          try {
            ...
            await Logging.logError({
              ...
              module: MODULE_NAME,
              method: 'clearAndDeleteChargingProfilesForSiteArea',
  ...
  public static async clearAndDeleteChargingProfile(
    tenant: Tenant,
    ...
    if (!chargingStation.capabilities?.supportChargingProfiles) {
      throw new BackendError({
        ...
        action: ServerAction.CHARGING_PROFILE_DELETE,
        module: MODULE_NAME,
        method: 'clearAndDeleteChargingProfile',
  ...
  public static async checkChargingStationAndEnrichSoapOcppHeaders(
    command: Command,
    headers: OCPPHeader,
    req: any
  ): Promise<void> {
  ...
  public static async setAndSaveChargingProfile(
    tenant: Tenant,
    chargingProfile: ChargingProfile
  ): Promise<string> {
    // Get charging station
    ...
    // Save
    const chargingProfileID = await ChargingStationStorage.saveChargingProfile(
      tenant,
      chargingProfile
    );
    await Logging.logInfo({
      ...
      message: `${Utils.buildConnectorInfo(
        ...
        chargingProfile.profile?.transactionId
      )} Charging Profile has been successfully pushed and saved`,
      detailedMessages: { chargingProfile },
    });
  ...
  public static isValidMeterValue(meterValue: OCPPNormalizedMeterValue): boolean {
    return (
      OCPPUtils.isSocMeterValue(meterValue) ||
  ...
  public static checkChargingStationConnectionData(
    action: ServerAction,
    tenantID: string,
    tokenID: string,
    chargingStationID: string
    ...
    if (!chargingStationID) {
      throw new BackendError({
        action,
        module: MODULE_NAME,
        method: 'checkChargingStationOcppParameters',
  ...
  public static async checkAndGetChargingStationConnectionData(
    action: ServerAction,
    tenantID: string,
    chargingStationID: string,
    tokenID: string,
    updateChargingStationData = true
  ): Promise<{ tenant: Tenant; chargingStation?: ChargingStation; token?: RegistrationToken }> {
    // Check parameters
    ...
    const chargingStation = await ChargingStationStorage.getChargingStation(
      tenant,
      chargingStationID,
      { withSiteArea: true, issuer: true }
    );
    if (!chargingStation) {
      // Must have a valid connection Token
      token = await OCPPUtils.ensureChargingStationHasValidConnectionToken(
        action,
        tenant,
        chargingStationID,
        tokenID
  ...
  public static async updateChargingStationOcppParametersWithTemplate(
    tenant: Tenant,
    chargingStation: ChargingStation
  ): Promise<OCPPChangeConfigurationResponse> {
    ...
    const currentOcppParameters = (
      await ChargingStationStorage.getOcppParameters(tenant, chargingStation.id)
    ).result;
    if (
      Utils.isEmptyArray(chargingStation.ocppStandardParameters) &&
      Utils.isEmptyArray(chargingStation.ocppVendorParameters)
    ) {
      await Logging.logInfo({
        ...LoggingHelper.getChargingStationProperties(chargingStation),
        tenantID: tenant.id,
    ...
    const ocppParameters = chargingStation.ocppStandardParameters.concat(
    ...
    for (const ocppParameter of ocppParameters) {
      ...
      try {
        ...
        result = await OCPPCommon.requestChangeChargingStationOcppParameter(
          tenant,
          chargingStation,
          {
        ...
        if (result.status === OCPPConfigurationStatus.ACCEPTED) {
          ...
          await Logging.logInfo({
            ...
            module: MODULE_NAME,
            method: 'updateChargingStationOcppParametersWithTemplate',
            message: `${!Utils.isUndefined(currentOcppParam) && 'Non existent '}OCPP Parameter '${
              ocppParameter.key
            }' has been successfully set from '${currentOcppParam?.value}' to '${
              ocppParameter.value
            }'`,
          });
          ...
          await Logging.logInfo({
            ...
            message: `${!Utils.isUndefined(currentOcppParam) && 'Non existent '}OCPP Parameter '${
              ocppParameter.key
            }' that requires reboot has been successfully set from '${
              currentOcppParam?.value
            }' to '${ocppParameter.value}'`,
          });
        } else {
          ...
          await Logging.logError({
            ...
            module: MODULE_NAME,
            method: 'updateChargingStationOcppParametersWithTemplate',
            message: `Error '${result.status}' in changing ${
              !Utils.isUndefined(currentOcppParam) && 'non existent '
            }OCPP Parameter '${ocppParameter.key}' from '${currentOcppParam?.value}' to '${
              ocppParameter.value
            }': `,
          });
        }
    ...
    await Logging.logActionsResponse(
      ...
      MODULE_NAME,
      'updateChargingStationOcppParametersWithTemplate',
      updatedOcppParameters,
      `{{inSuccess}} OCPP Parameter(s) were successfully synchronized, check details in the Tenant ${Utils.buildTenantName(
        tenant
      )})`,
      `{{inError}} OCPP Parameter(s) failed to be synchronized, check details in the Tenant ${Utils.buildTenantName(
        tenant
      )})`,
      `{{inSuccess}} OCPP Parameter(s) were successfully synchronized and {{inError}} failed to be synchronized, check details in the Tenant ${Utils.buildTenantName(
        tenant
      )})`,
    ...
    if (updatedOcppParameters.inSuccess) {
      result = await OCPPCommon.requestAndSaveChargingStationOcppParameters(
        tenant,
        chargingStation
      );
    }
  ...
  public static async updateChargingStationConnectorRuntimeDataWithTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    saveConnector = false
    ...
    if (connector) {
      if (!transaction.stop) {
        ...
        // Log
        const instantPower = Utils.truncTo(
          Utils.createDecimal(connector.currentInstantWatts).div(1000).toNumber(),
          3
        );
        const totalConsumption = Utils.truncTo(
          Utils.createDecimal(connector.currentTotalConsumptionWh).div(1000).toNumber(),
          3
        );
        await Logging.logInfo({
          ...LoggingHelper.getChargingStationProperties(chargingStation),
          tenantID: tenant.id,
          module: MODULE_NAME,
          method: 'updateChargingStationConnectorRuntimeDataWithTransaction',
          action: ServerAction.CONSUMPTION,
          user: transaction.userID,
          message: `${Utils.buildConnectorInfo(
            connector.connectorId,
            connector.currentTransactionID
          )} Power: ${instantPower} kW, Energy: ${totalConsumption} kW.h${
            connector.currentStateOfCharge
              ? ', SoC: ' + connector.currentStateOfCharge.toString() + ' %'
              : ''
          }`,
        });
        // Clear Connector
  ...
  public static clearChargingStationConnectorRuntimeData(
    chargingStation: ChargingStation,
    connectorID: number
  ): Connector {
  ...
  public static updateSignedData(
    transaction: Transaction,
    meterValue: OCPPNormalizedMeterValue
  ): boolean {
  ...
  public static async checkBillingPrerequisites(
    tenant: Tenant,
    action: ServerAction,
    chargingStation: ChargingStation,
    user: User
    ...
    if (billingImpl) {
      ...
      if (!Utils.isEmptyArray(errorCodes)) {
        throw new BackendError({
          ...
          module: MODULE_NAME,
          method: 'checkBillingPrerequisites',
  ...
  private static async enrichChargingStationWithTemplate(
    ...
    if (chargingStation.manualConfiguration) {
      await Logging.logWarning({
        ...
        action: ServerAction.UPDATE_CHARGING_STATION_WITH_TEMPLATE,
        module: MODULE_NAME,
        method: 'enrichChargingStationWithTemplate',
    ...
    if (chargingStationTemplate) {
      ...
      if (chargingStation.templateHash !== chargingStationTemplate.hash) {
        ...
        templateUpdateResult.technicalUpdated =
          OCPPUtils.enrichChargingStationWithTemplateTechnicalParams(
            chargingStation,
            chargingStationTemplate
          );
        // Check Capabilities
        templateUpdateResult.capabilitiesUpdated =
          await OCPPUtils.enrichChargingStationWithTemplateCapabilities(
            tenant,
            chargingStation,
            chargingStationTemplate
          );
        ...
        templateUpdateResult.ocppStandardUpdated =
          await OCPPUtils.enrichChargingStationWithTemplateOcppStandardParams(
            tenant,
            chargingStation,
            chargingStationTemplate
          );
  ...
  private static async enrichChargingStationWithTemplateOcppStandardParams(
    tenant: Tenant,
    chargingStation: ChargingStation,
    ...
    if (chargingStation.templateHashOcppStandard !== chargingStationTemplate.hashOcppStandard) {
      ...
      return OCPPUtils.enrichChargingStationWithTemplateOcppParams(
        tenant,
        chargingStation,
        chargingStationTemplate,
  ...
  private static async enrichChargingStationWithTemplateOcppVendorParams(
    tenant: Tenant,
    chargingStation: ChargingStation,
    chargingStationTemplate: ChargingStationTemplate
  ): Promise<boolean> {
  ...
  private static async enrichChargingStationWithTemplateOcppParams(
    tenant: Tenant,
    chargingStation: ChargingStation,
    chargingStationTemplate: ChargingStationTemplate,
    ocppProperty: 'ocppStandardParameters' | 'ocppVendorParameters'
  ): Promise<boolean> {
  ...
  private static async enrichChargingStationWithTemplateCapabilities(
    tenant: Tenant,
    chargingStation: ChargingStation,
    chargingStationTemplate: ChargingStationTemplate
  ): Promise<boolean> {
  ...
  private static enrichChargingStationWithTemplateTechnicalParams(
    chargingStation: ChargingStation,
    chargingStationTemplate: ChargingStationTemplate
  ): boolean {
  ...
  private static checkAndSetConnectorAmperageLimit(
    chargingStation: ChargingStation,
    connector: Connector,
    nrOfPhases?: number
  ): void {
    const numberOfPhases =
      nrOfPhases ?? Utils.getNumberOfConnectedPhases(chargingStation, null, connector.connectorId);
    // Check connector amperage limit
    const connectorAmperageLimit = OCPPUtils.checkAndGetConnectorAmperageLimit(
      chargingStation,
      connector,
      numberOfPhases
    );
  ...
  private static checkAndGetConnectorAmperageLimit(
    chargingStation: ChargingStation,
    connector: Connector,
    nrOfPhases?: number
  ): number {
    ...
    if (
      connector.amperageLimit && // Must be ignored when set to 0
  ...
  private static async setConnectorPhaseAssignment(
    tenant: Tenant,
    chargingStation: ChargingStation,
    connector: Connector,
    nrOfPhases?: number
  ): Promise<void> {
    const csNumberOfPhases =
      nrOfPhases ?? Utils.getNumberOfConnectedPhases(chargingStation, null, connector.connectorId);
    if (chargingStation.siteAreaID) {
      ...
      if (siteArea.numberOfPhases === 3) {
        switch (csNumberOfPhases) {
          // Tri-phased
          case 3:
            connector.phaseAssignmentToGrid = {
  ...
  private static isOcppParamForPowerLimitationKey(
    ocppParameterKey: string,
    chargingStation: ChargingStation
  ): boolean {
    for (const chargePoint of chargingStation.chargePoints) {
      if (
        chargePoint.ocppParamForPowerLimitation &&
        ocppParameterKey.includes(chargePoint.ocppParamForPowerLimitation)
      ) {
        return true;
      }
    }
