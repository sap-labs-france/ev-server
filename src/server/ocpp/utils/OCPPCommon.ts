...
export default class OCPPCommon {
  public static async requestChangeChargingStationOcppParameter(
    tenant: Tenant,
    chargingStation: ChargingStation,
    params: OCPPChangeConfigurationRequest,
    saveChange = true,
    triggerConditionalReset = false
  ...
  public static async requestAndSaveChargingStationOcppParameters(
    tenant: Tenant,
    chargingStation: ChargingStation
  ): Promise<OCPPChangeConfigurationResponse> {
    try {
      // Get the OCPP Configuration
      const ocppConfiguration = await OCPPCommon.requestChargingStationOcppParameters(
        tenant,
        chargingStation,
        {}
      );
      await Logging.logDebug({
        ...
        action: ServerAction.CHARGING_STATION_CHANGE_CONFIGURATION,
        module: MODULE_NAME,
        method: 'requestAndSaveChargingStationOcppParameters',
      ...
      if (!Utils.isEmptyArray(customParams)) {
        for (const customParam of customParams) {
          const foundCustomParam = chargingStationOcppParameters.configuration.find(
            (configuration) => configuration.key === customParam.key
          );
          if (!foundCustomParam) {
  ...
  public static async triggerChargingStationReset(
    tenant: Tenant,
    chargingStation: ChargingStation,
    hardResetFallback = false,
    resetType: OCPPResetType = OCPPResetType.SOFT
  ): Promise<OCPPResetResponse> {
    ...
    if (!chargingStationClient) {
      throw new BackendError({
        ...
        module: MODULE_NAME,
        method: 'triggerChargingStationReset',
        message: 'Charging Station is not connected to the backend',
      });
    }
  ...
  public static async requestChargingStationOcppParameters(
    ...
    if (!chargingStationClient) {
      throw new BackendError({
        ...
        module: MODULE_NAME,
        method: 'requestChargingStationOcppParameters',
        message: 'Charging Station is not connected to the backend',
      });
    }
