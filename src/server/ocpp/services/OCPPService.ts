...
import {
  ...
  OCPPReadingContext,
  OCPPSampledValue,
  OCPPStartTransactionRequestExtended,
...
export default class OCPPService {
  ...
  public async handleBootNotification(
    ...
    try {
      ...
      if (!chargingStation) {
        ...
        chargingStation = await this.createChargingStationFromBootNotification(
          tenant,
          bootNotification,
          headers
        );
      ...
      await this.enrichChargingStationFromBootNotification(
        tenant,
        chargingStation,
        headers,
        bootNotification
      ...
      return {
        ...
        currentTime: bootNotification.timestamp
          ? bootNotification.timestamp.toISOString()
    ...
    try {
      ...
      if (statusNotification.connectorId <= 0) {
        await Logging.logInfo({
          ...
          message: `Connector ID '0' > ${this.buildStatusNotification(
            statusNotification
          )}, will be ignored (Connector ID = '0')`,
          detailedMessages: { statusNotification },
        });
      } else {
        // Update only the given Connector ID
        await this.processConnectorFromStatusNotification(
          tenant,
          chargingStation,
          statusNotification
        );
  ...
  public async handleMeterValues(
    ...
    try {
      // Get the header infos
      const { chargingStation, tenant } = headers;
      await OCPPValidator.getInstance().validateMeterValues(
        tenant.id,
        chargingStation,
        meterValues
      );
      ...
      this.filterMeterValuesOnSpecificChargingStations(
        tenant,
        chargingStation,
        normalizedMeterValues
      );
      ...
      // Get Transaction
      const transaction = await this.getTransactionFromMeterValues(
        tenant,
        chargingStation,
        headers,
        meterValues
      ...
      if (FeatureToggles.isFeatureActive(Feature.OCPPJ_PERSIST_RAW_METER_VALUES)) {
        ...
        const endAt =
          normalizedMeterValues.values[normalizedMeterValues.values.length - 1].timestamp;
        // Save
      ...
      // Update Transaction
      this.updateTransactionWithMeterValues(
        chargingStation,
        transaction,
      ...
      // Handle current SOC
      await this.processTransactionCar(
        tenant,
        transaction,
        chargingStation,
        consumptions[consumptions.length - 1],
        null,
      ...
      for (const consumption of consumptions) {
        ...
        if (consumption.toPrice) {
          // Pricing
          await PricingFacade.processUpdateTransaction(
            tenant,
            transaction,
            chargingStation,
            consumption,
      ...
      if (
        !transaction.phasesUsed &&
        Utils.checkIfPhasesProvidedInTransactionInProgress(transaction) &&
        transaction.numberOfMeterValues >= 1
      ) {
        transaction.phasesUsed = Utils.getUsedPhasesInTransactionInProgress(
          chargingStation,
          transaction
        );
      }
      // OCPI
      await OCPIFacade.processUpdateTransaction(
        tenant,
        transaction,
        chargingStation,
      ...
      // OICP
      await OICPFacade.processUpdateTransaction(
        tenant,
        transaction,
        chargingStation,
        chargingStation.siteArea,
  ...
  public async handleAuthorize(
    ...
    try {
      ...
      await OCPPUtils.checkBillingPrerequisites(
        tenant,
        ServerAction.OCPP_AUTHORIZE,
        chargingStation,
        user
  ...
  public async handleDiagnosticsStatusNotification(
    ...
    try {
      // Check props
      OCPPValidator.getInstance().validateDiagnosticsStatusNotification(
  ...
  public async handleFirmwareStatusNotification(
    ...
    try {
      // Check props
      OCPPValidator.getInstance().validateFirmwareStatusNotification(
        chargingStation,
      ...
      await ChargingStationStorage.saveChargingStationFirmwareStatus(
        tenant,
        chargingStation.id,
  ...
  public async handleStartTransaction(
    headers: OCPPHeader,
    startTransaction: OCPPStartTransactionRequestExtended
    ...
    try {
      ...
      // Create consumption
      const firstConsumption = await OCPPUtils.createFirstConsumption(
        tenant,
        chargingStation,
        newTransaction
      );
      // Pricing
      await PricingFacade.processStartTransaction(
        tenant,
        newTransaction,
        chargingStation,
        firstConsumption,
      ...
      // Billing
      await BillingFacade.processStartTransaction(
        tenant,
        newTransaction,
        chargingStation,
        chargingStation.siteArea,
      ...
      // OCPI
      await OCPIFacade.processStartTransaction(
        tenant,
        newTransaction,
        chargingStation,
        chargingStation.siteArea,
      ...
      // OICP
      await OICPFacade.processStartTransaction(
        tenant,
        newTransaction,
        chargingStation,
        chargingStation.siteArea,
      ...
      await Logging.logInfo({
        ...
        user: user,
        message: `${Utils.buildConnectorInfo(
          newTransaction.connectorId,
          newTransaction.id
        )} Transaction has been started successfully`,
        detailedMessages: {
          transactionData: LoggingHelper.shrinkTransactionProperties(newTransaction),
          startTransaction,
        },
      });
      // Accepted
  ...
  public async handleDataTransfer(
    ...
    try {
      ...
      await Logging.logInfo({
        ...
        message: 'Data Transfer has been saved',
      ...
      await Logging.logActionExceptionMessage(
        headers.tenantID,
        ServerAction.CHARGING_STATION_DATA_TRANSFER,
        error,
        { dataTransfer }
      );
      // Rejected
  ...
  public async handleStopTransaction(
    ...
    isSoftStop = false,
    isStoppedByCentralSystem = false
    ...
    try {
      ...
      // Get Transaction
      const transaction = await this.getTransactionFromStopTransaction(
        tenant,
        chargingStation,
        headers,
        stopTransaction
      ...
      const { user, alternateUser } = await this.checkAuthorizeStopTransactionAndGetUsers(
        ...
        transaction,
        tagID,
        isStoppedByCentralSystem
      );
      // Free the connector
      const connector = OCPPUtils.clearChargingStationConnectorRuntimeData(
        chargingStation,
      ...
      await this.checkAndApplyLastConsumptionInStopTransaction(
        tenant,
        chargingStation,
        transaction,
        stopTransaction,
      ...
      OCPPUtils.updateTransactionWithStopTransaction(
        transaction,
        chargingStation,
        stopTransaction,
        user,
      ...
      // OCPI
      await OCPIFacade.processStopTransaction(
        tenant,
        transaction,
        chargingStation,
        chargingStation.siteArea,
      ...
      // OICP
      await OICPFacade.processStopTransaction(
        tenant,
        transaction,
        chargingStation,
        chargingStation.siteArea,
      ...
      await TransactionStorage.saveTransaction(tenant, transaction);
      // Notify
      NotificationHelper.notifyStopTransaction(
        tenant,
        transaction,
        chargingStation,
        user,
      ...
      await this.checkConnectorStatusOnStopTransaction(
        tenant,
        chargingStation,
        transaction,
        connector
      ...
      await Logging.logInfo({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} Transaction has been stopped successfully`,
  ...
  public async handleCancelReservation(
    ...
    if (!cancelReservation.reservationId) {
      throw new Error('Unexptected Situation - Reservation ID is not set');
    }
    try {
      const { chargingStation, tenant } = headers;
      ...
      await Logging.logError({
        ...
        message: `During cancellation of reservation ${cancelReservation.reservationId} an error occurred!`,
        module: MODULE_NAME,
        method: this.handleCancelReservation.name,
  ...
  public async softStopTransaction(
    tenant: Tenant,
    ...
    if (!chargingStation) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} The Charging Station is mandatory`,
        action: ServerAction.OCPP_STOP_TRANSACTION,
        detailedMessages: {
    ...
    const result = await this.handleStopTransaction(
      ...
      },
      {
        ...
        timestamp: Utils.convertToDate(
          transaction.lastConsumption
            ? transaction.lastConsumption.timestamp
            : transaction.timestamp
        ).toISOString(),
        meterStop: transaction.lastConsumption
          ? transaction.lastConsumption.value
  ...
  public async checkAuthorizeStopTransactionAndGetUsers(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    tagID: string,
    isStoppedByCentralSystem: boolean
  ): Promise<{ user: User; alternateUser: User }> {
    let user: User;
  ...
  private checkAndUpdateTransactionWithSignedDataInStopTransaction(
    transaction: Transaction,
    stopTransaction: OCPPStopTransactionRequestExtended
  ) {
  ...
  private async checkConnectorStatusOnStopTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    connector: Connector
    ...
    if (
      ...
      await Logging.logWarning({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} Connector is available before stopping the transaction`,
      });
  ...
  private async triggerSmartChargingStopTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction
  ) {
      ...
      setTimeout(async () => {
        try {
          ...
          await Logging.logError({
            ...
            tenantID: tenant.id,
            module: MODULE_NAME,
            method: 'triggerSmartChargingStopTransaction',
            action: ServerAction.OCPP_STOP_TRANSACTION,
            message: `${Utils.buildConnectorInfo(
              transaction.connectorId,
              transaction.id
            )} Smart Charging exception occurred`,
      ...
      try {
        ...
        await Logging.logDebug({
          ...
          message: `${Utils.buildConnectorInfo(
            transaction.connectorId,
            transaction.id
          )} TX Charging Profile with ID '${chargingProfile.id}'`,
          module: MODULE_NAME,
          method: 'deleteAllTransactionTxProfile',
        ...
        await Logging.logError({
          ...
          message: `${Utils.buildConnectorInfo(
            transaction.connectorId,
            transaction.id
          )} Cannot delete TX Charging Profile with ID '${chargingProfile.id}'`,
          module: MODULE_NAME,
          method: 'deleteAllTransactionTxProfile',
  ...
  public async triggerEndTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    ...
    await this.finalizeTransaction(
      ...
      transaction,
      connector,
      fakeStatusNotification
    );
  }

  // eslint-disable-next-line max-len
  private async processConnectorFromStatusNotification(
    tenant: Tenant,
    chargingStation: ChargingStation,
    statusNotification: OCPPStatusNotificationRequestExtended
  ) {
    ...
    const { connector, ignoreStatusNotification } =
      await this.checkAndGetConnectorFromStatusNotification(
        tenant,
        chargingStation,
        statusNotification
      );
    if (!ignoreStatusNotification) {
      ...
      await this.checkAndUpdateLastCompletedTransactionFromStatusNotification(
        tenant,
        chargingStation,
        statusNotification,
        connector
      ...
      if (!Utils.isEmptyArray(chargingStation?.connectors)) {
        chargingStation.connectors.sort(
      ...
      await this.processSmartChargingFromStatusNotification(
        tenant,
        chargingStation,
        connector,
        previousStatus
      ...
      await Logging.logInfo({
        ...
        tenantID: tenant.id,
        module: MODULE_NAME,
        method: 'processConnectorStatusNotification',
        message: `${Utils.buildConnectorInfo(
          statusNotification.connectorId,
          connector.currentTransactionID
        )} ${this.buildStatusNotification(statusNotification)} has been saved`,
        action: ServerAction.OCPP_STATUS_NOTIFICATION,
  ...
  private async processSmartChargingFromStatusNotification(
    tenant: Tenant,
    chargingStation: ChargingStation,
    connector: Connector,
    previousStatus: ChargePointStatus
  ): Promise<void> {
    // Trigger Smart Charging
    if (
      (connector.status === ChargePointStatus.CHARGING &&
        previousStatus !== ChargePointStatus.SUSPENDED_EVSE) ||
      ...
      try {
        ...
        await Logging.logError({
          ...
          tenantID: tenant.id,
          module: MODULE_NAME,
          method: 'processSmartChargingStatusNotification',
          action: ServerAction.OCPP_STATUS_NOTIFICATION,
          message: `${Utils.buildConnectorInfo(
  ...
  private async checkAndGetConnectorFromStatusNotification(
    tenant: Tenant,
    chargingStation: ChargingStation,
    statusNotification: OCPPStatusNotificationRequestExtended
  ): Promise<{ connector: Connector; ignoreStatusNotification: boolean }> {
    ...
    if (!connector) {
      ...
      Utils.objectAllPropertiesAreEqual(statusNotification, connector, [
        'status',
        'info',
        'errorCode',
        'vendorErrorCode',
      ...
      await Logging.logWarning({
        ...
        module: MODULE_NAME,
        method: 'handleStatusNotification',
        message: `${this.buildStatusNotification(
          statusNotification
        )} has not changed and will be ignored`,
  ...
  private async checkAndUpdateLastCompletedTransactionFromStatusNotification(
    tenant: Tenant,
    chargingStation: ChargingStation,
    statusNotification: OCPPStatusNotificationRequestExtended,
    connector: Connector
    ...
    if (
      ...
      if (lastTransaction) {
        try {
          ...
          OCPPUtils.clearChargingStationConnectorRuntimeData(
            chargingStation,
            lastTransaction.connectorId
          );
        } catch (error) {
          await Logging.logWarning({
            ...
            message: `${Utils.buildConnectorInfo(
              lastTransaction.connectorId,
              lastTransaction.id
            )} Error while trying to update the last Transaction`,
  ...
  private async checkAndComputeTransactionExtraInactivityFromStatusNotification(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    user: User,
    ...
    if (Utils.objectHasProperty(statusNotification, 'timestamp')) {
      ...
      if (!transaction.stop.extraInactivityComputed) {
        ...
        if (
          ...
          // Diff
          transaction.stop.extraInactivitySecs = Utils.createDecimal(
            currentStatusNotifTimestamp.getTime()
          )
            .minus(transactionStopTimestamp.getTime())
            .div(1000)
            .floor()
            .toNumber();
          // Negative inactivity
          if (transaction.stop.extraInactivitySecs < 0) {
            await Logging.logWarning({
              ...
              message: `${Utils.buildConnectorInfo(
                transaction.connectorId,
                transaction.id
              )} Extra Inactivity is negative and will be ignored: ${
          ...
          if (transaction.stop.extraInactivitySecs > 0) {
            ...
            await OCPPUtils.buildAndPriceExtraConsumptionInactivity(
              tenant,
              user,
              chargingStation,
              transaction
            ...
            await Logging.logInfo({
              ...
              message: `${Utils.buildConnectorInfo(
                transaction.connectorId,
                transaction.id
              )} Extra Inactivity of ${transaction.stop.extraInactivitySecs} secs has been added`,
          ...
          await Logging.logInfo({
            ...
            message: `${Utils.buildConnectorInfo(
              transaction.connectorId,
              transaction.id
            )} No Extra Inactivity for this transaction`,
            detailedMessages: { statusNotification, connector, lastTransaction: transaction },
          });
        }
        // Flag
  ...
  private async checkAndBillTransaction(
    tenant: Tenant,
    transaction: Transaction
  ): Promise<boolean> {
  ...
  private async notifyStatusNotification(
    tenant: Tenant,
    chargingStation: ChargingStation,
    connector: Connector,
    statusNotification: OCPPStatusNotificationRequestExtended
    ...
    if (
      ...
      await Logging.logError({
        ...
        action: ServerAction.OCPP_STATUS_NOTIFICATION,
        module: MODULE_NAME,
        method: 'notifyStatusNotification',
        message: `${Utils.buildConnectorInfo(
          connector.connectorId
        )} Error occurred: ${this.buildStatusNotification(statusNotification)}`,
      });
  ...
  private updateTransactionWithMeterValues(
    chargingStation: ChargingStation,
    transaction: Transaction,
      ...
      if (meterValue.attribute.measurand === OCPPMeasurand.POWER_ACTIVE_IMPORT) {
        ...
        if (meterValue.attribute.context === OCPPReadingContext.TRANSACTION_END) {
          ...
          const powerInMeterValueWatts =
            meterValue.attribute && meterValue.attribute.unit === OCPPUnitOfMeasure.KILO_WATT
              ? powerInMeterValue * 1000
              : powerInMeterValue;
          const currentType = Utils.getChargingStationCurrentType(
            chargingStation,
            null,
  ...
  private async checkNotificationEndOfCharge(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction
  ) {
    ...
    if (!transaction?.stop && transaction.currentTotalConsumptionWh > 0) {
      ...
      if (transaction.currentStateOfCharge > 0) {
        ...
        if (transaction.currentStateOfCharge === 100) {
          // Send Notification
          NotificationHelper.notifyEndOfCharge(
            tenant,
            transaction,
            chargingStation,
            transaction.user
          ...
          this.chargingStationConfig.notifBeforeEndOfChargePercent
        ) {
          // Send Notification
          NotificationHelper.notifyOptimalChargeReached(
            tenant,
            transaction,
            chargingStation,
            transaction.user
        ...
        if (
          ...
          const consumptions = await ConsumptionStorage.getTransactionConsumptions(
            tenant,
            { transactionId: transaction.id },
            { limit: 3, skip: 0, sort: { startedAt: -1 } }
          );
          if (consumptions.count === 3) {
            // Check the consumptions
            const noConsumption = consumptions.result.every(
              (consumption) =>
  ...
  private filterMeterValuesOnSpecificChargingStations(
    tenant: Tenant,
    chargingStation: ChargingStation,
    meterValues: OCPPNormalizedMeterValues
  ) {
    // Clean up Sample.Clock meter value
    if (
      chargingStation.chargePointVendor !== ChargerVendor.ABB ||
      ...
      meterValues.values = meterValues.values.filter(async (meterValue) => {
        ...
        if (
          ...
          await Logging.logWarning({
            ...
            tenantID: tenant.id,
            module: MODULE_NAME,
            method: 'filterMeterValuesOnSpecificChargingStations',
            action: ServerAction.OCPP_METER_VALUES,
  ...
  private normalizeMeterValues(
    ...
    for (const meterValue of meterValues.meterValue) {
      ...
      if (chargingStation.ocppVersion === OCPPVersion.VERSION_16) {
        ...
        for (const sampledValue of meterValue.sampledValue) {
          // Add Attributes
          const normalizedLocalMeterValue: OCPPNormalizedMeterValue =
            Utils.cloneObject(normalizedMeterValue);
  ...
  private buildMeterValueAttributes(sampledValue: OCPPSampledValue): OCPPAttribute {
    return {
      ...
      measurand: sampledValue.measurand
        ...
        : OCPPMeasurand.ENERGY_ACTIVE_IMPORT_REGISTER,
  ...
  private async processExistingTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    connectorId: number
  ) {
    ...
    do {
      ...
      activeTransaction = await TransactionStorage.getActiveTransaction(
        tenant,
        chargingStation.id,
        connectorId
      );
      ...
      if (activeTransaction) {
        ...
        if (activeTransaction.currentTotalConsumptionWh <= 0) {
          ...
          await Logging.logWarning({
            ...
            tenantID: tenant.id,
            module: MODULE_NAME,
            method: 'stopOrDeleteActiveTransactions',
            action: ServerAction.CLEANUP_TRANSACTION,
            actionOnUser: activeTransaction.user,
            message: `${Utils.buildConnectorInfo(
              activeTransaction.connectorId,
              activeTransaction.id
            )} Transaction with no consumption has been deleted`,
          });
          // Delete
          ...
          const result = await this.handleStopTransaction(
            ...
            },
            {
              ...
              meterStop: activeTransaction.lastConsumption
                ? activeTransaction.lastConsumption.value
                : activeTransaction.meterStart,
              timestamp: Utils.convertToDate(
                activeTransaction.lastConsumption
                  ? activeTransaction.lastConsumption.timestamp
          ...
          if (result.idTagInfo.status === OCPPAuthorizationStatus.INVALID) {
            ...
            await Logging.logError({
              ...
              message: `${Utils.buildConnectorInfo(
                activeTransaction.connectorId,
                activeTransaction.id
              )} Pending transaction cannot be stopped`,
            ...
            await Logging.logWarning({
              ...
              message: `${Utils.buildConnectorInfo(
                activeTransaction.connectorId,
                activeTransaction.id
              )}  Pending transaction has been stopped`,
              detailedMessages: { result },
            });
          }
        }
  ...
  private getStopTransactionTagId(
    stopTransaction: OCPPStopTransactionRequestExtended,
    transaction: Transaction
  ): string {
    // Stopped Remotely?
    if (transaction.remotestop) {
      ...
      const secs = moment
        .duration(moment().diff(moment(transaction.remotestop.timestamp)))
        .asSeconds();
      // In a minute
      if (secs < 60) {
  ...
  private async processTransactionCar(
    tenant: Tenant,
    transaction: Transaction,
    chargingStation: ChargingStation,
    ...
    switch (action) {
      case TransactionAction.START:
        ...
        if (Utils.isTenantComponentActive(tenant, TenantComponents.CAR) && user) {
          ...
          if (
            user.startTransactionData?.lastSelectedCar &&
            ...
            const defaultCar = await CarStorage.getDefaultUserCar(tenant, user.id, {}, [
              ...
              'carCatalogID',
              'vin',
              'carConnectorData.carConnectorID',
              'carConnectorData.carConnectorMeterID',
            ]);
            if (defaultCar) {
  ...
  private async getCurrentSoc(
    tenant: Tenant,
    transaction: Transaction,
    chargingStation: ChargingStation
  ): Promise<number> {
    if (
      Utils.isTenantComponentActive(tenant, TenantComponents.CAR_CONNECTOR) &&
      !Utils.isNullOrUndefined(transaction.car) &&
      !Utils.isNullOrUndefined(transaction.car.carConnectorData?.carConnectorID) &&
      Utils.getChargingStationCurrentType(chargingStation, null, transaction.connectorId) ===
        CurrentType.AC
    ) {
      const carImplementation = await CarConnectorFactory.getCarConnectorImpl(
        tenant,
        transaction.car.carConnectorData.carConnectorID
      );
  ...
  private enrichStartTransaction(
    tenant: Tenant,
    startTransaction: OCPPStartTransactionRequestExtended,
    chargingStation: ChargingStation
  ): void {
  ...
  private enrichBootNotification(
    headers: OCPPHeader,
    bootNotification: OCPPBootNotificationRequestExtended
  ): void {
    // Set the endpoint
  ...
  private async createChargingStationFromBootNotification(
    tenant: Tenant,
    bootNotification: OCPPBootNotificationRequestExtended,
    headers: OCPPHeader
    ...
    if (headers.token.siteAreaID) {
      const siteArea = await SiteAreaStorage.getSiteArea(tenant, headers.token.siteAreaID, {
        withSite: true,
      });
      if (siteArea) {
  ...
  private checkChargingStationIsIdenticalFromBootNotification(
    chargingStation: ChargingStation,
    bootNotification: OCPPBootNotificationRequestExtended
  ) {
    ...
    if (
      chargingStation.chargePointVendor !== bootNotification.chargePointVendor ||
      ...
      throw new BackendError({
        ...
        action: ServerAction.OCPP_BOOT_NOTIFICATION,
        module: MODULE_NAME,
        method: 'checkExistingChargingStation',
        message:
          'Boot Notification Rejected: Attribute mismatch: ' +
  ...
  private async enrichChargingStationFromBootNotification(
    tenant: Tenant,
    chargingStation: ChargingStation,
    headers: OCPPHeader,
    bootNotification: OCPPBootNotificationRequestExtended
  ...
  private async bypassStopTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    stopTransaction: OCPPStopTransactionRequestExtended
  ): Promise<boolean> {
  ...
  private async getTransactionFromMeterValues(
    tenant: Tenant,
    chargingStation: ChargingStation,
    headers: OCPPHeader,
    meterValues: OCPPMeterValuesRequest
  ): Promise<Transaction> {
    ...
    if (!meterValues.transactionId) {
      throw new BackendError({
        ...LoggingHelper.getChargingStationProperties(chargingStation),
        module: MODULE_NAME,
        method: 'getTransactionFromMeterValues',
        message: `${Utils.buildConnectorInfo(
          meterValues.connectorId
        )} Meter Values are not linked to a transaction and will be ignored`,
        action: ServerAction.OCPP_METER_VALUES,
        detailedMessages: { meterValues },
      });
    }
    const transaction = await TransactionStorage.getTransaction(tenant, meterValues.transactionId, {
      withUser: true,
      withTag: true,
      withCar: true,
    });
    ...
    if (transaction.transactionEndReceived) {
      await Logging.logWarning({
        ...
        message: `${Utils.buildConnectorInfo(
          meterValues.connectorId,
          meterValues.transactionId
        )} Meter Values received after the 'Transaction.End'`,
        detailedMessages: { meterValues },
      });
    }
    return transaction;
  ...
  private async abortOngoingTransactionInMeterValues(
    tenant: Tenant,
    chargingStation: ChargingStation,
    meterValues: OCPPMeterValuesRequest
  ) {
    ...
    if (!chargingStationClient) {
      await Logging.logError({
        ...
        tenantID: tenant.id,
        module: MODULE_NAME,
        method: 'abortOngoingTransactionInMeterValues',
        action: ServerAction.OCPP_METER_VALUES,
        message: `${Utils.buildConnectorInfo(
          meterValues.connectorId,
          meterValues.transactionId
        )} Charging Station is not connected to the backend, cannot send a Remote Stop Transaction on an ongoing Transaction`,
        detailedMessages: { meterValues },
      });
    } else {
      // Send Remote Stop
      ...
      if (result.status === OCPPRemoteStartStopStatus.ACCEPTED) {
        await Logging.logWarning({
          ...
          message: `${Utils.buildConnectorInfo(
            meterValues.connectorId,
            meterValues.transactionId
          )} Transaction has been automatically remotely stopped`,
          detailedMessages: { meterValues },
        });
      } else {
        await Logging.logError({
          ...
          message: `${Utils.buildConnectorInfo(
            meterValues.connectorId,
            meterValues.transactionId
          )} Cannot send a Remote Stop Transaction on an unknown ongoing Transaction`,
          detailedMessages: { meterValues },
        });
      }
    }
  ...
  private async getTransactionFromStopTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    headers: OCPPHeader,
    stopTransaction: OCPPStopTransactionRequestExtended
    ...
    const transaction = await TransactionStorage.getTransaction(
      ...
      { withUser: true, withTag: true }
    );
    if (!transaction) {
      throw new BackendError({
        ...LoggingHelper.getChargingStationProperties(chargingStation),
        module: MODULE_NAME,
        method: 'getTransactionFromStopTransaction',
  ...
  private checkSoftStopTransaction(
    transaction: Transaction,
    stopTransaction: OCPPStopTransactionRequestExtended,
    isSoftStop: boolean
  ) {
  ...
  private async checkAndApplyLastConsumptionInStopTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    transaction: Transaction,
    stopTransaction: OCPPStopTransactionRequestExtended,
    ...
    if (!transaction.transactionEndReceived) {
      ...
      const stopMeterValues = OCPPUtils.createTransactionStopMeterValues(
        chargingStation,
        transaction,
        stopTransaction
      );
      ...
      for (const consumption of consumptions) {
        ...
        if (consumption.toPrice) {
          // Price
          await PricingFacade.processStopTransaction(
            tenant,
            transaction,
            chargingStation,
            consumption,
      ...
      if (transaction.lastConsumption.value !== stopTransaction.meterStop) {
        await Logging.logWarning({
          ...
          actionOnUser: user,
          module: MODULE_NAME,
          method: 'checkAndApplyLastConsumptionInStopTransaction',
          message: `${Utils.buildConnectorInfo(
            transaction.connectorId,
            transaction.id
          )} Transaction.End consumption '${
            transaction.lastConsumption.value
          }' differs from Stop Transaction '${stopTransaction.meterStop}'`,
          detailedMessages: { stopTransaction, transaction },
        });
      }
      // Handle inactivity
      const inactivitySecs = Utils.createDecimal(
        new Date(stopTransaction.timestamp).getTime() -
          new Date(transaction.lastConsumption.timestamp).getTime()
      )
        .div(1000)
        .toNumber();
