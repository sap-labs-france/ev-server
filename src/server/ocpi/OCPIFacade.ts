...
export default class OCPIFacade {
  public static async processStartTransaction(
    ...
    if (
      !Utils.isTenantComponentActive(tenant, TenantComponents.OCPI) ||
      !chargingStation.issuer ||
      !chargingStation.public ||
    ...
    if (!transaction.authorizationID) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(transaction.connectorId, transaction.id)} Tag ID '${
          transaction.tagID
        }' is not authorized`,
      });
    }
    ...
    try {
      ...
      await Logging.logWarning({
        ...
        message: `${Utils.buildConnectorInfo(
          ...
          transaction.id
        )} Cannot process OCPI Update Transaction: ${error.message as string}`,
  ...
  public static async checkAndSendTransactionCdr(
    tenant: Tenant,
    transaction: Transaction,
    chargingStation: ChargingStation,
    siteArea: SiteArea,
  ...
  public static async updateConnectorStatus(
    tenant: Tenant,
    chargingStation: ChargingStation,
    connector: Connector
  ): Promise<void> {
    try {
      ...
      await Logging.logError({
        ...
        tenantID: tenant.id,
        module: MODULE_NAME,
        method: 'updateConnectorStatus',
        action: ServerAction.OCPI_CPO_UPDATE_STATUS,
        message: `${Utils.buildConnectorInfo(
          connector.connectorId
        )} An error occurred while patching the connector's Status`,
        detailedMessages: { error: error.stack, connector, chargingStation },
      });
    }
  }

  private static async checkAndGetOcpiCpoClient(
    tenant: Tenant,
    transaction: Transaction,
    user: User,
    action: ServerAction
  ): Promise<CpoOCPIClient> {
    // Check User
    if (!user) {
      throw new BackendError({
        ...LoggingHelper.getTransactionProperties(transaction),
        action,
        module: MODULE_NAME,
        method: 'checkAndGetOcpiCpoClient',
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} User is mandatory`,
    ...
    if (user.issuer) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} User does not belong to the local organization`,
      });
    }
    const ocpiClient = (await OCPIClientFactory.getAvailableOcpiClient(
      tenant,
    ...
    if (!ocpiClient) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} OCPI component requires at least one CPO endpoint to start a Transaction`,
      });
    }
    return ocpiClient;
  }
