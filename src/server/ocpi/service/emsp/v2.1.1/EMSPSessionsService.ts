...
export default class EMSPSessionsService {
  public static async handleGetSession(
    ...
    const transaction: Transaction = await TransactionStorage.getOCPITransactionBySessionID(
      tenant,
      sessionId
    );
    if (!transaction) {
  ...
  public static async handlePutSession(
    ...
    await OCPIUtilsService.processEmspTransactionFromSession(
      tenant,
      session,
      ServerAction.OCPI_EMSP_UPDATE_SESSION
    );
