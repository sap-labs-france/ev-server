...
export default class EMSPLocationsService {
  ...
  private static async putLocation(
    action: ServerAction,
    tenant: Tenant,
    locationID: string,
    ...
    site = await OCPIUtils.updateCreateSiteWithEmspLocation(
      tenant,
      location,
      company,
      site,
    ...
    // Process Site Area
    siteArea = await OCPIUtils.updateCreateSiteAreaWithEmspLocation(
      tenant,
      location,
      site,
      siteArea
    ...
    await OCPIUtils.updateCreateChargingStationsWithEmspLocation(
      ...
      siteArea,
      evses,
      ServerAction.OCPI_EMSP_UPDATE_LOCATION
    );
  }

  private static async putEvse(
    action: ServerAction,
    tenant: Tenant,
    locationID: string,
    evseUID: string,
  ...
  private static async putEvseConnector(
    action: ServerAction,
    tenant: Tenant,
    locationID: string,
    evseUID: string,
    ...
    evse.connectors = evse.connectors.filter((connector) => connector.id !== evseConnectorID);
    // Put the new one
  ...
  private static async patchLocation(
    action: ServerAction,
    tenant: Tenant,
    locationID: string,
    location: OCPILocation,
  ...
  private static async patchEvse(
    action: ServerAction,
    tenant: Tenant,
    locationID: string,
    evseUID: string,
  ...
  private static async patchEvseConnector(
    action: ServerAction,
    tenant: Tenant,
    locationID: string,
    evseUID: string,
  ...
  private static async getOrganizationsFromLocationID(
    action: ServerAction,
    tenant: Tenant,
    ocpiEndpoint: OCPIEndpoint,
    locationID: string,
    orgMustExist: boolean
  ): Promise<{ company: Company; site: Site; siteArea: SiteArea }> {
    // Get Company
    const company = await OCPIUtils.checkAndGetEmspCompany(tenant, ocpiEndpoint);
    if (orgMustExist && !company) {
      throw new AppError({
        module: MODULE_NAME,
        method: 'getOrganizationsFromLocationID',
        action,
        errorCode: StatusCodes.NOT_FOUND,
  ...
  private static async checkAndGetEvse(
    action: ServerAction,
    tenant: Tenant,
    locationID: string,
    evseUID: string,
