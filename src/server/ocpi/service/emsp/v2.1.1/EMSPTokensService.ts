...
export default class EMSPTokensService {
  public static async handleGetTokens(
    ...
    const limit =
      Utils.convertToInt(req.query.limit) > 0
  ...
  public static async handleAuthorizeToken(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (user.status !== UserStatus.ACTIVE) {
      res.json(
        OCPIUtils.success({
          allowed:
            tag.user.status === UserStatus.BLOCKED ? OCPIAllowed.BLOCKED : OCPIAllowed.NOT_ALLOWED,
          authorization_id: Utils.generateUUID(),
          location,
        })
      );
