...
export default class CPOCdrsService {
  public static async handleGetCdrs(
    ...
    // Get all cdrs
    const cdrs = await CPOCdrsService.getAllCdrs(
      tenant,
      limit,
      offset,
  ...
  private static async getAllCdrs(
    tenant: Tenant,
    limit: number,
    skip: number,
    dateFrom?: Date,
    dateTo?: Date
  ): Promise<DataResult<OCPICdr>> {
    ...
    return {
      ...
      result: transactions.result.map((transaction) => transaction.ocpiData?.cdr),
    };
  }
}
