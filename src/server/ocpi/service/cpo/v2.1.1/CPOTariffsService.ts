...
export default class CPOTariffsService {
  public static async handleGetTariffs(
    ...
    // Get all tariffs
    const tariffs = await CPOTariffsService.getAllTariffs(
      tenant,
      limit,
      offset,
      Utils.convertToDate(req.query.date_from),
      Utils.convertToDate(req.query.date_to)
    );
    if (tariffs.count === 0) {
      throw new AppError({
        module: MODULE_NAME,
        method: 'handleGetTariffs',
        action,
  ...
  private static async getAllTariffs(
    tenant: Tenant,
    limit: number,
    skip: number,
    dateFrom?: Date,
    dateTo?: Date
  ): Promise<DataResult<OCPITariff>> {
    // Result
    const tariffs: OCPITariff[] = [];
    let tariff: OCPITariff;
    if (tenant.components?.pricing?.active) {
      // Get simple pricing settings
      const pricingSettings = await SettingStorage.getPricingSettings(
        tenant,
        limit,
        skip,
        dateFrom,
  ...
  private static convertPricingSettings2ZeroFlatTariff(
