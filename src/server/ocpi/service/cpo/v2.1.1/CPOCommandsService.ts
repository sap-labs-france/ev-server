...
export default class CPOCommandsService {
  public static async handleCommand(
    ...
    switch (command) {
      case OCPICommandType.START_SESSION:
        res.json(
          await CPOCommandsService.remoteStartSession(action, req, res, next, tenant, ocpiEndpoint)
        );
        break;
      case OCPICommandType.STOP_SESSION:
        res.json(
          await CPOCommandsService.remoteStopSession(action, req, res, next, tenant, ocpiEndpoint)
        );
        break;
  ...
  private static async remoteStartSession(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction,
    ...
    let localToken = await TagStorage.getTag(tenant, startSession.token.uid, { withUser: true });
    // Create it on the fly
    if (!localToken) {
      // Check eMSP user
      const emspUser = await OCPIUtils.checkAndCreateEMSPUserFromToken(
        tenant,
        ocpiEndpoint.countryCode,
        ocpiEndpoint.partyId,
        startSession.token
      );
      localToken = await OCPIUtilsService.updateCreateTagWithEmspToken(
        tenant,
        startSession.token,
        localToken,
        emspUser,
    ...
    // Find the connector
    const connectorID = Utils.convertToInt(
      OCPIUtils.getConnectorIDFromEvseID(startSession.evse_uid)
    );
    ...
    void CPOCommandsService.remoteStartTransaction(
      action,
      tenant,
      chargingStation,
      connector,
  ...
  private static async remoteStopSession(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction,
    ...
    const transaction = await TransactionStorage.getOCPITransactionBySessionID(
      tenant,
    ...
    // Called Async as the response to the eMSP is sent asynchronously and this request has to finish before the command returns
    void CPOCommandsService.remoteStopTransaction(
      action,
      tenant,
      chargingStation,
      transaction.id,
  ...
  private static validateStopSession(stopSession: OCPIStopSession): boolean {
    if (!stopSession || !stopSession.response_url || !stopSession.session_id) {
      return false;
    }
    return true;
  }

  private static async remoteStartTransaction(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    connector: Connector,
    startSession: OCPIStartSession,
    ocpiEndpoint: OCPIEndpoint
  ): Promise<void> {
    try {
      ...
      if (result?.status === OCPPRemoteStartStopStatus.ACCEPTED) {
        await CPOCommandsService.sendCommandResponse(
          tenant,
          action,
          startSession.response_url,
          OCPICommandResponseType.ACCEPTED,
          ocpiEndpoint
        );
      } else {
        await CPOCommandsService.sendCommandResponse(
          ...
          action,
          startSession.response_url,
          OCPICommandResponseType.REJECTED,
          ocpiEndpoint
        );
      }
    } catch (error) {
  ...
  private static async remoteStopTransaction(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    transactionId: number,
    stopSession: OCPIStopSession,
    ocpiEndpoint: OCPIEndpoint
  ): Promise<void> {
    try {
      const chargingStationClient = await ChargingStationClientFactory.getChargingStationClient(
  ...
  private static async sendCommandResponse(
    tenant: Tenant,
    action: ServerAction,
    responseUrl: string,
    responseType: OCPICommandResponseType,
    ocpiEndpoint: OCPIEndpoint
  ) {
    // Build payload
