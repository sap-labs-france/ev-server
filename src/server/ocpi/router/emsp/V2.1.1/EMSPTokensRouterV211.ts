...
export default class EMSPTokensRouterV211 {
    ...
    this.router.post(
      ...
      async (req: Request, res: Response, next: NextFunction) => {
        await RouterUtils.handleOCPIServerAction(
          EMSPTokensService.handleAuthorizeToken.bind(this),
          ServerAction.OCPI_EMSP_AUTHORIZE_TOKEN,
          req,
          res,
          next
        );
  ...
  protected buildRouteGetTokens(): void {
    this.router.get(
      ...
      async (req: Request, res: Response, next: NextFunction) => {
        await RouterUtils.handleOCPIServerAction(
          EMSPTokensService.handleGetTokens.bind(this),
          ServerAction.OCPI_EMSP_GET_TOKENS,
          req,
          res,
          next
        );
