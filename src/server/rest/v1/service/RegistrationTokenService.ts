...
export default class RegistrationTokenService {
  public static async handleCreateRegistrationToken(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const filteredRequest =
      RegistrationTokenValidatorRest.getInstance().validateRegistrationTokenCreateReq(req.body);
    // Check the Site Area
    if (
    ...
    const registrationToken: RegistrationToken = {
      ...
      expirationDate: filteredRequest.expirationDate
        ? filteredRequest.expirationDate
        : moment().add(1, 'month').toDate(),
      createdBy: { id: req.user.id },
      createdOn: new Date(),
    };
    // Save
    registrationToken.id = await RegistrationTokenStorage.saveRegistrationToken(
      req.tenant,
      registrationToken
    );
    await Logging.logInfo({
      ...
      action,
      detailedMessages: { registrationToken },
    });
    res.json(Object.assign({ id: registrationToken.id }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }

  public static async handleUpdateRegistrationToken(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      RegistrationTokenValidatorRest.getInstance().validateRegistrationTokenUpdateReq(req.body);
    ...
    registrationToken.expirationDate = filteredRequest.expirationDate
      ? filteredRequest.expirationDate
      : moment().add(1, 'month').toDate();
    ...
    await Logging.logInfo({
      ...
      user: req.user,
      module: MODULE_NAME,
      method: 'handleUpdateRegistrationToken',
  ...
  public static async handleDeleteRegistrationToken(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const registrationTokenID =
      RegistrationTokenValidatorRest.getInstance().validateRegistrationTokenDeleteReq(req.query).ID;
    // Check and Get Registration Token
    const registrationToken = await UtilsService.checkAndGetRegistrationTokenAuthorization(
      req.tenant,
      req.user,
  ...
  public static async handleRevokeRegistrationToken(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const registrationTokenID =
      RegistrationTokenValidatorRest.getInstance().validateRegistrationTokenRevokeReq(req.query).ID;
    // Check and Get Registration Token
    const registrationToken = await UtilsService.checkAndGetRegistrationTokenAuthorization(
      req.tenant,
      req.user,
  ...
  public static async handleGetRegistrationTokens(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      RegistrationTokenValidatorRest.getInstance().validateRegistrationTokensGetReq(req.query);
    // Check dynamic auth
    ...
    // Get the tokens
    const registrationTokens = await RegistrationTokenStorage.getRegistrationTokens(
      req.tenant,
      {
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addRegistrationTokensAuthorizations(
        req.tenant,
        req.user,
        registrationTokens as RegistrationTokenDataResult,
        authorizations
      );
    }
  ...
  public static async handleGetRegistrationToken(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      RegistrationTokenValidatorRest.getInstance().validateRegistrationTokenGetReq(req.query);
    // Check and Get Registration Token
    const registrationToken = await UtilsService.checkAndGetRegistrationTokenAuthorization(
      req.tenant,
      req.user,
