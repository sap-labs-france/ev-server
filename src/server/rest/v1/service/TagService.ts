...
export default class TagService {
  public static async handleGetTag(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleGetTags(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleDeleteTags(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleUnassignTags(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = TagValidatorRest.getInstance().validateTagsByVisualIDsUnassignReq(
      req.body
    );
    // Delete
    const result = await TagService.unassignTags(
      req.tenant,
      action,
      req.user,
      filteredRequest.visualIDs
    );
    res.json({ ...result, ...Constants.REST_RESPONSE_SUCCESS });
    next();
  }

  public static async handleUnassignTag(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = TagValidatorRest.getInstance().validateTagByVisualIDUnassignReq(
      req.body
    );
    // Delete
    const response = await TagService.unassignTags(req.tenant, action, req.user, [
      filteredRequest.visualID,
    ]);
    if (response.inSuccess === 0) {
      throw new AppError({
        action: ServerAction.TAG_UNASSIGN,
        module: MODULE_NAME,
        method: 'handleUnassignTag',
  ...
  public static async handleGetTagByVisualID(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleDeleteTag(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Delete
    const response = await TagService.deleteTags(req.tenant, action, req.user, [
      filteredRequest.ID,
    ]);
    if (response.inSuccess === 0) {
      throw new AppError({
  ...
  public static async handleCreateTag(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    res
      ...
      .json(Object.assign({ id: newTag.id }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }

  public static async handleAssignTag(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const tag = await UtilsService.checkAndGetTagByVisualIDAuthorization(
      ...
      action,
      filteredRequest,
      { withNbrTransactions: true, withUser: true }
    );
    // Check Tag with Visual ID
    if (!tag) {
      throw new AppError({
    ...
    res
      .status(StatusCodes.CREATED)
      .json(Object.assign({ id: tag.id }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }

  public static async handleUpdateTagByVisualID(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    await Logging.logInfo({
      ...
      action: action,
      module: MODULE_NAME,
      method: 'handleUpdateTagByVisualID',
  ...
  public static async handleUpdateTag(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = TagValidatorRest.getInstance().validateTagUpdateReq({
      ...req.params,
      ...req.body,
    });
    // Check and Get Tag
  ...
  public static async handleImportTags(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
            ...
            void converter.subscribe(
              async (tag: ImportedTag) => {
                ...
                if (!result.inSuccess && !result.inError) {
                  ...
                  if (
                    !TagRequiredImportProperties.every((property) => tagKeys.includes(property))
                  ) {
                    if (!res.headersSent) {
                    ...
                    reject(
                      new Error(
                        `Missing one of required properties: '${TagRequiredImportProperties.join(
                          ', '
                        )}'`
                      )
                    );
                ...
                tag.importedData = {
                  ...
                  autoActivateTagAtImport: UtilsSecurity.filterBoolean(
                    req.headers.autoactivatetagatimport
                  ),
                };
                // Import
                const importSuccess = await TagService.processTag(
                  action,
                  req,
                  tag,
                  tagsToBeImported
                ...
                // Insert batched
                if (
                  !Utils.isEmptyArray(tagsToBeImported) &&
                  tagsToBeImported.length % Constants.IMPORT_BATCH_INSERT_SIZE === 0
                ) {
                  await TagService.insertTags(
                    req.tenant,
                    req.user,
                    action,
                    tagsToBeImported,
              ...
              async () => {
                ...
                await Logging.logActionsResponse(
                  ...
                  `No Tag have been uploaded in ${executionDurationSecs}s`,
                  req.user
                );
  ...
  public static async handleExportTags(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  private static async insertTags(
    tenant: Tenant,
    user: UserToken,
    action: ServerAction,
    tagsToBeImported: ImportedTag[],
  ...
  private static async deleteTags(
    tenant: Tenant,
    action: ServerAction,
  ...
  private static async unassignTags(
    tenant: Tenant,
    action: ServerAction,
    loggedUser: UserToken,
    visualIDs: string[]
  ...
  private static convertToCSV(req: Request, tags: Tag[], writeHeader = true): string {
    ...
    // Header
    if (writeHeader) {
      headers = ['id', 'visualID', 'description', 'firstName', 'name', 'email'].join(
  ...
  private static async getTags(
    req: Request,
    filteredRequest: HttpTagsGetRequest
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addTagsAuthorizations(
        req.tenant,
        req.user,
        tags as TagDataResult,
        authorizations
      );
    }
    return tags;
  ...
  private static async processTag(
    action: ServerAction,
    req: Request,
    ...
    try {
      const newImportedTag: ImportedTag = {
        ...
        description: importedTag.description
          ? importedTag.description
          : `Tag ID '${importedTag.id}'`,
  ...
  private static async checkAndDeleteTagRoaming(
    tenant: Tenant,
    loggedUser: UserToken,
    tag: Tag
  ): Promise<void> {
  ...
  private static async updateTagRoaming(
    action: ServerAction,
    tenant: Tenant,
    loggedUser: UserToken,
    tag: Tag
    ...
    if (Utils.isComponentActiveFromToken(loggedUser, TenantComponents.OCPI)) {
      try {
        ...
        if (ocpiClient) {
          await ocpiClient.pushToken(OCPIUtils.buildEmspTokenFromTag(tenant, tag));
        }
      } catch (error) {
        await Logging.logError({
          tenantID: tenant.id,
