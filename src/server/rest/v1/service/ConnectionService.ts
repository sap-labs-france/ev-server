...
export default class ConnectionService {
  public static async handleGetConnection(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const connectionID = ConnectionValidatorRest.getInstance().validateConnectionGetReq(
      req.query
    ).ID;
    ...
    UtilsService.assertObjectExists(
      action,
      connection,
      `Connection ID '${connectionID}' does not exist`,
      MODULE_NAME,
      'handleGetConnection',
      req.user
    );
  ...
  public static async handleGetConnections(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = ConnectionValidatorRest.getInstance().validateConnectionsGetReq(
      req.query
    );
    // Check Users
    let userProject: string[] = [];
    ...
    // Get
    const connections = await ConnectionStorage.getConnectionsByUserId(
      req.tenant,
  ...
  public static async handleCreateConnection(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = ConnectionValidatorRest.getInstance().validateConnectionCreateReq(
      req.body
    );
    let integrationConnector = null;
    switch (filteredRequest.connectorId) {
      case ConnectionType.MERCEDES:
        integrationConnector = await CarConnectorFactory.getCarConnectorImpl(
          req.tenant,
    ...
    if (!Utils.isNullOrUndefined(integrationConnector)) {
      // Create
      const connection: Connection = await integrationConnector.createConnection(
        filteredRequest.userId,
    ...
    res
      ...
      .json(Object.assign({ id: req.tenant.id }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }

  public static async handleDeleteConnection(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const connectionID = ConnectionValidatorRest.getInstance().validateConnectionDeleteReq(
      req.query
    ).ID;
    if (!connectionID) {
      throw new AppError({
