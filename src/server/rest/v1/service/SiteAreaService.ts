...
export default class SiteAreaService {
  public static async handleAssignAssetsToSiteArea(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      req.user,
      TenantComponents.ASSET,
      Action.UPDATE,
      Entity.ASSET,
      MODULE_NAME,
      'handleAssignAssetsToSiteArea'
    );
    // Filter request
    const filteredRequest = SiteAreaValidatorRest.getInstance().validateSiteAreaAssignAssetsReq(
      req.body
    );
    // Check and Get Site Area
    const authAction =
      action === ServerAction.ADD_ASSET_TO_SITE_AREA
  ...
  public static async handleAssignChargingStationsToSiteArea(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      MODULE_NAME,
      'handleAssignChargingStationsToSiteArea'
    );
    // Filter request
    const filteredRequest =
      SiteAreaValidatorRest.getInstance().validateSiteAreaAssignChargingStationsReq(req.body);
    // Check and Get Site Area
    const authAction =
    ...
    const chargingStations = await UtilsService.checkSiteAreaChargingStationsAuthorization(
      ...
      req.user,
      siteArea,
      filteredRequest.chargingStationIDs,
      action
    );
    ...
    if (action === ServerAction.ADD_CHARGING_STATIONS_TO_SITE_AREA) {
      await SiteAreaStorage.addChargingStationsToSiteArea(
        ...
        siteArea.siteID,
        siteArea.site.companyID,
        chargingStations.map((chargingStation) => chargingStation.id)
      );
    } else {
  ...
  public static async handleDeleteSiteArea(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleGetSiteArea(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleGetSiteAreaImage(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = SiteAreaValidatorRest.getInstance().validateSiteAreaGetImageReq(
      req.query
    );
    // Get it
    const siteAreaImage = await SiteAreaStorage.getSiteAreaImage(req.tenant, filteredRequest.ID);
  ...
  public static async handleGetSiteAreas(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const siteAreas = await SiteAreaStorage.getSiteAreas(
      req.tenant,
      {
        ...
        excludeSiteAreaIDs: filteredRequest.ExcludeSiteAreaID
          ? filteredRequest.ExcludeSiteAreaID.split('|')
          : null,
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addSiteAreasAuthorizations(
        req.tenant,
        req.user,
        siteAreas as SiteAreaDataResult,
        authorizations
      );
    }
    // Handle smart charging session parameters
    await SiteAreaService.addSmartChargingSessionParametersActive(
      req.tenant,
      req.user,
      siteAreas as SiteAreaDataResult
    );
  ...
  public static async handleGetSiteAreaConsumption(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.SITE_AREA,
      MODULE_NAME,
      'handleGetSiteAreaConsumption'
    );
    // Filter request
    const filteredRequest = SiteAreaValidatorRest.getInstance().validateSiteAreaGetConsumptionReq(
      req.query
    );
    // Check dates
    if (
      filteredRequest.StartDate &&
    ...
    const rootSiteArea = await SiteAreaService.buildSiteAreaTree(
      req.tenant,
      siteArea,
      parentSiteArea,
      [siteArea.siteID]
  ...
  public static async handleCreateSiteArea(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (filteredRequest.parentSiteAreaID) {
      parentSiteArea = await UtilsService.checkAndGetSiteAreaAuthorization(
        req.tenant,
        req.user,
        filteredRequest.parentSiteAreaID,
        Action.UPDATE,
        action
      );
    }
    ...
    const subSiteAreasActions = SiteAreaService.buildSubSiteAreaActions(
      ...
      req.user,
      siteArea,
      filteredRequest.subSiteAreasAction
    );
    ...
    // Save
    siteArea.id = await SiteAreaStorage.saveSiteArea(
      req.tenant,
      siteArea,
  ...
  public static async handleUpdateSiteArea(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    SiteAreaService.checkChargingStationNumberOfPhases(
      action,
      req.user,
      siteArea.chargingStations,
      filteredRequest.numberOfPhases
    );
    // Keep Parent ID
    ...
    await SiteAreaService.processSubSiteAreaActions(
      ...
      parentSiteArea,
      subSiteAreasActions,
      formerParentSiteAreaID
    );
    // Save
    await SiteAreaStorage.saveSiteArea(
      req.tenant,
      siteArea,
      Utils.objectHasProperty(filteredRequest, 'image')
    );
    ...
    if (!siteArea.smartCharging) {
      void OCPPUtils.clearAndDeleteChargingProfilesForSiteArea(req.tenant, siteArea, {
  ...
  public static buildSubSiteAreaActions(
    action: ServerAction,
    user: UserToken,
    siteArea,
    ...
    // Split
    const subSiteAreasActions = subSiteAreasAction
      ? (subSiteAreasAction?.split('|') as SubSiteAreaAction[])
      : [];
    ...
    if (subSiteAreasActions?.length > 1) {
      throw new AppError({
        ...
        action,
        module: MODULE_NAME,
        method: 'buildSubSiteAreaActionsFromHttpRequest',
      });
    }
    return subSiteAreasActions;
  }

  private static async checkAndGetParentSiteArea(
    tenant: Tenant,
    user: UserToken,
    siteArea: SiteArea,
    ...
    if (parentSiteAreaID) {
      ...
      if (siteArea.id === parentSiteArea.id) {
        throw new AppError({
          ...
          module: MODULE_NAME,
          method: 'checkAndGetParentSiteArea',
  ...
  private static checkChargingStationNumberOfPhases(
    action: ServerAction,
    user: UserToken,
    chargingStations: ChargingStation[],
    numberOfPhasesToUpdate: number
  ) {
        ...
        for (const connector of chargingStation.connectors) {
          ...
          if (numberOfPhases !== 1) {
            throw new AppError({
              ...
              action,
              module: MODULE_NAME,
              method: 'checkChargingStationNumberOfPhases',
              user: user,
            });
          }
        }
  ...
  private static async triggerSmartCharging(
    tenant: Tenant,
    action: ServerAction,
    siteArea: SiteArea
  ) {
  ...
  private static async addSmartChargingSessionParametersActive(
    tenant: Tenant,
    user: UserToken,
  ...
  private static async processSubSiteAreaActions(
    tenant: Tenant,
    rootSiteArea: SiteArea,
    siteArea: SiteArea,
    parentSiteArea: SiteArea,
    subSiteAreasActions: SubSiteAreaAction[],
    formerParentSiteAreaID?: string
  ) {
      ...
      for (const subSiteAreasAction of subSiteAreasActions) {
        switch (subSiteAreasAction) {
          ...
          case SubSiteAreaAction.CLEAR:
            await SiteAreaStorage.attachSiteAreaChildrenToNewParent(tenant, siteArea.id, null);
            break;
  ...
  private static async updateSiteAreaChildrenWithSmartCharging(
    ...
    for (const siteArea of siteAreas) {
      ...
      if (siteArea.id !== currentSiteAreaID) {
        ...
        // Clear Charging Profiles (async)
        if (!smartCharging) {
          void OCPPUtils.clearAndDeleteChargingProfilesForSiteArea(tenant, siteArea, {
            profilePurposeType: ChargingProfilePurposeType.TX_PROFILE,
          });
        }
      }
  ...
  private static async checkAndGetSiteAreaTree(
    tenant: Tenant,
    siteArea: SiteArea,
    parentSiteArea: SiteArea,
    siteIDs: string[],
    subSiteAreaActions?: SubSiteAreaAction[]
  ): Promise<SiteArea> {
  ...
  private static checkIfSiteAreaTreeIsConsistent(
    siteArea: SiteArea,
    subSiteAreaActions: SubSiteAreaAction[] = []
  ): void {
    ...
    for (const childSiteArea of siteArea.childSiteAreas) {
      ...
      const actionOnSite = subSiteAreaActions.filter((subSiteAreaAction) =>
        [SubSiteAreaAction.ATTACH, SubSiteAreaAction.CLEAR, SubSiteAreaAction.UPDATE].includes(
          subSiteAreaAction
        )
      );
      if (Utils.isEmptyArray(actionOnSite) && siteArea.siteID !== childSiteArea.siteID) {
        throw new AppError({
          ...LoggingHelper.getSiteAreaProperties(siteArea),
          ...
          module: MODULE_NAME,
          method: 'checkIfSiteAreaTreeIsConsistent',
          detailedMessages: { siteArea, childSiteArea },
        });
      }
      const actionOnSmartCharging = subSiteAreaActions.filter(
        (subSiteAreaAction) => SubSiteAreaAction.FORCE_SMART_CHARGING === subSiteAreaAction
      );
      if (
        Utils.isEmptyArray(actionOnSmartCharging) &&
  ...
  private static async buildSiteAreaTree(
    tenant: Tenant,
    siteArea: SiteArea,
    parentSiteArea: SiteArea,
    siteIDs: string[]
    ...
    const allSiteAreasOfSite = await SiteAreaStorage.getSiteAreas(
      tenant,
      { siteIDs, excludeSiteAreaIDs: siteArea.id ? [siteArea.id] : [], withSite: true },
