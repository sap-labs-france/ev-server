...
export default class BillingService {
  public static async handleClearBillingTestData(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Action.CLEAR_BILLING_TEST_DATA,
      Entity.BILLING,
      MODULE_NAME,
      'handleClearBillingTestData'
    );
    // Check dynamic auth
    await AuthorizationService.checkAndGetBillingAuthorizations(
      req.tenant,
      req.user,
      Action.CLEAR_BILLING_TEST_DATA
    );
    ...
    if (!billingImpl) {
      throw new AppError({
        ...
        module: MODULE_NAME,
        method: 'handleClearBillingTestData',
        action: action,
        user: req.user,
      });
    }
  ...
  public static async handleCheckBillingConnection(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (!billingImpl) {
      throw new AppError({
        ...
        message: 'Billing service is not configured',
        module: MODULE_NAME,
        method: 'handleCheckBillingConnection',
        action: action,
        user: req.user,
      });
    }
  ...
  public static async handleForceSynchronizeUser(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.USER,
      MODULE_NAME,
      'handleForceSynchronizeUser'
    );
    // Filter
    const filteredRequest = BillingSecurity.filterSynchronizeUserRequest(req.body);
    // Check dynamic authorization
    await AuthorizationService.checkAndGetUserAuthorizations(
      req.tenant,
      req.user,
      {},
      Action.SYNCHRONIZE_BILLING_USER
    ...
    if (!billingImpl) {
      throw new AppError({
        ...
        message: 'Billing service is not configured',
        module: MODULE_NAME,
        method: 'handleForceSynchronizeUser',
        action: action,
        user: req.user,
      });
    }
    ...
    UtilsService.assertObjectExists(
      action,
      user,
      `User ID '${filteredRequest.id}' does not exist`,
      MODULE_NAME,
  ...
  public static async handleGetBillingTaxes(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleGetInvoices(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingInvoicesGetReq(
      req.query
    );
    // Check dynamic authorization
    ...
    const invoices = await BillingStorage.getInvoices(
      req.tenant,
      {
        ...
        invoiceStatus: filteredRequest.Status
          ? (filteredRequest.Status.split('|') as BillingInvoiceStatus[])
          : null,
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addInvoicesAuthorizations(
        req.tenant,
        req.user,
        invoices as BillingInvoiceDataResult,
        authorizations
      );
    }
    res.json(invoices);
  ...
  public static async handleGetInvoice(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingInvoiceGetReq(
      req.query
    );
    // Check and get invoice
    const invoice = await UtilsService.checkAndGetInvoiceAuthorization(
      req.tenant,
      req.user,
      filteredRequest.ID,
      Action.READ,
  ...
  public static async handleBillingSetupPaymentMethod(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      req.user,
      TenantComponents.BILLING,
      Action.BILLING_SETUP_PAYMENT_METHOD,
      Entity.BILLING,
      MODULE_NAME,
      'handleSetupSetupPaymentMethod'
    );
    // Filter
    const filteredRequest =
      BillingValidatorRest.getInstance().validateBillingSetupUserPaymentMethodReq(req.body);
    // Dynamic auth
    await AuthorizationService.checkAndGetPaymentMethodAuthorizations(
      req.tenant,
      req.user,
      filteredRequest,
      Action.CREATE
    ...
    const user: User = await UtilsService.checkAndGetUserAuthorization(
      req.tenant,
      req.user,
    ...
    const operationResult: BillingOperationResult = await billingImpl.setupPaymentMethod(
      user,
      paymentMethodId
    );
    if (operationResult) {
  ...
  public static async handleBillingGetPaymentMethods(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      BillingValidatorRest.getInstance().validateBillingGetUserPaymentMethodsReq(req.query);
    // Check if component is active
    UtilsService.assertComponentIsActiveFromToken(
      req.user,
      TenantComponents.BILLING,
      Action.BILLING_PAYMENT_METHODS,
      Entity.BILLING,
      MODULE_NAME,
    ...
    if (!billingImpl) {
      throw new AppError({
        ...
        message: 'Billing service is not configured',
        module: MODULE_NAME,
        method: 'handleBillingGetPaymentMethods',
        action: action,
        user: req.user,
      });
    }
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addPaymentMethodsAuthorizations(
        req.tenant,
        req.user,
        dataResult,
        authorizations,
  ...
  public static async handleBillingDeletePaymentMethod(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      BillingValidatorRest.getInstance().validateBillingDeleteUserPaymentMethodReq(req.body);
    // Check if component is active
    UtilsService.assertComponentIsActiveFromToken(
      req.user,
      TenantComponents.BILLING,
      ...
      Entity.BILLING,
      MODULE_NAME,
      'handleBillingDeletePaymentMethod'
    );
    // Dynamic auth
    await AuthorizationService.checkAndGetPaymentMethodAuthorizations(
      req.tenant,
    ...
    UtilsService.assertObjectExists(
      action,
      user,
      `User ID '${filteredRequest.userID}' does not exist`,
      MODULE_NAME,
      'handleBillingDeletePaymentMethod',
      req.user
    );
    // Invoke the billing implementation
    const operationResult: BillingOperationResult = await billingImpl.deletePaymentMethod(
      user,
      filteredRequest.paymentMethodId
    );
    // Log
    await Logging.logInfo({
      tenantID: req.tenant.id,
  ...
  public static async handleDownloadInvoice(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertObjectExists(
      action,
      billingInvoice,
      `Invoice ID '${filteredRequest.ID}' does not exist`,
      MODULE_NAME,
      'handleDownloadInvoice',
      req.user
    );
    // Check and get authorizations
    await UtilsService.checkAndGetInvoiceAuthorization(
      req.tenant,
      req.user,
      filteredRequest.ID,
      Action.DOWNLOAD,
  ...
  public static async handleTransferInvoiceDownload(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      MODULE_NAME,
      'handleTransferInvoiceDownload'
    );
    // Filter
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingTransferGetReq(
      req.query
    );
    // Get the Invoice
    ...
    UtilsService.assertObjectExists(
      action,
      billingTransfer,
      `Transfer ID '${filteredRequest.ID}' does not exist`,
      MODULE_NAME,
      'handleTransferInvoiceDownload',
      req.user
    );
    // Check and get authorizations
    await UtilsService.checkAndGetTransferAuthorization(
      req.tenant,
      req.user,
      filteredRequest.ID,
      Action.DOWNLOAD,
    ...
    if (!billingImpl) {
      throw new AppError({
        ...
        message: 'Billing service is not configured',
        module: MODULE_NAME,
        method: 'handleTransferInvoiceDownload',
        action: action,
        user: req.user,
      });
    }
  ...
  public static async handleGetBillingSetting(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.SETTING,
      MODULE_NAME,
      'handleGetBillingSetting'
    );
    const billingSettings: BillingSettings =
      await UtilsService.checkAndGetBillingSettingAuthorization(
        req.tenant,
        req.user,
        null,
        Action.READ,
  ...
  public static async handleUpdateBillingSetting(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.SETTING,
      MODULE_NAME,
      'handleUpdateBillingSetting'
    );
    const newBillingProperties = BillingValidatorRest.getInstance().validateBillingSettingUpdateReq(
      { ...req.params, ...req.body }
    );
    ...
    const { immediateBillingAllowed, periodicBillingAllowed, taxID, platformFeeTaxID } =
      newBillingProperties.billing;
  ...
  public static async handleOnboardAccount(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      req.user,
      TenantComponents.BILLING_PLATFORM,
      Action.BILLING_ONBOARD_ACCOUNT,
      Entity.BILLING_ACCOUNT,
      MODULE_NAME,
      'handleOnboardAccount'
    );
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingAccountGetReq(
      req.params
    );
    // Check authorization
    await AuthorizationService.checkAndGetBillingAccountAuthorizations(
      req.tenant,
      req.user,
      filteredRequest,
      Action.BILLING_ONBOARD_ACCOUNT
    ...
    UtilsService.assertObjectExists(
      action,
      billingAccount,
      `Account ID '${filteredRequest.ID}' does not exist`,
      MODULE_NAME,
      'handleOnboardAccount',
      req.user
    );
    ...
    UtilsService.assertObjectExists(
      action,
      user,
      `User ID '${billingAccount.businessOwnerID}' does not exist`,
      MODULE_NAME,
      'handleOnboardAccount',
      req.user
    );
    ...
    NotificationHandler.sendBillingAccountCreationLink(
      req.tenant,
      Utils.generateUUID(),
      user,
      notificationData
  ...
  public static async handleCreateAccount(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingCreateAccountReq(
      req.body
    );
    // Check authorization
    await AuthorizationService.checkAndGetBillingAccountAuthorizations(
    ...
    UtilsService.assertObjectExists(
      action,
      user,
      `User ID '${filteredRequest.businessOwnerID}' does not exist`,
      MODULE_NAME,
      'handleCreateAccount',
      req.user
    );
  ...
  public static async handleRefreshAccount(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (!req.tenant) {
      throw new AppError({
        ...
        module: MODULE_NAME,
        method: 'handleRefreshAccount',
        action: action,
      });
    }
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingActivateAccountReq({
      ...req.params,
      ...req.body,
    });
    let billingAccount = await BillingStorage.getAccountByID(req.tenant, filteredRequest.ID);
    ...
    const refreshedAccount = await billingImpl.refreshConnectedAccount(
      billingAccount,
      onboardingURL
    );
    // Update account properties
  ...
  public static async handleActivateAccount(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    NotificationHandler.sendBillingAccountActivationNotification(
      ...
      Utils.generateUUID(),
      user,
      { evseDashboardURL: Utils.buildEvseURL(req.tenant.subdomain), user }
    ).catch((error) => {
      Logging.logPromiseError(error, req?.tenant?.id);
    });
    res.json(Object.assign({ id: billingAccount.id }, Constants.REST_RESPONSE_SUCCESS));
  ...
  public static async handleBillingGetAccounts(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingAccountsGetReq(
      req.query
    );
    // Check auth
    const authorizations = await AuthorizationService.checkAndGetBillingAccountsAuthorizations(
      req.tenant,
      req.user,
    ...
    const billingAccounts = await BillingStorage.getAccounts(
      req.tenant,
      {
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addAccountsAuthorizations(
        req.tenant,
        req.user,
        billingAccounts,
        authorizations
  ...
  public static async handleBillingGetAccount(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.BILLING_ACCOUNT,
      MODULE_NAME,
      'handleBillingGetAccount'
    );
    // Get the billing accounts
    const billingAccount = await UtilsService.checkAndGetBillingAccountAuthorization(
      req.tenant,
      req.user,
      filteredRequest.ID,
      Action.READ,
  ...
  public static async handleBillingGetTransfers(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingTransfersGetReq(
      req.query
    );
    // Check auth
    const authorizations = await AuthorizationService.checkAndGetTransfersAuthorizations(
      req.tenant,
      req.user,
      Action.LIST,
      filteredRequest /* , false */
    ...
    const transfers = await BillingStorage.getTransfers(
      req.tenant,
      {
        ...
        transferExternalIDs: filteredRequest.TransferExternalID
          ? filteredRequest.TransferExternalID.split('|')
          : null,
        search: filteredRequest.Search ? filteredRequest.Search : null,
        status: filteredRequest.Status ? filteredRequest.Status.split('|') : null,
      },
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addTransfersAuthorizations(
        req.tenant,
        req.user,
        transfers,
        authorizations
  ...
  public static async handleGetTransfer(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const transfer = await UtilsService.checkAndGetTransferAuthorization(
      req.tenant,
      req.user,
      filteredRequest.ID,
      Action.READ,
  ...
  public static async handleFinalizeTransfer(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      req.user,
      TenantComponents.BILLING_PLATFORM,
      Action.BILLING_FINALIZE_TRANSFER,
      Entity.BILLING_TRANSFER,
      MODULE_NAME,
      'handleFinalizeTransfer'
    );
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingTransferFinalizeReq(
      req.params
    );
    // Check authorization
    await AuthorizationService.checkAndGetTransfersAuthorizations(
      req.tenant,
      req.user,
      Action.BILLING_FINALIZE_TRANSFER
    );
    ...
    UtilsService.assertObjectExists(
      action,
      billingAccount,
      `Account ID '${transfer.accountID}' does not exist`,
      MODULE_NAME,
      'handleSendTransferInvoice',
      req.user
    );
    // Get the account owner
    const user = await UserStorage.getUser(req.tenant, billingAccount.businessOwnerID);
    ...
    // Funds to transfer
    const transferAmount = Utils.createDecimal(transfer.collectedFunds)
      .minus(invoice.totalAmount)
      .toNumber();
  ...
  public static async handleSendTransfer(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      req.user,
      TenantComponents.BILLING_PLATFORM,
      Action.BILLING_SEND_TRANSFER,
      Entity.BILLING_TRANSFER,
      MODULE_NAME,
      'handleSendTransferInvoice'
    );
    const filteredRequest = BillingValidatorRest.getInstance().validateBillingTransferSendReq(
      req.params
    );
    // Check authorization
    await AuthorizationService.checkAndGetTransfersAuthorizations(
  ...
  private static async checkActivationPrerequisites(
    action: ServerAction,
    req: Request
  ): Promise<void> {
    const billingImpl = await BillingFactory.getBillingImpl(req.tenant);
    if (!billingImpl) {
      throw new AppError({
        ...
        message: 'Billing service is not configured',
        module: MODULE_NAME,
        method: 'checkActivationPrerequisites',
        action: action,
        user: req.user,
      });
    }
