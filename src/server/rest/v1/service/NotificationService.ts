...
export default class NotificationService {
  public static async handleGetNotifications(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest = NotificationValidatorRest.getInstance().validateNotificationsGetReq(
      req.query
    );
    // Check User
  ...
  public static async handleEndUserReportError(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (!(await Authorizations.canEndUserReportError(req.user)).authorized) {
      throw new AppAuthError({
        ...
        module: MODULE_NAME,
        method: 'handleEndUserReportError',
      });
    }
    // Filter
    const filteredRequest = NotificationValidatorRest.getInstance().validateEndUserErrorReportReq(
      req.body
    );
    // Check and Get User
    ...
    // Notify
    NotificationHandler.sendEndUserErrorNotification(req.tenant, endUserErrorNotification).catch(
      (error) => {
