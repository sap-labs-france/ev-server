...
import {
  OCPICommandResponse,
  OCPICommandResponseType,
} from '../../../../types/ocpi/OCPICommandResponse';
...
import {
  HttpChargingStationCompositeScheduleGetRequest,
...
export default class ChargingStationService {
  public static async handleUpdateChargingStationParams(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationParametersUpdateReq({
        ...req.params,
        ...req.body,
      });
    ...
    // Update props
    ChargingStationService.updateChargingStationCommonProperties(
      action,
      req.tenant,
      chargingStation,
      siteArea,
    ...
    const resetAndApplyTemplate =
      await ChargingStationService.updateChargingStationManualConfiguration(
        action,
        chargingStation,
        req.user,
        filteredRequest
    ...
    // Existing Connectors
    ChargingStationService.updateChargingStationConnectors(
      chargingStation,
      req.user,
      filteredRequest
    );
    // Handle Manual Config
    ChargingStationService.updateChargingStationManualAutoConfig(
      chargingStation,
      req.user,
      filteredRequest
    );
    // Update Site Area
    ChargingStationService.updateChargingStationSiteArea(
      action,
      chargingStation,
      req.user,
      siteArea
  ...
  public static async handleChargingStationLimitPower(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationLimitPowerReq(req.body);
    // Check dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(
      req.tenant,
      req.user,
    ...
    UtilsService.assertObjectExists(
      action,
      chargePoint,
      `Charge Point ID '${filteredRequest.chargePointID}' does not exist.`,
      MODULE_NAME,
    ...
    if (
      filteredRequest.ampLimitValue <
      StaticLimitAmps.MIN_LIMIT_PER_PHASE * numberOfPhases * chargePoint.connectorIDs.length
    ) {
      throw new AppError({
        ...
        message: `Limitation to ${filteredRequest.ampLimitValue}A is too low, min required is ${
          StaticLimitAmps.MIN_LIMIT_PER_PHASE * numberOfPhases * chargePoint.connectorIDs.length
        }A`,
      });
    }
    // Get the Vendor instance
    const chargingStationVendor =
      ChargingStationVendorFactory.getChargingStationVendorImpl(chargingStation);
    ...
    await ChargingStationService.updateChargingStationProfiles(
      action,
      req.tenant,
      chargingStation,
      req.user,
      chargingProfiles.result,
      updatedChargingProfiles,
      filteredRequest
    );
    // Call the limitation
    const result = await chargingStationVendor.setStaticPowerLimitation(
      req.tenant,
      chargingStation,
      chargePoint,
      filteredRequest.ampLimitValue
    ...
    if (
      result.status !== OCPPConfigurationStatus.ACCEPTED &&
  ...
  public static async handleGetChargingProfiles(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingProfilesGetReq(req.query);
    // Check dynamic auth
    ...
    if (filteredRequest.WithAuth) {
      await AuthorizationService.addChargingProfilesAuthorizations(
        req.tenant,
        req.user,
        chargingProfiles,
        authorizations
  ...
  public static async handleTriggerSmartCharging(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.SITE_AREA,
      MODULE_NAME,
      'handleTriggerSmartCharging'
    );
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateSmartChargingTriggerReq(req.query);
    // Get Site Area
  ...
  public static async handleGenerateQrCodeForConnector(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationQRCodeGenerateReq(
        req.query
      );
    // Check dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(
    ...
    UtilsService.assertObjectExists(
      action,
      Utils.getConnectorFromID(chargingStation, filteredRequest.ConnectorID),
      `Connector ID '${filteredRequest.ConnectorID}' does not exist`,
      MODULE_NAME,
      'handleGetChargingStationOcppParameters',
      req.user
    );
  ...
  public static async handleCreateChargingProfile(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingProfileCreateReq(req.body);
    // Check dynamic auth
    await AuthorizationService.checkAndGetChargingProfileAuthorizations(
      req.tenant,
      req.user,
      {},
      Action.CREATE
    ...
    // Create
    const chargingProfileID = await ChargingStationService.setAndSaveChargingProfile(
      filteredRequest,
      action,
      req
    );
    res
      ...
      .send(Object.assign({ id: chargingProfileID }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }

  public static async handleUpdateChargingProfile(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingProfileUpdateReq({
        ...req.params,
        ...req.body,
      });
    // Check dynamic auth
    await UtilsService.checkAndGetChargingProfileAuthorization(
      req.tenant,
      req.user,
      filteredRequest.id,
      Action.UPDATE,
  ...
  public static async handleDeleteChargingProfile(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const chargingProfileID =
      ChargingStationValidatorRest.getInstance().validateChargingProfileDeleteReq(req.query).ID;
    // Check dynamic auth
    const chargingProfile = await UtilsService.checkAndGetChargingProfileAuthorization(
      req.tenant,
      req.user,
      chargingProfileID,
      Action.DELETE,
  ...
  public static async handleGetChargingStationOcppParameters(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationOcppParametersGetReq(
        req.query
      );
    // Check dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(
    ...
    // Get the Parameters
    const parameters = await ChargingStationStorage.getOcppParameters(
      req.tenant,
      chargingStation.id
    );
    // Return the result
  ...
  public static async handleRequestChargingStationOcppParameters(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationOcppParametersRequestReq(
        req.body
      );
    // Check dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(
    ...
    let result = await OCPPCommon.requestAndSaveChargingStationOcppParameters(
      req.tenant,
      chargingStation
    );
    if (filteredRequest.forceUpdateOCPPParamsFromTemplate) {
      result = await OCPPUtils.updateChargingStationOcppParametersWithTemplate(
        req.tenant,
        chargingStation
      );
    }
  ...
  public static async handleDeleteChargingStation(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const chargingStationID =
      ChargingStationValidatorRest.getInstance().validateChargingStationDeleteReq(req.query).ID;
    // Check dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(
      req.tenant,
      req.user,
    ...
    const ongoingTransactions = await TransactionStorage.getTransactions(
      req.tenant,
      { chargingStationIDs: [chargingStationID], status: TransactionStatus.ACTIVE },
  ...
  public static async handleGetChargingStation(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationGetReq({
        ...req.params,
        ...req.query,
      });
    // Check dynamic auth
    let chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(
      ...
      null,
      {
        ...
        withSite: true,
        withSiteArea: true,
        withReservation: filteredRequest.WithReservation,
      },
      true
    );
  ...
  public static async handleGetChargingStations(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationsGetReq(req.query);
    // Get Charging Stations
    res.json(await ChargingStationService.getChargingStations(req, filteredRequest));
    next();
  }

  public static async handleExportChargingStationsOCPPParams(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    const chargingStations = await ChargingStationService.getChargingStations(
      req,
      filteredRequest,
      Action.EXPORT,
    ...
    for (const chargingStation of chargingStations.result) {
      const ocppParameters = await ChargingStationStorage.getOcppParameters(
        req.tenant,
        chargingStation.id
      );
      // Get OCPP Params
  ...
  public static async handleExportChargingStations(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleDownloadQrCodesPdf(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationQRCodeDownloadReq(
        req.query
      );
    // Export
    await UtilsService.exportToPDF(
      req,
      res,
      'exported-charging-stations-qr-code.pdf',
      ChargingStationService.getChargingStations.bind(
        this,
        req,
        filteredRequest,
        Action.GENERATE_QR
  ...
  public static async handleGetChargingStationsInError(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationInErrorReq(req.query);
    // Check dynamic auth
    ...
    // Get Charging Stations
    const chargingStations = await ChargingStationStorage.getChargingStationsInError(
      req.tenant,
      {
        search: filteredRequest.Search,
        siteIDs: filteredRequest.SiteID ? filteredRequest.SiteID.split('|') : null,
  ...
  public static async handleGetStatusNotifications(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationNotificationsGetReq(
        req.query
      );
    // Check dynamic auth
    const authorizations = await AuthorizationService.checkAndGetChargingStationsAuthorizations(
    ...
    const statusNotifications = await OCPPStorage.getStatusNotifications(
      req.tenant,
      authorizations.filters,
      {
        limit: filteredRequest.Limit,
  ...
  public static async handleReserveNow(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationActionReserveNowReq(
        req.body
      );
    ...
    if (response.status.toUpperCase() !== 'ACCEPTED') {
      throw new AppError({
        ...
        module: MODULE_NAME,
        method: ChargingStationService.handleReserveNow.name,
  ...
  public static async handleCancelReservation(
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationActionReservationCancelReq(
        req.body
      );
    // Check and get dynamic auth
    const chargingStation = await UtilsService.checkAndGetChargingStationAuthorization(
    ...
    if (!chargingStationClient) {
      throw new BackendError({
        action,
        module: MODULE_NAME,
        method: ChargingStationService.handleCancelReservation.name,
        message: 'Charging Station is not connected to the backend',
      });
    }
  ...
  public static async handleGetBootNotifications(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Get all Status Notifications
    const bootNotifications = await OCPPStorage.getBootNotifications(
      req.tenant,
      authorizations.filters,
      {
        limit: filteredRequest.Limit,
  ...
  public static async handleGetFirmware(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Filter
    const filteredRequest =
      ChargingStationValidatorRest.getInstance().validateChargingStationFirmwareDownloadReq(
        req.query
      );
    // Open a download stream and pipe it in the response
    const bucketStream = ChargingStationStorage.getChargingStationFirmware(
      filteredRequest.FileName
    );
    // Set headers
  ...
  public static async handleOcpiAction(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    try {
      ...
      if (action === ServerAction.OCPI_EMSP_START_SESSION) {
        ...
        const remoteStartRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionTransactionStartReq(
            req.body
          );
        actionFound = true;
        // Start Transaction
      ...
      if (action === ServerAction.OCPI_EMSP_STOP_SESSION) {
        ...
        const remoteStopRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionTransactionStopReq(
            req.body
          );
        actionFound = true;
        // Stop Transaction
  ...
  public static async handleOcppAction(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    try {
      ...
      if (command === Command.CLEAR_CACHE) {
        ChargingStationValidatorRest.getInstance().validateChargingStationActionCacheClearReq(
          req.body
        );
        commandFound = true;
      ...
      if (command === Command.CHANGE_AVAILABILITY) {
        const changeAvailabilityRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionAvailabilityChangeReq(
            req.body
          );
        commandFound = true;
      ...
      if (command === Command.GET_CONFIGURATION) {
        const getConfigurationRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionConfigurationGetReq(
            req.body
          );
        commandFound = true;
        result = await chargingStationClient.getConfiguration({
          key: getConfigurationRequest.args.key,
        });
      }
      // Change Configuration
      if (command === Command.CHANGE_CONFIGURATION) {
        const changeConfigurationRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionConfigurationChangeReq(
            req.body
          );
        commandFound = true;
      ...
      if (command === Command.DATA_TRANSFER) {
        const dataTransferRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionDataTransferReq(
            req.body
          );
        commandFound = true;
      ...
      if (command === Command.GET_COMPOSITE_SCHEDULE) {
        const compositeScheduleRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionCompositeScheduleGetReq(
            req.body
          );
        commandFound = true;
      ...
      if (command === Command.GET_DIAGNOSTICS) {
        const diagnosticsRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationDiagnosticsGetReq(
            req.body
          );
        commandFound = true;
      ...
      if (command === Command.UNLOCK_CONNECTOR) {
        const unlockConnectorRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionConnectorUnlockReq(
            req.body
          );
        commandFound = true;
        result = await chargingStationClient.unlockConnector({
          connectorId: unlockConnectorRequest.args.connectorId,
        });
      }
      // Update Firmware
      if (command === Command.UPDATE_FIRMWARE) {
        const updateFirmwareRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionFirmwareUpdateReq(
            req.body
          );
        commandFound = true;
      ...
      if (command === Command.RESET) {
        const resetRequest =
          ChargingStationValidatorRest.getInstance().validateChargingStationActionResetReq(
            req.body
          );
        commandFound = true;
      ...
      if (
        Utils.objectHasProperty(result, 'status') &&
        ![OCPPStatus.ACCEPTED, OCPPUnlockStatus.UNLOCKED].includes(result.status)
      ) {
  ...
  public static async handleCheckSmartChargingConnection(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.CHARGING_STATION,
      MODULE_NAME,
      'handleCheckSmartChargingConnection'
    );
    // Check auth
    await AuthorizationService.checkAndGetSmartChargingAuthorizations(
      req.tenant,
      req.user,
      Action.CHECK_CONNECTION
    );
  ...
  private static async getChargingStations(
    req: Request,
    ...
    const chargingStations = await ChargingStationStorage.getChargingStations(
      req.tenant,
      {
        ...
        chargingStationIDs: filteredRequest.ChargingStationID
          ? filteredRequest.ChargingStationID.split('|')
          : null,
        connectorStatuses: (filteredRequest.ConnectorStatus
          ? filteredRequest.ConnectorStatus.split('|')
          : null) as ChargePointStatus[],
        connectorTypes: (filteredRequest.ConnectorType
          ? filteredRequest.ConnectorType.split('|')
  ...
  private static convertOCPPParamsToCSV(ocppParams: OCPPParams, writeHeader = true): string {
    ...
    // Header
    if (writeHeader) {
      headers = ['chargingStation', 'name', 'value', 'siteArea', 'site'].join(
        Constants.CSV_SEPARATOR
      );
    }
    // Content
    const rows = ocppParams.params
      .map((param) => {
        const row = [
  ...
  private static convertToCSV(
    ...
    const getCreatedOnCell = (chargingStation: ChargingStation, i18nManager: I18nManager) => {
      if (chargingStation.createdOn) {
        return [
          i18nManager.formatDateTime(chargingStation.createdOn, 'L') +
            ' ' +
            i18nManager.formatDateTime(chargingStation.createdOn, 'LT'),
        ];
      }
      return [
    ...
    const rows = chargingStations
      .map((chargingStation) => {
        const row = [
          ...
          i18nManager.formatDateTime(chargingStation.lastSeen, 'L') +
            ' ' +
            i18nManager.formatDateTime(chargingStation.lastSeen, 'LT'),
          i18nManager.formatDateTime(chargingStation.lastReboot, 'L') +
            ' ' +
            i18nManager.formatDateTime(chargingStation.lastReboot, 'LT'),
  ...
  private static async convertQrCodeToPDF(
    req: Request,
    ...
    for (const chargingStation of chargingStations) {
      if (!Utils.isEmptyArray(chargingStation.connectors)) {
        for (const connector of chargingStation.connectors) {
          ...
          // Build title
          const qrCodeTitle = `${chargingStation.id} / ${i18nManager.translate(
            'chargers.connector'
          )} ${Utils.getConnectorLetterFromConnectorID(connector.connectorId)}`;
          // Add the QR Codes
          ChargingStationService.build3SizesPDFQrCode(pdfDocument, qrCodeImage, qrCodeTitle);
          // Add page (expect the last one)
          if (
            !connectorID &&
            (chargingStations[chargingStations.length - 1] !== chargingStation ||
  ...
  private static build3SizesPDFQrCode(
    pdfDocument: PDFKit.PDFDocument,
    qrCodeImage: string,
    qrCodeTitle: string
  ): void {
    ...
    pdfDocument.image(
      qrCodeImage,
      pdfDocument.page.width / 2 - mediumSquareSide / 2,
      bigSquareSide + marginHeight * 2,
      {
        width: mediumSquareSide,
        height: mediumSquareSide,
      }
    );
    pdfDocument.fontSize(12);
    pdfDocument.text(qrCodeTitle, 65, bigSquareSide + mediumSquareSide + marginHeight * 2 + 10, {
      align: 'center',
    });
    // Add small QR-Code
    pdfDocument.image(
      qrCodeImage,
      pdfDocument.page.width / 2 - smallSquareSide / 2,
      bigSquareSide + mediumSquareSide + marginHeight * 3,
      {
    ...
    pdfDocument.text(
      qrCodeTitle,
      65,
      bigSquareSide + mediumSquareSide + smallSquareSide + marginHeight * 3 + 10,
      { align: 'center' }
    );
  }

  private static async setAndSaveChargingProfile(
    filteredRequest: ChargingProfile,
    action: ServerAction,
    req: Request
  ): Promise<string> {
    // Check dynamic auth
    ...
    if (!chargingStation.capabilities?.supportChargingProfiles) {
      throw new AppError({
        ...
        errorCode: HTTPError.FEATURE_NOT_SUPPORTED_ERROR,
        module: MODULE_NAME,
        method: 'setAndSaveChargingProfile',
  ...
  private static async executeChargingStationGetCompositeSchedule(
    action: ServerAction,
    chargingStation: ChargingStation,
    command: Command,
    filteredRequest: HttpChargingStationCompositeScheduleGetRequest,
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<any> {
    ...
    if (!chargingStationVendor) {
      throw new AppError({
        ...
        errorCode: HTTPError.FEATURE_NOT_SUPPORTED_ERROR,
        module: MODULE_NAME,
        method: 'executeChargingStationGetCompositeSchedule',
        message: `No vendor implementation is available (${chargingStation.chargePointVendor}) for limiting the charge`,
      });
    }
    ...
    if (filteredRequest.args.connectorId === 0) {
      ...
      for (const connector of chargingStation.connectors) {
        ...
        result.push(
          await chargingStationVendor.getCompositeSchedule(
            req.tenant,
            chargingStation,
            chargePoint,
            connector.connectorId,
            filteredRequest.args.duration,
            filteredRequest.args.chargingRateUnit
          )
        );
      }
    } else {
      ...
      result = await chargingStationVendor.getCompositeSchedule(
        ...
        chargingStation,
        chargePoint,
        filteredRequest.args.connectorId,
        filteredRequest.args.duration,
        filteredRequest.args.chargingRateUnit
      );
    }
  ...
  private static async executeChargingStationStartTransaction(
    action: ServerAction,
    chargingStation: ChargingStation,
    command: Command,
    filteredRequest: HttpChargingStationTransactionStartRequest,
    req: Request,
    res: Response,
    next: NextFunction,
    ...
    if (
      ...
      (!filteredRequest.args.visualTagID && !filteredRequest.args.tagID)
    ) {
      throw new AppError({
        action,
        user: req.user,
        ...
        module: MODULE_NAME,
        method: 'executeChargingStationStartTransaction',
  ...
  private static async executeChargingStationStopTransaction(
    action: ServerAction,
    chargingStation: ChargingStation,
    command: Command,
    filteredRequest: HttpChargingStationTransactionStopRequest,
    req: Request,
    res: Response,
    next: NextFunction,
    chargingStationClient: ChargingStationClient
    ...
    const transaction = await TransactionStorage.getTransaction(
      req.tenant,
      filteredRequest.args.transactionId,
      { withUser: true }
    );
    UtilsService.assertObjectExists(
      action,
      transaction,
      `Transaction ID '${filteredRequest.args.transactionId}' does not exist`,
      MODULE_NAME,
      'handleAction',
      req.user
    );
    ...
    const tags = await TagStorage.getTags(
      req.tenant,
      { userIDs: [req.user.id], active: true },
      Constants.DB_PARAMS_SINGLE_RECORD,
      ['id']
    );
    if (Utils.isEmptyArray(tags.result)) {
      throw new AppError({
        ...
        errorCode: HTTPError.USER_NO_BADGE_ERROR,
        module: MODULE_NAME,
        method: 'executeChargingStationStopTransaction',
        message: 'The user does not have any active badge',
      });
    }
    const tag = tags.result[0];
    // Check if user is authorized
    await CommonUtilsService.isAuthorizedToStopTransaction(
      req.tenant,
      chargingStation,
      transaction,
      tag.id,
  ...
  private static async executeChargingStationChangeConfiguration(
    action: ServerAction,
    chargingStation: ChargingStation,
    command: Command,
    filteredRequest: HttpChargingStationConfigurationChangeRequest,
    req: Request,
    res: Response,
    next: NextFunction,
    chargingStationClient: ChargingStationClient
    ...
    if (
      ...
      if (result.status === OCPPConfigurationStatus.REBOOT_REQUIRED) {
        await Logging.logWarning({
          ...
          user: req.user,
          module: MODULE_NAME,
          method: 'executeChargingStationChangeConfiguration',
      ...
      if (filteredRequest.args.custom) {
        ...
        const ocppParametersFromDB = await ChargingStationStorage.getOcppParameters(
          req.tenant,
          chargingStation.id
        );
        // Set new structure
        ...
        const foundOcppParam = chargingStationOcppParameters.configuration.find(
          (ocppParam) => ocppParam.key === filteredRequest.args.key
        );
        if (foundOcppParam) {
          // Update param
          ...
          if (foundOcppParam.custom) {
            // Save
            await ChargingStationStorage.saveOcppParameters(
              req.tenant,
              chargingStationOcppParameters
            );
          } else {
            // Not a custom param: refresh the whole OCPP Parameters
            await OCPPCommon.requestAndSaveChargingStationOcppParameters(
              req.tenant,
              chargingStation
            );
          }
      ...
      const chargingStationVendor =
        ChargingStationVendorFactory.getChargingStationVendorImpl(chargingStation);
      if (chargingStationVendor) {
        await chargingStationVendor.checkUpdateOfOCPPParams(
          req.tenant,
          chargingStation,
          filteredRequest.args.key,
          filteredRequest.args.value
        );
      }
    }
    return result;
  ...
  private static async updateChargingStationRoaming(
    tenant: Tenant,
    ...
    if (Utils.isComponentActiveFromToken(loggedUser, TenantComponents.OCPI)) {
      ...
      try {
        const ocpiClient = (await OCPIClientFactory.getAvailableOcpiClient(
          tenant,
          OCPIRole.CPO
        )) as CpoOCPIClient;
        ...
        await Logging.logError({
          tenantID: tenant.id,
          module: MODULE_NAME,
          method: 'updateChargingStationRoaming',
          action,
          user: loggedUser,
    ...
    if (Utils.isComponentActiveFromToken(loggedUser, TenantComponents.OICP)) {
      ...
      try {
        const oicpClient = (await OICPClientFactory.getAvailableOicpClient(
          tenant,
          OCPIRole.CPO
        )) as CpoOICPClient;
        if (oicpClient) {
          ...
          await oicpClient.pushEvseData(
            OICPUtils.convertChargingStation2MultipleEvses(
              site,
              chargingStation.siteArea,
              chargingStation,
              options
  ...
  private static async deactivateChargingStationRoaming(
    action: ServerAction,
    tenant: Tenant,
    user: UserToken,
    chargingStation: ChargingStation,
    ...
    if (chargingStation.public) {
      if (Utils.isComponentActiveFromToken(user, TenantComponents.OICP)) {
        try {
          const oicpClient: CpoOICPClient = (await OICPClientFactory.getAvailableOicpClient(
            tenant,
            OCPIRole.CPO
          )) as CpoOICPClient;
          if (oicpClient) {
          ...
          await Logging.logError({
            tenantID: tenant.id,
            module: MODULE_NAME,
            method: 'deactivateChargingStationRoaming',
            action,
            user: user,
  ...
  private static updateChargingStationCommonProperties(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    siteArea: SiteArea,
    user: UserToken,
    filteredRequest: HttpChargingStationParamsUpdateRequest
  ): void {
    ...
    if (Utils.objectHasProperty(filteredRequest, 'public')) {
      ...
      if (
        Utils.isComponentActiveFromToken(user, TenantComponents.ORGANIZATION) &&
        filteredRequest.public &&
        !siteArea.site?.public
      ) {
        throw new AppError({
          ...LoggingHelper.getChargingStationProperties(chargingStation),
          action,
          ...
          module: MODULE_NAME,
          method: 'updateChargingStationCommonProperties',
        });
      }
      chargingStation.public = filteredRequest.public;
      // Handle update in Roaming
      void ChargingStationService.updateChargingStationRoaming(
        tenant,
        user,
        chargingStation,
        action
  ...
  private static async updateChargingStationManualConfiguration(
    action: ServerAction,
    chargingStation: ChargingStation,
    user: UserToken,
    filteredRequest: HttpChargingStationParamsUpdateRequest
      ...
      if (!chargingStation.manualConfiguration && filteredRequest.manualConfiguration) {
        ...
        if (!chargingStationTemplate) {
          throw new AppError({
            ...
            errorCode: HTTPError.GENERAL_ERROR,
            module: MODULE_NAME,
            method: 'updateChargingStationManualConfiguration',
  ...
  private static updateChargingStationConnectors(
    chargingStation: ChargingStation,
    user: UserToken,
    filteredRequest: HttpChargingStationParamsUpdateRequest
  ): void {
    if (!Utils.isEmptyArray(filteredRequest.connectors)) {
      for (const filteredConnector of filteredRequest.connectors) {
        ...
        if (connector) {
          ...
          if (
            Utils.isEmptyArray(chargingStation.chargePoints) ||
            chargingStation.manualConfiguration
          ) {
  ...
  private static updateChargingStationManualAutoConfig(
    chargingStation: ChargingStation,
    user: UserToken,
    filteredRequest: HttpChargingStationParamsUpdateRequest
  ): void {
      ...
      if (!Utils.isEmptyArray(filteredRequest.chargePoints)) {
        ...
        for (const filteredChargePoint of filteredRequest.chargePoints) {
          const chargePoint = Utils.getChargePointFromID(
            chargingStation,
            filteredChargePoint.chargePointID
          );
          ...
          if (chargePoint) {
            (chargePoint.currentType = filteredChargePoint.currentType),
              ...
              (chargePoint.sharePowerToAllConnectors =
                filteredChargePoint.sharePowerToAllConnectors),
              (chargePoint.excludeFromPowerLimitation =
                filteredChargePoint.excludeFromPowerLimitation);
            (chargePoint.ocppParamForPowerLimitation =
              filteredChargePoint.ocppParamForPowerLimitation),
  ...
  private static updateChargingStationSiteArea(
    action: ServerAction,
    chargingStation: ChargingStation,
    user: UserToken,
    siteArea: SiteArea
    ...
    if (siteArea) {
      ...
      for (const connector of chargingStation.connectors) {
        const numberOfConnectedPhase = Utils.getNumberOfConnectedPhases(
          chargingStation,
          null,
          connector.connectorId
        );
        if (numberOfConnectedPhase !== 1 && siteArea?.numberOfPhases === 1) {
          throw new AppError({
            ...
            module: MODULE_NAME,
            method: 'updateChargingStationSiteArea',
  ...
  private static async checkAndApplyChargingStationTemplate(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    user: UserToken,
    resetAndApplyTemplate: boolean
  ): Promise<void> {
    ...
    if (resetAndApplyTemplate) {
      try {
        ...
        throw new AppError({
          ...
          errorCode: HTTPError.GENERAL_ERROR,
          module: MODULE_NAME,
          method: 'checkAndApplyChargingStationTemplate',
  ...
  private static async updateChargingStationProfiles(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    user: UserToken,
    chargingProfiles: ChargingProfile[],
    updatedChargingProfiles: ChargingProfile[],
    filteredRequest: HttpChargingStationLimitPowerRequest
  ): Promise<void> {
    for (let index = 0; index < updatedChargingProfiles.length; index++) {
      ...
      // Check schedules
      if (
        updatedChargingProfile.profile &&
        updatedChargingProfile.profile.chargingSchedule &&
        updatedChargingProfile.profile.chargingSchedule.chargingSchedulePeriod
      ) {
        for (const chargingSchedulePeriod of updatedChargingProfile.profile.chargingSchedule
      ...
      if (planHasBeenAdjusted) {
        ...
        if (!filteredRequest.forceUpdateChargingPlan) {
          throw new AppError({
            ...
            errorCode: HTTPError.GENERAL_ERROR,
            module: MODULE_NAME,
            method: 'updateChargingStationProfiles',
  ...
  private static async handleOcpiStopTransaction(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    user: UserToken,
    remoteStopRequest: HttpChargingStationTransactionStopRequest
  ): Promise<OCPICommandResponse> {
    // Get Transaction
    const transaction = await TransactionStorage.getTransaction(
      tenant,
      remoteStopRequest.args.transactionId,
      { withUser: true }
    );
    UtilsService.assertObjectExists(
      action,
      transaction,
      `Transaction ID '${remoteStopRequest.args.transactionId}' does not exist`,
      MODULE_NAME,
      'handleOcpiStopTransaction',
      user
    );
    // Get OCPI client
    const ocpiClient = (await OCPIClientFactory.getAvailableOcpiClient(
    ...
    if (!ocpiClient) {
      throw new BackendError({
        ...
        action,
        module: MODULE_NAME,
        method: 'handleOcpiStopTransaction',
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} OCPI component requires at least one eMSP endpoint to start a Transaction`,
      });
    }
  ...
  private static async handleOcpiStartTransaction(
    action: ServerAction,
    tenant: Tenant,
    chargingStation: ChargingStation,
    user: UserToken,
    remoteStartRequest: HttpChargingStationTransactionStartRequest
  ): Promise<OCPICommandResponse> {
    // Get the Tag
    ...
    if (!tagID && remoteStartRequest.args.visualTagID) {
      ...
      const tag = await UtilsService.checkAndGetTagByVisualIDAuthorization(
        tenant,
        user,
        remoteStartRequest.args.visualTagID,
        Action.READ,
        action,
        null,
        {},
      ...
      if (!tag) {
        throw new AppError({
          ...
          errorCode: HTTPError.GENERAL_ERROR,
          module: MODULE_NAME,
          method: 'handleOcpiStartTransaction',
    ...
    if (!ocpiClient) {
      throw new BackendError({
        ...
        method: 'handleOcpiStartTransaction',
        message: `${Utils.buildConnectorInfo(
          remoteStartRequest.args.connectorId
        )} OCPI component requires at least one eMSP endpoint to stop a Transaction`,
      });
    }
    // Start the Transaction
    return ocpiClient.remoteStartSession(
      chargingStation,
      remoteStartRequest.args.connectorId,
      tagID
    );
  ...
  private static async finalizeReserveNow(
    ...
    if (Utils.isComponentActiveFromToken(currentUser, TenantComponents.RESERVATION)) {
      ...
      await ReservationService.preventMultipleReserveNow(
        tenant,
        { id: reserveNow.reservationId, type: ReservationType.RESERVE_NOW },
        currentUser.id
      );
