...
export default class AssetService {
  public static async handleGetAssetConsumption(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.ASSET,
      MODULE_NAME,
      'handleGetAssetConsumption'
    );
    // Filter
    const filteredRequest = AssetValidatorRest.getInstance().validateAssetGetConsumptionsReq(
      req.query
    );
    // Check and get Asset
    ...
    if (!filteredRequest.StartDate || !filteredRequest.EndDate) {
      throw new AppError({
        ...
        module: MODULE_NAME,
        method: 'handleGetAssetConsumption',
        user: req.user,
        action: action,
      });
    }
    // Check dates order
    AssetService.checkDateOrder(
    ...
    const consumptions = await ConsumptionStorage.getAssetConsumptions(
      req.tenant,
      {
  ...
  public static async handleCreateAssetConsumption(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.ASSET,
      MODULE_NAME,
      'handleCreateAssetConsumption'
    );
    // Validate request
    const filteredRequest = AssetValidatorRest.getInstance().validateAssetConsumptionCreateReq({
      ...req.query,
      ...req.body,
    });
    // Check and get Asset
    ...
    if (!Utils.isNullOrUndefined(asset.connectionID)) {
      throw new AppError({
        ...
        module: MODULE_NAME,
        method: 'handleCreateAssetConsumption',
        user: req.user,
        action: action,
      });
    }
    // Check dates order
    AssetService.checkDateOrder(
      filteredRequest.startedAt,
      filteredRequest.endedAt,
      req.user,
      action,
      'handleCreateAssetConsumption'
    );
    // Get latest consumption and check dates
    const lastConsumption = await ConsumptionStorage.getLastAssetConsumption(req.tenant, {
      assetID: filteredRequest.assetID,
    });
    if (!Utils.isNullOrUndefined(lastConsumption)) {
      if (moment(filteredRequest.startedAt).isBefore(moment(lastConsumption.endedAt))) {
        throw new AppError({
          ...LoggingHelper.getAssetProperties(asset),
          errorCode: HTTPError.GENERAL_ERROR,
          message: `The start date '${moment(
            filteredRequest.startedAt
          ).toISOString()}' of the pushed consumption is before the end date '${moment(
            lastConsumption.endedAt
          ).toISOString()}' of the latest asset consumption`,
          module: MODULE_NAME,
          method: 'handleCreateAssetConsumption',
          user: req.user,
          action: action,
    ...
    if (Utils.isNullOrUndefined(consumptionToSave.consumptionWh)) {
      const timePeriod = moment(consumptionToSave.endedAt).diff(
        moment(consumptionToSave.startedAt),
        'minutes'
      );
      consumptionToSave.consumptionWh = Utils.createDecimal(consumptionToSave.instantWatts)
        .mul(Utils.createDecimal(timePeriod).div(60))
        .toNumber();
    }
    // Add Amps
    if (Utils.isNullOrUndefined(consumptionToSave.instantAmps)) {
      consumptionToSave.instantAmps = Utils.createDecimal(consumptionToSave.instantWatts)
    ...
    asset.lastConsumption = {
      timestamp: consumptionToSave.endedAt,
      value: consumptionToSave.consumptionWh,
    };
    // Save Asset
    ...
    res
      ...
      .json(Object.assign({ consumption: consumptionToSave }, Constants.REST_RESPONSE_SUCCESS));
    next();
  }

  public static async handleCheckAssetConnection(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    UtilsService.assertComponentIsActiveFromToken(
      ...
      Entity.ASSET,
      MODULE_NAME,
      'handleCheckAssetConnection'
    );
    // Filter request, NOTE: ID in this request is a connection ID not an Asset ID
    const filteredRequest = AssetValidatorRest.getInstance().validateAssetCheckConnectionReq(
      req.query
    );
    // Check dynamic auth
    await AuthorizationService.checkAndGetAssetsAuthorizations(
      req.tenant,
      req.user,
      Action.CHECK_CONNECTION
    );
  ...
  public static async handleRetrieveConsumption(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (!assetImpl) {
      throw new AppError({
        ...
        message: 'Asset service is not configured',
        module: MODULE_NAME,
        method: 'handleRetrieveConsumption',
  ...
  public static async handleGetAssetsInError(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    // Get the assets
    const assets = await AssetStorage.getAssetsInError(
      req.tenant,
      {
  ...
  public static async handleDeleteAsset(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleGetAsset(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleGetAssetImage(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleGetAssets(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  public static async handleCreateAsset(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    if (
      ...
      filteredAssetRequest.siteAreaID
    ) {
  ...
  public static async handleUpdateAsset(
    action: ServerAction,
    req: Request,
    res: Response,
    next: NextFunction
  ...
  private static checkDateOrder(
    ...
    // Check dates order
    if (startSate && endDate && !moment(endDate).isAfter(moment(startSate))) {
      throw new AppError({
        errorCode: HTTPError.GENERAL_ERROR,
        message: `The requested start date '${moment(
          startSate
        ).toISOString()}' is after the end date '${moment(endDate).toISOString()}' `,
        module: MODULE_NAME,
        method: methodName,
        user: user,
        action: action,
