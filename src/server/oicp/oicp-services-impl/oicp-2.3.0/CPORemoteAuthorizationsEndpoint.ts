...
import {
  OICPAuthorizeRemoteStartCpoReceive,
  OICPAuthorizeRemoteStopCpoReceive,
...
export default class CPORemoteAuthorizationsEndpoint extends AbstractEndpoint {
  ...
  private async authorizeRemoteStart(
    ...
    const chargingStationConnector = await OICPUtils.getChargingStationConnectorFromEvseID(
      tenant,
      evseID
    );
    ...
    if (!chargingStation) {
      await Logging.logError({
        ...
        module: MODULE_NAME,
        method: 'authorizeRemoteStart',
      });
      return OICPUtils.noSuccess(
        session,
        `EVSE for EvseID '${authorizeRemoteStart.EvseID}' not found`
      );
    }
    if (!connector) {
      await Logging.logError({
    ...
    if (!chargingStation.issuer || !chargingStation.public) {
      ...
      return OICPUtils.noSuccess(
        session,
        `EVSE '${authorizeRemoteStart.EvseID}' cannot be used with OICP`
      );
    }
    if (
    ...
    if (existingAuthorization) {
      ...
      if (
        existingAuthorization?.oicpIdentification?.RemoteIdentification?.EvcoID !==
        authorizeRemoteStart.Identification?.RemoteIdentification?.EvcoID
      ) {
        ...
        if (OICPUtils.isAuthorizationValid(existingAuthorization.timestamp)) {
          ...
          return OICPUtils.noSuccess(
            session,
            `An existing remote authorization exists for Charging Station '${chargingStation.id}' and Connector ID ${connector.connectorId}`
          );
        }
      }
      ...
      existingAuthorization.tagId = OICPUtils.convertOICPIdentification2TagID(
        authorizeRemoteStart.Identification
      );
    ...
    const result = await this.remoteStartTransaction(
      tenant,
      chargingStation,
      connector,
  ...
  private async authorizeRemoteStop(
    req: Request,
    res: Response,
    next: NextFunction,
    tenant: Tenant
    ...
    const transaction = await TransactionStorage.getOICPTransactionBySessionID(
      tenant,
      authorizeRemoteStop.SessionID
    );
    if (!transaction) {
      await Logging.logError({
        tenantID: tenant.id,
        ...
        module: MODULE_NAME,
        method: 'authorizeRemoteStop',
      });
      return OICPUtils.noSuccess(
        session,
        `Transaction with OICP Transaction ID '${authorizeRemoteStop.SessionID}' does not exists`
      );
    }
    if (!transaction.issuer) {
      ...
      return OICPUtils.noSuccess(
        session,
        `Transaction with OICP Transaction ID '${authorizeRemoteStop.SessionID}' has been issued locally`
      );
    }
    if (transaction.stop) {
      await Logging.logError({
      ...
      return OICPUtils.noSuccess(
        session,
        `Transaction with OICP Transaction ID '${authorizeRemoteStop.SessionID}' is already stopped`
      );
    }
    ...
    if (!chargingStation) {
      ...
      return OICPUtils.noSuccess(
        session,
        `Charging Station '${transaction.chargeBoxID}' not found`
      );
    }
  ...
  private async remoteStartTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
    connector: Connector,
    authorizeRemoteStart: OICPAuthorizeRemoteStartCpoReceive
  ...
  private async remoteStopTransaction(
    tenant: Tenant,
    chargingStation: ChargingStation,
