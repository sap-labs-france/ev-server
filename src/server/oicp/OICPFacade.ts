...
export default class OICPFacade {
  public static async processStartTransaction(
    ...
    if (
      !Utils.isTenantComponentActive(tenant, TenantComponents.OICP) ||
    ...
    try {
      ...
      await Logging.logWarning({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} Cannot process OICP Update Transaction: ${error.message as string}`,
  ...
  public static async checkAndSendTransactionCdr(
    ...
    if (
      Utils.isTenantComponentActive(tenant, TenantComponents.OICP) &&
      transaction.oicpData?.session &&
      !transaction.oicpData.cdr?.SessionID
    ) {
      // Get the lock
  ...
  private static async checkAndGetOICPCpoClient(
    tenant: Tenant,
    transaction: Transaction,
    user: User,
    action: ServerAction
    ...
    if (!user) {
      throw new BackendError({
        ...
        action,
        module: MODULE_NAME,
        method: 'checkAndGetOICPCpoClient',
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} User is mandatory`,
    ...
    if (!oicpClient) {
      throw new BackendError({
        ...
        message: `${Utils.buildConnectorInfo(
          transaction.connectorId,
          transaction.id
        )} OICP component requires at least one CPO endpoint to start a Transaction`,
      });
    }
    return oicpClient;
  }
