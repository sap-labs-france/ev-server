...
export default class OICPServer {
  ...
  public constructor(oicpRestConfig: OICPServiceConfiguration) {
    ...
    oicpServices.getOICPServiceImplementations().forEach((oicpService) => {
      ...
      this.expressApplication.use(
        oicpService.getPath(),
        async (req: TenantIdHoldingRequest, res: Response, next: NextFunction) => {
          try {
  ...
  public start(): void {
    ServerUtils.startHttpServer(
      this.oicpRestConfig,
      ServerUtils.createHttpServer(this.oicpRestConfig, this.expressApplication),
      MODULE_NAME,
      ServerType.OICP_SERVER
    );
  }
}
