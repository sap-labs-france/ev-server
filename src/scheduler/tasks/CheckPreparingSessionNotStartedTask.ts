...
export default class CheckPreparingSessionNotStartedTask extends TenantSchedulerTask {
  public async processTenant(
    tenant: Tenant,
    config: CheckPreparingSessionNotStartedTaskConfig
  ): Promise<void> {
    // Get the lock
    const sessionNotStartedLock = LockingManager.createExclusiveLock(
      tenant.id,
      LockEntity.CHARGING_STATION,
      'preparing-session-not-started'
    );
    if (await LockingManager.acquire(sessionNotStartedLock)) {
      try {
        ...
        const chargingStations = await ChargingStationStorage.getChargingStations(
          tenant,
          {
            statusChangedBefore: moment()
              .subtract(config.preparingStatusMaxMins, 'minutes')
              .toDate(),
        ...
        for (const chargingStation of chargingStations.result) {
          ...
          if (chargingStation.siteArea && chargingStation.siteArea.siteID) {
            // Get Site Owners
            const siteOwners = await SiteStorage.getSiteUsers(
              tenant,
              { siteIDs: [chargingStation.siteArea.siteID], siteOwnerOnly: true },
              Constants.DB_PARAMS_MAX_LIMIT
            );
            if (siteOwners && siteOwners.count > 0) {
              // Send notification
              for (const connector of chargingStation.connectors) {
                NotificationHandler.sendPreparingTransactionNotStarted(
                  tenant,
                  chargingStation,
                  siteOwners.result[0].user,
                  {
