...
export default class CheckChargingStationTemplateTask extends TenantSchedulerTask {
  public async processTenant(tenant: Tenant): Promise<void> {
    // Get the lock
    const checkChargingStationTemplateLock = LockingManager.createExclusiveLock(
      tenant.id,
      LockEntity.CHARGING_STATION,
      'check-charging-station-template'
    );
  ...
  private async applyTemplateToChargingStations(tenant: Tenant) {
    ...
    while (true) {
      ...
      for (const chargingStation of chargingStations.result) {
        try {
          // Apply template
          const chargingStationTemplateUpdateResult =
            await OCPPUtils.checkAndApplyTemplateToChargingStation(tenant, chargingStation);
          // Save
          ...
          await Logging.logError({
            ...
            module: MODULE_NAME,
            method: 'applyTemplateToChargingStations',
            message: `Template update error in Tenant ${Utils.buildTenantName(tenant)}): ${
    ...
    if (updated > 0) {
      await Logging.logDebug({
        ...
        module: MODULE_NAME,
        method: 'applyTemplateToChargingStations',
        message: `${updated} Charging Stations have been processed with Template in Tenant ${Utils.buildTenantName(
          tenant
        )})`,
      });
    }
