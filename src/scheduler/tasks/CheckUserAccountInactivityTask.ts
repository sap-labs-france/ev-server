...
export default class CheckUserAccountInactivityTask extends TenantSchedulerTask {
  public async processTenant(
    tenant: Tenant,
    config: CheckUserAccountInactivityTaskConfig
  ): Promise<void> {
    // Get the lock
    const accountInactivityLock = LockingManager.createExclusiveLock(
      tenant.id,
      LockEntity.USER,
    ...
    if (await LockingManager.acquire(accountInactivityLock)) {
      try {
        ...
        const someMonthsAgo = moment()
          .subtract(config.userAccountInactivityMonths, 'months')
          .toDate();
        const params = {
          issuer: true,
          statuses: ['A'],
        ...
        for (const user of users.result) {
          // Notification
          NotificationHandler.sendUserAccountInactivity(tenant, user, {
            user,
