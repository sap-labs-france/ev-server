...
export default class CheckSessionNotStartedAfterAuthorizeTask extends TenantSchedulerTask {
  public async processTenant(
    tenant: Tenant,
    config: CheckSessionNotStartedAfterAuthorizeTaskConfig
  ): Promise<void> {
    // Get the lock
    ...
    if (await LockingManager.acquire(sessionNotStartedLock)) {
      try {
        // Get notification
        const notificationTransactionNotStarted =
          await TransactionStorage.getNotStartedTransactions(tenant, {
        ...
        if (
          ...
          notificationTransactionNotStarted.result.length > 0
        ) {
          for (const notification of notificationTransactionNotStarted.result) {
            NotificationHandler.sendTransactionNotStarted(
              tenant,
