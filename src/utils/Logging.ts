...
import PerformanceRecord, {
  PerformanceRecordGroup,
...
export default class Logging {
  ...
  public static async traceDatabaseRequestEnd(
    tenant: Tenant,
    module: string,
    method: string,
    ...
    if (Logging.getTraceConfiguration().traceDatabase) {
      ...
      const sizeOfRequestDataKB = Utils.truncTo(
        Utils.createDecimal(sizeof(request)).div(1024).toNumber(),
        2
      );
      const sizeOfResponseDataKB = Utils.truncTo(
        Utils.createDecimal(sizeof(response)).div(1024).toNumber(),
        2
      );
      const numberOfRecords = Array.isArray(response) ? response.length : 0;
      const message = `${module}.${method} - ${executionDurationMillis.toString()} ms - Req ${sizeOfRequestDataKB} KB - Res ${
        sizeOfResponseDataKB > 0 ? sizeOfResponseDataKB : '?'
      } KB - ${numberOfRecords.toString()} rec(s)`;
      Utils.isDevelopmentEnv() && Logging.logConsoleInfo(message);
      if (sizeOfResponseDataKB > Constants.PERF_MAX_DATA_VOLUME_KB) {
        const error = new Error(
          `Data must be < ${Constants.PERF_MAX_DATA_VOLUME_KB}KB, got ${sizeOfResponseDataKB}KB`
        );
        ...
        if (Utils.isDevelopmentEnv()) {
          ...
          Logging.logConsoleWarning(
            `Tenant ID '${tenant.id ? tenant.id : Constants.DEFAULT_TENANT_ID}'`
          );
      ...
      if (executionDurationMillis > Constants.PERF_MAX_RESPONSE_TIME_MILLIS) {
        const error = new Error(
          `Execution must be < ${Constants.PERF_MAX_RESPONSE_TIME_MILLIS} ms`
        );
        await Logging.logWarning({
          tenantID: tenant.id,
          action: ServerAction.PERFORMANCES,
  ...
  public static async traceNotificationEnd(
    tenant: Tenant,
    module: string,
    method: string,
    timeStartMillis: number,
    ...
    if (Logging.getTraceConfiguration().traceNotification) {
      ...
      const executionDurationMillis = new Date().getTime() - timeStartMillis;
      const sizeOfRequestDataKB = Utils.truncTo(
        Utils.createDecimal(sizeof(data)).div(1024).toNumber(),
        2
      );
  ...
  public static async logError(log: Log): Promise<string> {
    ...
    if (global.tenantIdMap && Utils.isMonitoringEnabled()) {
      // hack for unit test
      ...
      if (log.tenantID && !subDomain) {
        ...
        const labelValues = {
          tenant: subDomain,
          chargingstation: log.chargingStationID,
          serverAction: log.action,
        };
        const errorMetric = global.monitoringServer.getCounterClearableMetric(
          'logging',
          'error',
          'Number of errors',
          labelValues
  ...
  public static async logActionsResponse(
    ...
    messageSuccessAndError = messageSuccessAndError.replace(
      '{{inSuccess}}',
      actionsResponse.inSuccess.toString()
    );
    messageSuccessAndError = messageSuccessAndError.replace(
      '{{inError}}',
      actionsResponse.inError.toString()
    );
    // Success and Error
  ...
  public static async logOcpiResult(
    ...
    messageSuccessAndError = messageSuccessAndError.replace(
      '{{inSuccess}}',
      ocpiResult.success.toString()
    );
    messageSuccessAndError = messageSuccessAndError.replace(
      '{{inError}}',
      ocpiResult.failure.toString()
    );
  ...
  public static async logOicpResult(
    tenantID: string,
    action: ServerAction,
    module: string,
    method: string,
    oicpResult: OICPResult,
    messageSuccess: string,
    messageError: string,
    messageSuccessAndError: string,
    messageNoSuccessNoError: string
  ): Promise<void> {
    await Logging.logOcpiResult(
      tenantID,
      action,
      module,
      method,
  ...
  public static async traceExpressRequest(
    req: Request,
    res: Response,
    next: NextFunction,
    action?: ServerAction
    ...
    if (Logging.getTraceConfiguration().traceIngressHttp) {
      try {
        ...
        // Compute Length
        const sizeOfRequestDataKB = Utils.truncTo(
          Utils.createDecimal(sizeof({ headers: req.headers, query: req.query, body: req.body }))
            .div(1024)
            .toNumber(),
          2
        );
        ...
        const message = `Express HTTP Request - '${Utils.last5Chars(performanceID)}' << Req ${
          sizeOfRequestDataKB > 0 ? sizeOfRequestDataKB : '?'
        } KB << ${req.method} '${req.url}'`;
        Utils.isDevelopmentEnv() && Logging.logConsoleInfo(message);
  ...
  public static traceExpressResponse(
    req: Request,
    res: Response,
    next: NextFunction,
    action?: ServerAction
      ...
      res.on('finish', async () => {
        ...
        if (res.getHeader('content-length')) {
          sizeOfResponseDataKB = Utils.truncTo(
            Utils.createDecimal(res.getHeader('content-length') as number)
              .div(1024)
              .toNumber(),
            2
          );
        }
        const message = `Express HTTP Response ${
          req['performanceID'] ? "- '" + Utils.last5Chars(req['performanceID']) + "' " : ''
        }>> ${executionDurationMillis > 0 ? executionDurationMillis : '?'} ms - Res ${
          sizeOfResponseDataKB > 0 ? sizeOfResponseDataKB : '?'
        } KB >> ${req.method}/${res.statusCode} '${req.url}'`;
        Utils.isDevelopmentEnv() && Logging.logConsoleInfo(message);
        if (sizeOfResponseDataKB > Constants.PERF_MAX_DATA_VOLUME_KB) {
          const error = new Error(
            `Data must be < ${Constants.PERF_MAX_DATA_VOLUME_KB} KB, got ${
              sizeOfResponseDataKB > 0 ? sizeOfResponseDataKB : '?'
            } KB`
          );
          await Logging.logWarning({
          ...
          if (Utils.isDevelopmentEnv()) {
            Logging.logConsoleWarning('====================================');
            Logging.logConsoleWarning(
              `Tenant ID '${tenantID ? tenantID : Constants.DEFAULT_TENANT_ID}'`
            );
            Logging.logConsoleWarning(error.stack);
            Logging.logConsoleWarning(message);
            Logging.logConsoleWarning('====================================');
        ...
        if (executionDurationMillis > Constants.PERF_MAX_RESPONSE_TIME_MILLIS) {
          const error = new Error(
            `Execution must be < ${Constants.PERF_MAX_RESPONSE_TIME_MILLIS} ms, got ${
              executionDurationMillis > 0 ? executionDurationMillis : '?'
            } ms`
          );
          await Logging.logWarning({
            tenantID,
  ...
  public static async traceExpressError(
    error: Error,
    req: Request,
    res: Response,
    next: NextFunction
    ...
    await Logging.logActionExceptionMessageAndSendResponse(
      error['params'] && error['params']['action']
        ? error['params']['action']
        : ServerAction.HTTP_ERROR,
      error,
      req,
      res,
      next
  ...
  public static async traceAxiosRequest(
    tenant: Tenant,
    ...
    if (Logging.getTraceConfiguration().traceEgressHttp) {
      ...
      const message = `Axios HTTP Request - '${Utils.last5Chars(performanceID)}' >> Req ${
        sizeOfRequestDataKB > 0 ? sizeOfRequestDataKB : '?'
      } KB - ${request.method.toLocaleUpperCase()} '${request.url}'`;
      Utils.isDevelopmentEnv() && Logging.logConsoleInfo(message);
      await Logging.logDebug({
        tenantID: tenant.id,
        action: Utils.getAxiosActionFromURL(request.url),
        module: Constants.MODULE_AXIOS,
        method: 'interceptor',
        message,
        detailedMessages: {
  ...
  public static async traceAxiosResponse(tenant: Tenant, response: AxiosResponse): Promise<void> {
    if (Logging.getTraceConfiguration().traceEgressHttp) {
      ...
      if (response.config.headers['Content-Length']) {
        sizeOfResponseDataKB = Utils.truncTo(
          Utils.createDecimal(Utils.convertToInt(response.config.headers['Content-Length']))
            .div(1024)
            .toNumber(),
          2
        );
        ...
        sizeOfResponseDataKB = Utils.truncTo(
          Utils.createDecimal(sizeof(response.data)).div(1024).toNumber(),
          2
        );
      }
      const message = `Axios HTTP Response ${
        response.config['performanceID']
          ? "- '" + Utils.last5Chars(response.config['performanceID']) + "' "
          : ''
      }<< ${executionDurationMillis > 0 ? executionDurationMillis : '?'} ms - Res ${
        sizeOfResponseDataKB > 0 ? sizeOfResponseDataKB : '?'
      } KB << ${response.config.method.toLocaleUpperCase()}/${response.status} '${
        response.config.url
      }'`;
      Utils.isDevelopmentEnv() && Logging.logConsoleInfo(message);
      if (sizeOfResponseDataKB > Constants.PERF_MAX_DATA_VOLUME_KB) {
  ...
  public static async traceAxiosError(tenant: Tenant, error: AxiosError): Promise<void> {
    ...
    await Logging.logError({
      ...
      message: `Axios HTTP Error >> ${error.config?.method?.toLocaleUpperCase()}/${
      ...
      if (error.response?.config['performanceID']) {
        ...
        if (error.response?.config['timestamp']) {
          executionDurationMillis =
            new Date().getTime() - error.response?.config['timestamp'].getTime();
        }
        if (error.response?.data) {
          sizeOfResponseDataKB = Utils.truncTo(
            Utils.createDecimal(sizeof(error.response?.data)).div(1024).toNumber(),
            2
          );
        }
        const labelValues = { tenant: tenant.subdomain };
  ...
  public static async logActionExceptionMessage(
    tenantID: string,
    action: ServerAction,
    exception: Error,
    detailedMessages = {}
  ...
  // Used to log exception in catch(...) only
  public static async logActionExceptionMessageAndSendResponse(
    action: ServerAction,
    exception: Error,
    req: Request,
    res: Response,
    ...
    if (!res.headersSent) {
      const errorCode =
        exception['params'] && exception['params']['errorCode']
          ? exception['params']['errorCode']
  ...
  public static async traceOcppMessageRequest(
    module: string,
    tenant: Tenant,
    ...
    request: any,
    direction: '<<' | '>>',
    chargingStationDetails: { siteID: string; siteAreaID: string; companyID: string }
  ): Promise<PerformanceTracingData> {
    if (Logging.getTraceConfiguration().traceOcpp) {
      ...
      const message = `${direction} OCPP Request '${action}~${Utils.last5Chars(
        performanceID
      )}' on '${chargingStationID}' has been ${
        direction === '>>' ? 'received' : 'sent'
      } - Req ${sizeOfRequestDataKB} KB`;
      Utils.isDevelopmentEnv() && Logging.logConsoleInfo(message);
      await Logging.logDebug({
        tenantID: tenant.id,
  ...
  public static async traceOcppMessageResponse(
    module: string,
    tenant: Tenant,
    chargingStationID: string,
    action: ServerAction,
    ...
    direction: '<<' | '>>',
    chargingStationDetails: { siteID: string; siteAreaID: string; companyID: string },
    performanceTracingData?: PerformanceTracingData
  ): Promise<void> {
    if (Logging.getTraceConfiguration().traceOcpp && performanceTracingData) {
      // Compute duration if provided
      const executionDurationMillis = performanceTracingData?.startTimestamp
        ? Date.now() - performanceTracingData.startTimestamp
        : 0;
      const sizeOfResponseDataKB = Utils.truncTo(
        Utils.createDecimal(sizeof(response)).div(1024).toNumber(),
        2
      ...
      const message = `${direction} OCPP Request '${action}~${Utils.last5Chars(
        performanceTracingData.performanceID
      )}' on '${chargingStationID}' has been processed ${
        executionDurationMillis ? 'in ' + executionDurationMillis.toString() + ' ms' : ''
      } - Res ${sizeOfResponseDataKB > 0 ? sizeOfResponseDataKB : '?'} KB`;
      Utils.isDevelopmentEnv() && Logging.logConsoleInfo(message);
      if (executionDurationMillis > Constants.PERF_MAX_RESPONSE_TIME_MILLIS) {
        const error = new Error(
          `Execution must be < ${Constants.PERF_MAX_RESPONSE_TIME_MILLIS} ms`
        ...
        await Logging.logWarning({
          ...
          action: ServerAction.PERFORMANCES,
          module,
          method: 'traceChargingStationActionEnd',
          message: `${message}: ${error.message}`,
          detailedMessages: { error: error.stack },
        });
        if (Utils.isDevelopmentEnv()) {
  ...
  private static async logActionException(
    tenantID: string,
    action: ServerAction,
    exception: any,
    detailedMessages = {}
  ...
  private static async logActionAppException(
    tenantID: string,
    action: ServerAction,
    exception: AppError,
    detailedMessages = {}
  ...
  private static async logActionBackendException(
    tenantID: string,
    action: ServerAction,
  ...
  private static async logActionAppAuthException(
    tenantID: string,
    action: ServerAction,
    exception: AppAuthError,
    detailedMessages = {}
  ): Promise<void> {
    Utils.handleExceptionDetailedMessages(exception);
    await Logging.logError({
  ...
  private static async logActionOcppException(
    tenantID: string,
    action: ServerAction,
    exception: OCPPError,
    detailedMessages = {}
  ...
  private static async anonymizeSensitiveData(message: any): Promise<any> {
    ...
    if (typeof message === 'string') {
      ...
      if (dataParts.length > 1) {
        for (let i = 0; i < dataParts.length; i++) {
          ...
          queryParamKey =
            queryParamKeyParts.length > 1 ? queryParamKeyParts[1] : queryParamKeyParts[0];
          for (const sensitiveData of Constants.SENSITIVE_DATA) {
            if (queryParamKey.toLowerCase().startsWith(sensitiveData.toLocaleLowerCase())) {
              ...
              dataParts[i] =
                dataPart.substring(0, posSensitiveData + sensitiveData.length + 1) +
                Constants.ANONYMIZED_VALUE;
            }
          }
        }
    ...
    if (typeof message === 'object') {
      for (const key of Object.keys(message)) {
        ...
        if (
          Constants.SENSITIVE_DATA.filter(
            (sensitiveData) => key.toLowerCase() === sensitiveData.toLowerCase()
          ).length > 0
        ) {
          ...
          // Otherwise, apply the anonymizeSensitiveData function
