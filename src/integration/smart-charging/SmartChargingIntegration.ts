...
export default abstract class SmartChargingIntegration<T extends SmartChargingSetting> {
  ...
  public async computeAndApplyChargingProfiles(
    siteArea: SiteArea,
    retry = false
    ...
    const chargingProfiles: ChargingProfile[] = await this.buildChargingProfiles(
      siteArea,
      this.excludedChargingStations
    );
    if (!chargingProfiles) {
      await Logging.logInfo({
        ...
        module: MODULE_NAME,
        method: 'computeAndApplyChargingProfiles',
    ...
    chargingProfiles.sort(
      (a, b) =>
        a.profile.chargingSchedule.chargingSchedulePeriod[0].limit -
        b.profile.chargingSchedule.chargingSchedulePeriod[0].limit
    );
    ...
    await Logging.logActionsResponse(
      this.tenant.id,
      ServerAction.CHECK_AND_APPLY_SMART_CHARGING,
      MODULE_NAME,
      'computeAndApplyChargingProfiles',
      actionsResponse,
  ...
  private async handleRefusedChargingProfile(
    tenant: Tenant,
    ...
    for (let i = 0; i < 2; i++) {
      try {
        ...
        await Logging.logError({
          ...
          action: ServerAction.CHARGING_PROFILE_UPDATE,
          module: MODULE_NAME,
          method: 'handleRefusedChargingProfile',
  ...
  public abstract buildChargingProfiles(
    siteArea: SiteArea,
