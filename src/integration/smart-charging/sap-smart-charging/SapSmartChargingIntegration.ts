...
import {
  ...
  ExcludedAmperage,
  OptimizerCar,
  OptimizerCarConnectorAssignment,
  OptimizerChargingProfilesRequest,
  OptimizerChargingStationConnectorFuse,
  OptimizerChargingStationFuse,
  OptimizerFuse,
  OptimizerFuseTree,
  OptimizerFuseTreeNode,
  OptimizerResult,
...
export default class SapSmartChargingIntegration extends SmartChargingIntegration<SapSmartChargingSetting> {
  ...
  public async buildChargingProfiles(
    sourceSiteArea: SiteArea,
    excludedChargingStations?: string[]
    ...
    const siteAreas = await SiteAreaStorage.getSiteAreas(
      this.tenant,
      { siteIDs: [sourceSiteArea.siteID], withAssets: true },
    ...
    if (!Utils.isEmptyArray(siteAreas.result)) {
      const chargingStations = await ChargingStationStorage.getChargingStations(
        this.tenant,
        {
          siteIDs: [sourceSiteArea.siteID],
          connectorStatuses: [ChargePointStatus.CHARGING, ChargePointStatus.SUSPENDED_EVSE],
        },
        Constants.DB_PARAMS_MAX_LIMIT
      );
    ...
    const transactions = await TransactionStorage.getTransactions(
      this.tenant,
      { transactionIDs, withSmartChargingData: true },
      Constants.DB_PARAMS_MAX_LIMIT
    );
    // Build request
    const request = await this.buildOptimizerRequest(
      rootSiteArea,
      excludedChargingStations,
      false,
      currentChargingProfiles,
      transactions.result
    );
    // Call optimizer
    ...
    await this.checkIfChargingProfileAlreadyApplied(
      chargingProfiles,
      currentChargingProfiles,
      sourceSiteArea
    );
  ...
  private async buildOptimizerUrl(siteArea: SiteArea): Promise<string> {
    ...
    const requestUrl =
      url.slice(0, indexProtocolStringEnd) +
      user +
      ':' +
      password +
      '@' +
      url.slice(indexProtocolStringEnd);
    return requestUrl;
  }

  private async buildOptimizerRequest(
    siteArea: SiteArea,
    excludedChargingStations: string[] = [],
    retry = false,
    currentChargingProfiles: ChargingProfile[] = [],
    ...
    await this.buildFuseNodes(
      siteArea,
      fuseTree.rootFuse,
      fuseID,
      carConnectorAssignments,
      cars,
      excludedChargingStations,
      chargingStationsInError,
      currentChargingProfiles,
      transactions
    );
    ...
    const request: OptimizerChargingProfilesRequest = {
      ...
      state: {
        ...
        currentTimeSeconds: Utils.createDecimal(moment().diff(moment().startOf('hour'), 'seconds'))
          .div(900)
          .modulo(1)
          .mul(900)
          .toNumber(),
  ...
  private async buildFuseNodes(
    siteArea: SiteArea,
    fuseTreeNode: OptimizerFuse | OptimizerFuseTreeNode[],
    fuseID: { value: number },
    carConnectorAssignments: OptimizerCarConnectorAssignment[],
    cars: OptimizerCar[],
    excludedChargingStations: string[],
    chargingStationsInError: { value: boolean },
    currentChargingProfiles: ChargingProfile[],
    transactions: Transaction[]
  ): Promise<ExcludedAmperage> {
    ...
    const rootFuse = await this.buildRootFuse(
      siteArea,
      fuseID,
      excludedChargingStations,
      excludedAmperage
    );
    await this.buildFuseChargingStationNodes(
      siteArea,
      rootFuse,
      fuseID,
      carConnectorAssignments,
    ...
    if (!Utils.isEmptyArray(siteArea.childSiteAreas)) {
      ...
      for (const siteAreaChild of siteArea.childSiteAreas) {
        ...
        const excludedAmperageOnSubSiteAreas = await this.buildFuseNodes(
          siteAreaChild,
          !Array.isArray(fuseTreeNode)
            ? fuseTreeNode.children
            : fuseTreeNode[fuseTreeNode.length - 1].children,
          fuseID,
          carConnectorAssignments,
          cars,
          excludedChargingStations,
  ...
  private async buildFuseChargingStationNodes(
    siteArea: SiteArea,
    ...
    for (const chargingStation of siteArea.chargingStations) {
      ...
      for (const connector of chargingStation.connectors) {
        // Get the transaction
        const transaction = await this.getTransactionFromChargingConnector(
          siteArea,
          chargingStation,
          connector,
          transactions
        ...
        const chargingStationConnectorFuse = this.buildChargingStationConnectorFuse(
          siteArea,
          fuseID,
          chargingStation,
          connector
      ...
      const chargingStationFuse = this.buildChargingStationFuse(
        fuseID,
        sumConnectorAmperagePhase1,
        sumConnectorAmperagePhase2,
        sumConnectorAmperagePhase3,
  ...
  private async getTransactionFromChargingConnector(
    siteArea: SiteArea,
    ...
    if (!connector.currentTransactionID) {
      ...
      await Logging.logError({
        ...
        action: ServerAction.SMART_CHARGING,
        module: MODULE_NAME,
        method: 'getTransactionFromChargingConnector',
    ...
    const currentTransaction = transactions.find(
      (transaction) => transaction.id === connector.currentTransactionID
    );
  ...
  private async buildRootFuse(
    siteArea: SiteArea,
    fuseID: { value: number },
    excludedChargingStations?: string[],
    excludedAmperage?: ExcludedAmperage
  ): Promise<OptimizerFuse> {
    ...
    if (siteArea.maximumPower !== siteArea.maximumPower - assetConsumptionInWatts) {
      await Logging.logDebug({
        tenantID: this.tenant.id,
        action: ServerAction.SMART_CHARGING,
        message: `${siteArea.name} > limit of ${
          siteArea.maximumPower
        } W has been adjusted to ${Math.round(
          siteArea.maximumPower - assetConsumptionInWatts
        )} W due Asset Consumption`,
        module: MODULE_NAME,
        method: 'buildRootFuse',
        detailedMessages: { siteArea },
      });
      if (assetConsumptionInWatts > 0) {
        const excludedAmperagePerPhase = Utils.createDecimal(assetConsumptionInWatts)
          .div(siteArea.voltage)
          .div(siteArea.numberOfPhases)
          .toNumber();
    ...
    const siteMaxAmps = Utils.createDecimal(siteArea.maximumPower)
      .minus(assetConsumptionInWatts)
      .div(siteArea.voltage)
      .toNumber();
    const siteMaxAmpsPerPhase = Utils.createDecimal(siteMaxAmps)
      .div(siteArea.numberOfPhases)
      .toNumber();
    ...
    for (let i = siteArea.chargingStations.length - 1; i >= 0; i--) {
      ...
      for (let j = chargingStation.connectors.length - 1; j >= 0; j--) {
        ...
        const chargePoint = connector.chargePointID
          ? Utils.getChargePointFromID(chargingStation, connector.chargePointID)
          : null;
        // Check if charging station needs to be excluded and if already processed
        if (
          (Utils.isNullOrUndefined(chargePoint) ||
            ...
            excludedChargingStations?.includes(chargingStation.id)) &&
          !chargePointIDsAlreadyProcessed.includes(chargePoint?.chargePointID)
        ) {
          // Remove the power of the connector in use
          const connectorAmperage = Utils.getChargingStationAmperage(
            chargingStation,
            chargePoint,
          ...
          if (siteArea.numberOfPhases === 1) {
            ...
            let connectorAmperagePerPhase = Utils.createDecimal(connectorAmperage)
              .div(3)
              .toNumber();
            // Take into account efficiency
            if (
              Utils.getChargingStationCurrentType(chargingStation, null, connector.connectorId) ===
              CurrentType.DC
            ) {
              if (chargePoint?.efficiency > 0) {
                connectorAmperagePerPhase = Utils.createDecimal(connectorAmperagePerPhase)
                  .mul(100)
                  .div(chargePoint.efficiency)
                  .toNumber();
              } else {
                // Use safe value if efficiency is not provided
                connectorAmperagePerPhase = Utils.createDecimal(connectorAmperagePerPhase)
                  .mul(100)
                  .div(Constants.DC_CHARGING_STATION_DEFAULT_EFFICIENCY_PERCENT)
                  .toNumber();
              }
            }
    ...
    if (siteMaxAmps !== rootFuse.fusePhase1 + rootFuse.fusePhase2 + rootFuse.fusePhase3) {
      await Logging.logDebug({
        tenantID: this.tenant.id,
        action: ServerAction.SMART_CHARGING,
        message: `${siteArea.name} > limit of ${siteMaxAmps} Amps has been lowered to ${Math.round(
          rootFuse.fusePhase1 + rootFuse.fusePhase2 + rootFuse.fusePhase3
        )} Amps due to unsupported charging stations currently being used`,
        module: MODULE_NAME,
        method: 'buildRootFuse',
  ...
  private buildSafeCar(
    fuseID: number,
    chargingStation: ChargingStation,
    transaction: Transaction
  ): OptimizerCar {
    const voltage = Utils.getChargingStationVoltage(chargingStation);
    const maxConnectorAmpsPerPhase = Utils.getChargingStationAmperagePerPhase(
      chargingStation,
      null,
  ...
  private buildCar(
    ...
    if (!Utils.isNullOrUndefined(transaction.car)) {
      ...
      if (
        ...
        if (transaction?.carCatalog?.fastChargePowerMax > 0) {
          const maxDCCurrent = Utils.convertWattToAmp(
            ...
            null,
            transaction.connectorId,
            transaction.carCatalog.fastChargePowerMax * 1000
          ); // Charge capability in Amps
      ...
      if (transaction?.carCatalog?.batteryCapacityFull > 0) {
        ...
        customCar.minLoadingState =
          ((transaction.carCatalog.batteryCapacityFull * 1000) / voltage) * 0.5; // Battery level at the end of the charge in Amp.h set at 50%
      }
    }
    // Override
    this.overrideCarWithRuntimeData(
      chargingStation,
      transaction,
      customCar,
      currentChargingProfiles
    ...
    if (currentType === CurrentType.DC) {
      ...
      if (chargePoint?.efficiency > 0) {
        customCar.maxCurrentPerPhase = Utils.createDecimal(customCar.maxCurrentPerPhase)
          .div(chargePoint.efficiency)
          .mul(100)
          .toNumber();
        customCar.maxCurrent = customCar.maxCurrentPerPhase * 3;
    ...
    if (this.setting.prioritizationParametersActive) {
      ...
      this.handleCurrentStateOfCharge(
        customCar,
        transaction,
        smartChargingSessionParameters.carStateOfCharge
      );
      this.handleTargetStateOfCharge(
        customCar,
        transaction,
        smartChargingSessionParameters.targetStateOfCharge
      );
      this.handleTimestampDeparture(
        customCar,
        transaction,
        currentType,
        smartChargingSessionParameters.departureTime
      );
    }
    return customCar;
  }
  ...
  private handleCurrentStateOfCharge(
    customCar: OptimizerCar,
    transaction: Transaction,
    defaultCurrentStateOfCharge: number
  ): void {
  ...
  private handleTargetStateOfCharge(
    customCar: OptimizerCar,
    transaction: Transaction,
  ...
  private handleTimestampDeparture(
    optimizerCar: OptimizerCar,
    transaction: Transaction,
    currentType: CurrentType,
    defaultDepartureTime: string
  ): void {
    const currentTimeInSeconds = Utils.createDecimal(
      moment().diff(moment().startOf('hour'), 'seconds')
    )
      .div(900)
      .modulo(1)
      .mul(900)
    ...
    if (!Utils.isNullOrUndefined(transaction.departureTime)) {
      optimizerCar.timestampDeparture =
        moment(transaction.departureTime).diff(moment(), 'seconds') + 1 + currentTimeInSeconds;
    } else if (currentType === CurrentType.DC) {
      // Set static departure time for DC sessions
      optimizerCar.timestampDeparture = moment(transaction.timestamp)
        .add(1, 'hours')
        .diff(moment(), 'seconds');
      ...
      if (defaultDepartureTime) {
        ...
        timestampDeparture = moment()
          .set('hour', defaultDepartureHour)
          .set('minute', defaultDepartureMinute);
        if (timestampDeparture < moment()) {
          timestampDeparture.add(1, 'days');
        }
      }
      optimizerCar.timestampDeparture =
        moment(timestampDeparture).diff(moment(), 'seconds') + 1 + currentTimeInSeconds;
    }
  ...
  private overrideCarWithRuntimeData(
    chargingStation: ChargingStation,
    transaction: Transaction,
    car: OptimizerCar,
    currentChargingProfiles: ChargingProfile[]
    ...
    if (transaction.phasesUsed) {
      const numberOfPhasesInProgress = Utils.getNumberOfUsedPhasesInTransactionInProgress(
        chargingStation,
        transaction
      );
        ...
        if (this.setting.stickyLimitation) {
          ...
          if (!carIsIncreasingConsumption) {
            ...
            if (transaction.currentInstantAmps > 0) {
              ...
              car.maxCurrentPerPhase = Utils.roundTo(
                (transaction.currentInstantAmps / numberOfPhasesInProgress) *
                  (1 +
                    (typeof this.setting.limitBufferAC === 'number'
                      ? this.setting.limitBufferAC
                      : 0) /
                      100),
                3
              );
      ...
      if (!carIsIncreasingConsumption) {
        ...
        const currentInstantAmps = Utils.convertWattToAmp(
          chargingStation,
          null,
          transaction.connectorId,
        ...
        car.maxCurrentPerPhase = Utils.roundTo(
          (currentInstantAmps / 3) *
            (1 +
              (typeof this.setting.limitBufferDC === 'number' ? this.setting.limitBufferDC : 0) /
                100),
          3
        );
  ...
  private getConnectorNbrOfPhasesAndAmps(
    siteArea: SiteArea,
    chargingStation: ChargingStation,
    connector: Connector
  ): ConnectorAmps {
    ...
    if (connector.chargePointID) {
      const chargePoint = Utils.getChargePointFromID(chargingStation, connector.chargePointID);
      // Get the usual power
      connectorAmps.numberOfConnectedPhase = Utils.getNumberOfConnectedPhases(
        chargingStation,
        chargePoint,
        connector.connectorId
      );
      connectorAmps.totalAmps = Utils.getChargingStationAmperage(
        chargingStation,
        chargePoint,
        connector.connectorId
      );
    ...
    if (!connectorAmps.numberOfConnectedPhase) {
      throw new BackendError({
        ...
        action: ServerAction.SMART_CHARGING,
        module: MODULE_NAME,
        method: 'getConnectorNbrOfPhasesAndAmps',
  ...
  private buildChargingStationConnectorFuse(
    siteArea: SiteArea,
    fuseID: { value: number },
    chargingStation: ChargingStation,
    connector: Connector
    ...
    if (
      ...
      const chargePoint = Utils.getChargePointFromID(chargingStation, connector.chargePointID);
      if (chargePoint?.efficiency > 0) {
        connectorAmpsPerPhase = Utils.createDecimal(connectorAmpsPerPhase)
          .div(chargePoint.efficiency)
          .mul(100)
          .toNumber();
      } else {
  ...
  private buildChargingStationFuse(
    fuseID: { value: number },
    sumConnectorAmperagePhase1: number,
    sumConnectorAmperagePhase2: number,
    sumConnectorAmperagePhase3: number,
    chargingStationConnectorsFuse: OptimizerChargingStationConnectorFuse[]
  ): OptimizerChargingStationFuse {
  ...
  private async buildChargingProfilesFromOptimizerResponse(
    sourceSiteArea: SiteArea,
    siteAreas: SiteArea[],
    ...
    for (const car of optimizerResult.cars) {
      ...
      for (const siteArea of siteAreas) {
        const index = siteArea.chargingStations.findIndex(
          (chargingStation) => chargingStation.id === chargingStationID
        );
        if (index > -1) {
      ...
      if (!currentChargingStation) {
        throw new BackendError({
          ...
          action: ServerAction.SMART_CHARGING,
          module: MODULE_NAME,
          method: 'buildChargingProfilesFromOptimizerResponse',
      ...
      const maxScheduleLength = parseInt(
        await ChargingStationStorage.getOcppParameterValue(
          this.tenant,
          chargingStationID,
      ...
      for (
        let i = 0;
        i < (!isNaN(maxScheduleLength) && maxScheduleLength < 16 ? maxScheduleLength : 16);
        i++
      ) {
        chargingSchedule.chargingSchedulePeriod.push({
          ...
          limit: this.calculateCarConsumption(
            currentChargingStation,
            connector,
  ...
  private calculateCarConsumption(
    chargingStation: ChargingStation,
    connector: Connector,
    ...
    if (
      ...
      if (chargePoint?.efficiency > 0) {
        return Utils.roundTo(
          ((currentLimit * chargePoint.efficiency) / 100) * numberOfConnectedPhase,
          1
        );
      }
      return Utils.roundTo(
        ((currentLimit * Constants.DC_CHARGING_STATION_DEFAULT_EFFICIENCY_PERCENT) / 100) *
          numberOfConnectedPhase,
        1
      );
    }
  ...
  private checkIfCarIsIncreasingConsumption(
    chargingStation: ChargingStation,
    currentChargingProfiles: ChargingProfile[],
    transaction: Transaction,
    currentType: CurrentType,
    numberOfPhasesInProgress?: number
  ): boolean {
    // Get the current charging profile for the current car
    const currentProfile = currentChargingProfiles.filter(
      (chargingProfile) =>
    ...
    if (currentProfile.length === 1) {
      ...
      const numberOfPhasesChargingStation = Utils.getNumberOfConnectedPhases(
        chargingStation,
        null,
        transaction.connectorId
      );
  ...
  private getCurrentSapSmartChargingProfileLimit(currentProfile: ChargingProfile): number {
    ...
    const currentSlot = Math.floor(
      moment().diff(moment(currentProfile.profile.chargingSchedule.startSchedule), 'minutes') / 15
    );
    ...
    if (currentSlot < currentProfile.profile.chargingSchedule.chargingSchedulePeriod.length) {
      ...
      const currentLimit =
        currentProfile.profile.chargingSchedule.chargingSchedulePeriod[currentSlot]?.limit;
      return currentLimit;
    }
    return -1;
  }
  ...
  private async getAssetConsumptionInWatts(siteArea: SiteArea): Promise<number> {
    if (
      Utils.isTenantComponentActive(this.tenant, TenantComponents.ASSET) &&
      !Utils.isEmptyArray(siteArea.assets)
    ) {
          ...
          if (asset.dynamicAsset) {
            ...
            if (moment().diff(moment(asset.lastConsumption?.timestamp), 'minutes') < 2) {
              if (asset.currentInstantWatts > 0) {
                ...
                consumptionSaveValue =
                  asset.currentInstantWatts + fluctuation < asset.staticValueWatt
                    ? asset.currentInstantWatts + fluctuation
                    : asset.staticValueWatt;
              } else if (asset.currentInstantWatts < 0) {
                // Ensure production does not exceed 0
                consumptionSaveValue =
                  asset.currentInstantWatts + fluctuation < 0
                    ? asset.currentInstantWatts + fluctuation
                    : 0;
              }
            } else if (
              asset.assetType === AssetType.CONSUMPTION ||
              asset.assetType === AssetType.CONSUMPTION_AND_PRODUCTION
            ) {
  ...
  private async checkIfChargingProfileAlreadyApplied(
    ...
    for (let i = 0; i < currentChargingProfiles.length; i++) {
      ...
      const chargingProfileIndex = chargingProfiles.findIndex(
        (cp) => cp.profile.transactionId === currentChargingProfiles[i].profile.transactionId
      );
      if (chargingProfileIndex >= 0) {
        // Check if profiles are identical
        if (
          JSON.stringify(chargingProfiles[chargingProfileIndex].profile) ===
          JSON.stringify(currentChargingProfiles[i].profile)
        ) {
    ...
    if (removedChargingProfiles > 0) {
      await Logging.logDebug({
        ...
        module: MODULE_NAME,
        method: 'checkIfChargingProfileAlreadyApplied',
      });
    }
  }
}
