...
export default class StripeBillingIntegration extends BillingIntegration {
  ...
  public async repairInvoice(billingInvoice: BillingInvoice): Promise<void> {
    await this.checkConnection();
    const transactions = new Map<string, { transactionID: number, description: string, pricingData: [] }>();
    ...
    //       "description" : "Sesión de carga: 1884157429 - Fecha de inicio: 3 de agosto de 2021 a las 11:15 - Consumo de energía: 0,211 kWh en la estación de recarga CS-SCHNEIDER-0001",
    //       "pricingData" : {
    //           "quantity" : 0.211,
    //           "amount" : 3.37,
    //           "currency" : "EUR"
      ...
      if (transactionID) {
        ...
        if (!transactions.has(transactionID)) {
          transactions.set(transactionID, {
            ...
            description: `Session: ${transactionID} - (generated by repair invoice task)`,
            pricingData: [] // TODO - how to build it properly?
          });
        }
      }
    }
