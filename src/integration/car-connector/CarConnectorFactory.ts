...
export default class CarConnectorFactory {
  public static async getCarConnectorImpl(
    tenant: Tenant,
    carConnectorId: string
  ): Promise<CarConnectorIntegration<CarConnectorSettings>> {
    ...
    if (Utils.isTenantComponentActive(tenant, TenantComponents.CAR_CONNECTOR)) {
      ...
      if (settings?.carConnector?.connections) {
        ...
        const foundConnection = settings.carConnector.connections.find(
          (connection) => connection.id === carConnectorId
        );
        if (foundConnection) {
          ...
          switch (foundConnection.type) {
            case CarConnectorConnectionType.MERCEDES:
              carConnectorIntegrationImpl = new MercedesCarConnectorIntegration(
                tenant,
                settings,
                foundConnection
              );
            ...
            case CarConnectorConnectionType.TRONITY:
              carConnectorIntegrationImpl = new TronityCarConnectorIntegration(
                tenant,
                settings,
                foundConnection
              );
            ...
            case CarConnectorConnectionType.TARGA_TELEMATICS:
              carConnectorIntegrationImpl = new TargaTelematicsCarConnectorIntegration(
                tenant,
                settings,
                foundConnection
              );
