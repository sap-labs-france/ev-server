  ...
  public static async importLocalCarCatalog() : Promise<void> {
    ...
    try {
      ...
      if (!Utils.isEmptyArray(cars)) {
        for (const car of cars) {
          ...
          for (const imageURL of carCatalog.imageURLs) {
            ...
            await CarStorage.saveCarImage(carCatalog.id, base64Image);
          }
          // Save
      ...
      await Logging.logError({
        ...
        module: MODULE_NAME, method: 'importLocalCarCatalog',
        action: ServerAction.CAR_CATALOG_SYNCHRONIZATION,
        message: `Error while importing the local Cars: ${error.message}`,
