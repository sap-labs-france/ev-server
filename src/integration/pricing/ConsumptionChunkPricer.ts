...
export default class ConsumptionChunkPricer {
  ...
  public constructor(consumptionPricer: ConsumptionPricer, consumptionChunk: ConsumptionChunk) {
    ...
    const actualPricingDefinitions = this.getPricingModel().pricingDefinitions.filter(
      (pricingDefinition) => this.checkPricingDefinitionRestrictions(pricingDefinition)
    );
  ...
  private checkPricingDefinitionRestrictions(
    pricingDefinition: ResolvedPricingDefinition
  ): ResolvedPricingDefinition {
    if (pricingDefinition.restrictions) {
      if (
        !this.checkDayOfWeek(pricingDefinition.restrictions) ||
  ...
  private checkDayOfWeek(restrictions: PricingRestriction): boolean {
    if (!Utils.isNullOrUndefined(restrictions.daysOfWeek)) {
      ...
      return restrictions.daysOfWeek.includes(
        moment(this.consumptionChunk.startedAt).tz(timezone).isoWeekday()
      );
    }
  ...
  private checkTimeValidity(restrictions: PricingRestriction): boolean {
    ...
    if (
      Utils.isNullOrUndefined(restrictions.timeFrom) &&
      Utils.isNullOrUndefined(restrictions.timeTo)
    ) {
    ...
    const consumptionStartTime = PricingTimeLimit.parseTime(
      moment(this.consumptionChunk.startedAt).tz(timezone).format('HH:mm:ss')
    );
  ...
  private checkMinEnergy(restrictions: PricingRestriction): boolean {
    if (!Utils.isNullOrUndefined(restrictions.minEnergyKWh)) {
      if (
        Utils.createDecimal(this.consumptionChunk.cumulatedConsumptionWh)
          .div(1000)
          .lessThan(restrictions.minEnergyKWh)
      ) {
        return false;
      }
    }
  ...
  private checkMaxEnergy(restrictions: PricingRestriction): boolean {
    if (!Utils.isNullOrUndefined(restrictions.maxEnergyKWh)) {
      if (
        Utils.createDecimal(this.consumptionChunk.cumulatedConsumptionWh)
          .div(1000)
          .greaterThanOrEqualTo(restrictions.maxEnergyKWh)
      ) {
        return false;
      }
    }
  ...
  private checkMinDuration(restrictions: PricingRestriction): boolean {
    if (!Utils.isNullOrUndefined(restrictions.minDurationSecs)) {
      if (
        Utils.createDecimal(this.consumptionChunk.totalDurationSecs).lessThan(
          restrictions.minDurationSecs
        )
      ) {
        return false;
      }
  ...
  private checkMaxDuration(restrictions: PricingRestriction): boolean {
    if (!Utils.isNullOrUndefined(restrictions.maxDurationSecs)) {
      if (
        Utils.createDecimal(this.consumptionChunk.totalDurationSecs).greaterThanOrEqualTo(
          restrictions.maxDurationSecs
        )
      ) {
        return false;
      }
  ...
  private priceFlatFeeConsumption(): PricedDimensionData {
    ...
    if (!this.getPricingModel().pricerContext.flatFeeAlreadyPriced) {
      const activePricingDefinition = this.getActiveDefinition4Dimension(
        this.actualPricingDefinitions,
  ...
  private priceEnergyConsumption(): PricedDimensionData {
    ...
    if (activePricingDefinition) {
      ...
      if (dimensionToPrice.stepSize) {
        if (this.consumptionChunk.cumulatedConsumptionWh > 0) {
          const delta = Utils.createDecimal(this.consumptionChunk.cumulatedConsumptionWh).minus(
            this.getAbsorbedConsumption()
          );
  ...
  private enrichPricedData(
    pricedData: PricedDimensionData,
    activePricingDefinition: ResolvedPricingDefinition
  ): PricedDimensionData {
    if (pricedData) {
      pricedData.sourceName = activePricingDefinition.name;
    }
  ...
  private absorbConsumption() {
    ...
    this.getPricingModel().pricerContext.lastAbsorbedConsumption =
      this.consumptionChunk.cumulatedConsumptionWh;
  }
  ...
  private absorbStepConsumption(pricedData?: PricedDimensionData) {
    ...
    const consumption = Utils.createDecimal(this.consumptionChunk.cumulatedConsumptionWh)
      .div(pricedData.stepSize)
      .trunc()
      .mul(pricedData.stepSize)
      .toNumber();
  ...
  private getAbsorbedChargingTime() {
    return (
      this.getPricingModel().pricerContext.lastAbsorbedChargingTime ||
      this.getPricingModel().pricerContext.sessionStartDate
    );
  }
  ...
  private absorbChargingTime(remainder = 0) {
    ...
    // This may happen when combining several tariffs in a single session
    this.getPricingModel().pricerContext.lastAbsorbedChargingTime = moment(
      this.consumptionChunk.endedAt
    )
  ...
  private getAbsorbedParkingTime() {
    return (
      this.getPricingModel().pricerContext.lastAbsorbedParkingTime ||
      this.getPricingModel().pricerContext.sessionStartDate
    );
  }
  ...
  private absorbParkingTime(remainder = 0) {
    ...
    // This may happen when combining several tariffs in a single session
    this.getPricingModel().pricerContext.lastAbsorbedParkingTime = moment(
      this.consumptionChunk.endedAt
    )
      .subtract(remainder, 'seconds')
      .toDate();
  ...
  private getActiveDefinition4Dimension(
  ...
  private checkPricingDimensionRestrictions(
    pricingDefinition: ResolvedPricingDefinition,
    dimensionType: string
  ): ResolvedPricingDefinition {
  ...
  private priceTimeDimension(
    pricingDimension: PricingDimension,
    lastStepDate: Date
  ): { pricedData: PricedDimensionData; remainder: number } {
    ...
    if (pricingDimension.stepSize) {
      ...
      const seconds = moment(this.consumptionChunk.endedAt).diff(
        moment(this.consumptionChunk.startedAt),
        'seconds'
      );
      if (seconds > 0) {
        return {
  ...
  private addPricedData(pricingDimension: PricingDimension, pricedData: PricedDimensionData): void {
    ...
    if (previousData) {
      previousData.amountAsDecimal = Utils.createDecimal(previousData.amountAsDecimal).plus(
        pricedData.amountAsDecimal
      );
      previousData.amount = previousData.amountAsDecimal.toNumber();
      previousData.quantity = Utils.createDecimal(previousData.quantity)
        .plus(pricedData.quantity)
        .toNumber();
