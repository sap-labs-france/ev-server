...
export default abstract class ChargingStationVendorIntegration {
  ...
  public getStaticPowerLimitation(
  ...
  public async setStaticPowerLimitation(
    ...
    if (chargePoint.excludeFromPowerLimitation) {
      await Logging.logWarning({
        ...
        module: MODULE_NAME,
        method: 'setStaticPowerLimitation',
    ...
    if (maxAmps < StaticLimitAmps.MIN_LIMIT_PER_PHASE * numberOfPhases * numberOfConnectors) {
      throw new BackendError({
        ...
        module: MODULE_NAME,
        method: 'setStaticPowerLimitation',
        message: `Cannot set the minimum power limit to ${maxAmps}A, minimum expected ${
    ...
    const ocppLimitAmpValue = this.convertLimitAmpPerPhase(
      chargingStation,
      chargePoint,
      0,
      maxAmps * ocppParamValueMultiplier
    );
    ...
    try {
      await Logging.logDebug({
        ...
        detailedMessages: {
          maxAmps,
          ocppParam: chargePoint.ocppParamForPowerLimitation,
          ocppLimitAmpValue: ocppLimitAmpValue,
        },
      });
          ...
          for (const connectorID of chargePoint.connectorIDs) {
            ...
            if (connector) {
              connector.amperageLimit = this.convertLimitAmpToAllPhases(
                ...
                chargePoint,
                connectorID,
                Utils.convertToInt(ocppParamValue) / ocppParamValueDivider
              );
              await Logging.logInfo({
                ...LoggingHelper.getChargingStationProperties(chargingStation),
                tenantID: tenant.id,
                action: ServerAction.OCPP_PARAM_UPDATE,
                message: `${Utils.buildConnectorInfo(connectorID)} Amperage limit set to ${
                  connector.amperageLimit
                }A following an update of OCPP Parameter '${ocppParamName}'`,
                module: MODULE_NAME,
                method: 'checkUpdateOfOCPPParams',
                detailedMessages: {
                  ocppParamName,
                  ocppParamValue,
                  connectorID,
      ...
      if (chargingProfile.connectorID === 0) {
        ...
        if (result.status !== OCPPChargingProfileStatus.ACCEPTED) {
          await Logging.logWarning({
            ...
            action: ServerAction.CHARGING_PROFILE_UPDATE,
            message:
              'Set Charging Profile on Connector ID 0 has been rejected, will try connector per connector',
            module: MODULE_NAME,
            method: 'clearChargingProfile',
  ...
  public async clearChargingProfile(
    ...
    chargingStation: ChargingStation,
    chargingProfile: ChargingProfile
  ): Promise<OCPPClearChargingProfileResponse | OCPPClearChargingProfileResponse[]> {
    // Check if feature is supported
    if (!chargingStation.capabilities?.supportChargingProfiles) {
      throw new BackendError({
        ...LoggingHelper.getChargingStationProperties(chargingStation),
      ...
      if (chargingProfile.connectorID === 0) {
        ...
        if (result.status !== OCPPClearChargingProfileStatus.ACCEPTED) {
          await Logging.logWarning({
            ...
            method: 'clearChargingProfile',
            message:
              'Clear Charging Profile on Connector ID 0 has been rejected, will try connector per connector',
            detailedMessages: { result },
          });
  ...
  public async getCurrentConnectorLimit(
    tenant: Tenant,
    chargingStation: ChargingStation,
    chargePoint: ChargePoint,
    connectorID: number
  ): Promise<ConnectorCurrentLimit> {
    const connector = Utils.getConnectorFromID(chargingStation, connectorID);
    if (!connector) {
      throw new BackendError({
        ...LoggingHelper.getChargingStationProperties(chargingStation),
        action: ServerAction.GET_CONNECTOR_CURRENT_LIMIT,
        module: MODULE_NAME,
        method: 'getCurrentConnectorLimit',
    ...
    try {
      ...
      if (chargingStation.capabilities?.supportChargingProfiles) {
        ...
        const txChargingProfiles = chargingProfiles.filter(
          (chargingProfile) =>
        ...
        let txDefaultChargingProfiles = chargingProfiles.filter(
          (chargingProfile) =>
        ...
        if (Utils.isEmptyArray(txDefaultChargingProfiles)) {
          txDefaultChargingProfiles = chargingProfiles.filter(
            (chargingProfile) =>
              chargingProfile.connectorID === connectorID &&
              chargingProfile.profile.chargingProfilePurpose ===
                ChargingProfilePurposeType.TX_DEFAULT_PROFILE
        ...
        resultChargingProfile = await this.getCurrentConnectorLimitFromProfiles(
          ...
          chargePoint,
          connectorID,
          txDefaultChargingProfiles
        );
        if (resultChargingProfile) {
          return resultChargingProfile;
        }
        // Check the Max Charging Profiles from the DB
        const maxChargingProfiles = chargingProfiles.filter(
          (chargingProfile) =>
            chargingProfile.connectorID === connectorID &&
            chargingProfile.profile.chargingProfilePurpose ===
      ...
      if (chargingStation.capabilities && chargingStation.capabilities.supportStaticLimitation) {
        ...
        const connectorLimitAmps = this.getStaticPowerLimitation(
          chargingStation,
          chargePoint,
          connectorID
        );
        if (connectorLimitAmps > 0) {
          const resultConnector: ConnectorCurrentLimit = {
            ...
            limitWatts: Utils.convertAmpToWatt(
              chargingStation,
              chargePoint,
              connectorID,
              connectorLimitAmps
          ...
          await Logging.logInfo({
            ...
            tenantID: tenant.id,
            action: ServerAction.GET_CONNECTOR_CURRENT_LIMIT,
            message: `${Utils.buildConnectorInfo(connectorID)} Current limit: ${
              resultConnector.limitAmps
            } A, ${resultConnector.limitWatts} W, source '${Utils.getConnectorLimitSourceString(
              resultConnector.limitSource
            )}'`,
            module: MODULE_NAME,
            method: 'getCurrentConnectorLimit',
    ...
    await Logging.logInfo({
      ...
      tenantID: tenant.id,
      action: ServerAction.GET_CONNECTOR_CURRENT_LIMIT,
      message: `${Utils.buildConnectorInfo(connectorID)} Current limit: ${result.limitAmps} A, ${
        result.limitWatts
      } W, source '${Utils.getConnectorLimitSourceString(result.limitSource)}'`,
      module: MODULE_NAME,
      method: 'getCurrentConnectorLimit',
      detailedMessages: { result },
    });
  ...
  public convertToVendorChargingProfile(
    chargingStation: ChargingStation,
    chargePoint: ChargePoint,
    chargingProfile: ChargingProfile
  ): ChargingProfile {
    ...
    if (
      chargingStation.connectors &&
      vendorSpecificChargingProfile.profile &&
      vendorSpecificChargingProfile.profile.chargingSchedule
    ) {
      // Convert to Watts?
      if (chargingStation.powerLimitUnit === ChargingRateUnitType.WATT) {
        vendorSpecificChargingProfile.profile.chargingSchedule.chargingRateUnit =
          ChargingRateUnitType.WATT;
      }
      // Divide the power by the number of connectors and number of phases
      for (const schedulePeriod of vendorSpecificChargingProfile.profile.chargingSchedule
        .chargingSchedulePeriod) {
        ...
        if (chargingStation.powerLimitUnit === ChargingRateUnitType.AMPERE) {
          schedulePeriod.limit = Utils.roundTo(
            this.convertLimitAmpPerPhase(
              chargingStation,
              chargePoint,
              vendorSpecificChargingProfile.connectorID,
              schedulePeriod.limit
            ),
            1
          );
  ...
  public convertFromVendorChargingSchedule(
    chargingStation: ChargingStation,
    chargePoint: ChargePoint,
    connectorID: number,
    ...
    if (chargingSchedule.chargingSchedulePeriod) {
      for (const chargingSchedulePeriod of chargingSchedule.chargingSchedulePeriod) {
        chargingSchedulePeriod['startPeriodMins'] = Math.round(
          chargingSchedulePeriod.startPeriod / 60
        );
        ...
        if (chargingSchedule.chargingRateUnit === ChargingRateUnitType.WATT) {
          ...
          chargingSchedulePeriod.limit = Utils.convertWattToAmp(
            chargingStation,
            chargePoint,
            connectorID,
        ...
        chargingSchedulePeriod['limitWatts'] = Utils.convertAmpToWatt(
          chargingStation,
          chargePoint,
          connectorID,
          chargingSchedulePeriod.limit
  ...
  private hasStaticLimitationFullSupport(
    chargingStation: ChargingStation,
    chargePoint?: ChargePoint
  ): boolean {
    if (
      this.hasStaticLimitationSupport(chargingStation) &&
  ...
  private convertLimitAmpPerPhase(
    chargingStation: ChargingStation,
    chargePoint: ChargePoint,
    connectorID = 0,
    limitAmpAllPhases: number
  ): number {
  ...
  private convertLimitAmpToAllPhases(
    chargingStation: ChargingStation,
    chargePoint: ChargePoint,
    connectorID = 0,
    limitAmpPerPhase: number
  ): number {
    ...
    // Per phase
    limitAmpAllPhases *= Utils.getNumberOfConnectedPhases(
      chargingStation,
      chargePoint,
      connectorID
    );
  ...
  private async getCurrentConnectorLimitFromProfiles(
    tenant: Tenant,
    chargingStation: ChargingStation,
    chargePoint: ChargePoint,
    connectorID: number,
          ...
          if (
            ...
            const result: ConnectorCurrentLimit = {
              ...
              limitWatts: Utils.convertAmpToWatt(
                ...
                chargePoint,
                connectorID,
                Utils.convertToInt(schedulePeriod.limit)
              ),
            ...
            await Logging.logInfo({
              ...
              message: `${Utils.buildConnectorInfo(connectorID)} Current limit: ${
                result.limitAmps
              } A, ${result.limitWatts} W, source '${Utils.getConnectorLimitSourceString(
                result.limitSource
              )} in DB'`,
              module: MODULE_NAME,
              method: 'getCurrentConnectorLimit',
          ...
          if (
            ...
            const result: ConnectorCurrentLimit = {
              ...
              limitWatts: Utils.convertAmpToWatt(
                ...
                chargePoint,
                connectorID,
                Utils.convertToInt(lastButOneSchedule.limit)
              ),
              limitSource: ConnectorCurrentLimitSource.CHARGING_PROFILE,
            };
            await Logging.logInfo({
