...
export default class SapConcurRefundIntegration extends RefundIntegration<ConcurRefundSetting> {
  ...
  public async createConnection(userID: string, data: any): Promise<Connection> {
    try {
      await Logging.logDebug({
        ...
        action: ServerAction.REFUND,
        message: `Request Concur access token for User ID '${userID}'`,
      });
      ...
      await Logging.logDebug({
        ...
        method: 'createConnection',
        action: ServerAction.REFUND,
        message: `Concur access token granted for User ID '${userID}'`,
      });
      // Check first
  ...
  public async refund(
    ...
    let expenseReportId;
    if (!quickRefund) {
      expenseReportId = await this.createExpenseReport(
        connection,
        transactions[0].timezone,
        userID
      );
    }
    await Promise.map(
      ...
      async (transaction: Transaction) => {
        try {
          ...
          if (quickRefund) {
            const entryId = await this.createQuickExpense(
              connection,
              transaction,
              location,
              userID
            ...
            const entryId = await this.createExpenseReportEntry(
              connection,
              expenseReportId,
              transaction,
              location,
          ...
          await TransactionStorage.saveTransactionRefundData(
            this.tenant,
            transaction.id,
            transaction.refundData
          );
    ...
    await Logging.logInfo({
      ...
      } transactions have been transferred to Concur in ${moment().diff(
        startDate,
        'milliseconds'
      )} ms`,
    });
  ...
  private async createQuickExpense(
    connection: Connection,
    transaction: Transaction,
    location: ConcurLocation,
    userID: string
  ) {
    try {
      // Get the user
      ...
      const concurURL = `${this.setting.authenticationUrl}/quickexpense/v4/users/${
        jwt.decode(connection.data.access_token).sub
      }/context/TRAVELER/quickexpenses`;
      const response = await this.axiosInstance.post(
        concurURL,
        {
          comment: `Session started on ${i18nManager.formatDateTime(
            moment.tz(transaction.timestamp, transaction.timezone).toDate()
          )} during ${moment
            .duration(transaction.stop.totalDurationSecs, 'seconds')
            .format('h[h]mm', { trim: false })} and consumed ${i18nManager.formatNumber(
            Math.trunc(transaction.stop.totalConsumptionWh / 10) / 100
          )} kW.h`,
          ...
          transactionDate: moment
            .tz(transaction.timestamp, transaction.timezone)
            .format('YYYY-MM-DD'),
        },
        {
          headers: {
      ...
      await Logging.logDebug({
        ...
        message: `Transaction ${
          transaction.id
        } has been successfully transferred in ${moment().diff(
          startDate,
          'milliseconds'
  ...
  private async createExpenseReportEntry(
    connection: Connection,
    ...
    try {
      ...
      const response = await this.axiosInstance.post(
        concurURL,
        {
          Description: `E-Mobility reimbursement ${moment
            .tz(transaction.timestamp, transaction.timezone)
            .format('YYYY-MM-DD')}`,
          Comment: `Session started on ${i18nManager.formatDateTime(
            moment.tz(transaction.timestamp, transaction.timezone).toDate()
      ...
      await Logging.logDebug({
        ...
        action: ServerAction.REFUND,
        module: MODULE_NAME,
        method: 'createExpenseReportEntry',
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        message: `Transaction ${
          transaction.id
        } has been successfully transferred in ${moment().diff(
  ...
  private async createExpenseReport(connection: Connection, timezone: string, userID: string) {
    try {
      ...
      await Logging.logDebug({
        ...
        method: 'createExpenseReport',
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        message: `Report has been successfully created in ${moment().diff(
          startDate,
          'milliseconds'
        )} ms with ${this.getRetryCount(response)} retries`,
      });
  ...
  private async refreshToken(userID: string, connection: Connection): Promise<Connection> {
    try {
      ...
      await Logging.logDebug({
        ...
        method: 'refreshToken',
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        message: `Concur access token has been successfully generated in ${moment().diff(
          startDate,
          'milliseconds'
        )} ms with ${this.getRetryCount(response)} retries`,
      });
