...
export default class RemoveDuplicateTagVisualIDsTask extends TenantMigrationTask {
  public async migrateTenant(tenant: Tenant): Promise<void> {
    ...
    const tags = await global.database
    ...
    if (!Utils.isEmptyArray(tags)) {
      ...
      for (const tag of tags) {
        // Get dup Tags
        const duplicateTags = await TagStorage.getTags(
          tenant,
          {
            visualIDs: [tag._id],
          },
        ...
        for (const duplicateTag of duplicateTags.result) {
          await global.database
            .getCollection<any>(tenant.id, 'tags')
            .updateOne({ _id: duplicateTag.id }, { $set: { visualID: new ObjectId().toString() } });
          updated++;
        }
      }
    }
    ...
    if (updated > 0) {
      await Logging.logDebug({
        ...
        message: `${updated} duplicate Tag(s) Visual IDs have been made unique in Tenant ${Utils.buildTenantName(
          tenant
        )}`,
      });
    }
