...
export default class RepairTransactionPricedAtZero extends TenantMigrationTask {
  ...
  public async migrateTenant(tenant: Tenant) {
    ...
    if (transactionsMDB.length > 0 && this.pricingSettings?.simple?.price > 0) {
      let message = `${
        transactionsMDB.length
      } Transaction(s) are going to be repaired in Tenant ${Utils.buildTenantName(tenant)}...`;
      ...
      Utils.isDevelopmentEnv() &&
        console.debug(chalk.yellow(`${new Date().toISOString()} - ${message}`));
      await Promise.map(
        transactionsMDB,
        async (transactionMDB) => {
          const numberOfProcessedTransactions =
            transactionsUpdated.inError + transactionsUpdated.inSuccess;
          if (numberOfProcessedTransactions > 0 && numberOfProcessedTransactions % 100 === 0) {
            message = `> ${transactionsUpdated.inError + transactionsUpdated.inSuccess}/${
              transactionsMDB.length
            } - Transaction consumptions recomputed in Tenant ${Utils.buildTenantName(tenant)}`;
          ...
          try {
            ...
            await Logging.logError({
              ...
              message: `> ${transactionsUpdated.inError + transactionsUpdated.inSuccess}/${
                ...
                transactionMDB._id
              }' in Tenant ${Utils.buildTenantName(tenant)}`,
        ...
        void Logging.logActionsResponse(
          ...
          'migrateTenant',
          transactionsUpdated,
          `{{inSuccess}} transaction(s) were successfully processed in ${totalDurationSecs} secs in Tenant ${Utils.buildTenantName(
            tenant
          )}`,
          `{{inError}} transaction(s) failed to be processed in ${totalDurationSecs} secs in Tenant ${Utils.buildTenantName(
            tenant
          )}`,
          `{{inSuccess}} transaction(s) were successfully processed in ${totalDurationSecs} secs and {{inError}} failed to be processed in Tenant ${Utils.buildTenantName(
            tenant
          )}`,
  ...
  private async priceTransactionAgain(tenant: Tenant, transaction: Transaction): Promise<void> {
    ...
    const price = Utils.createDecimal(transaction.stop.totalConsumptionWh)
      .mul(pricePerkWh)
      .div(1000)
      .toNumber();
