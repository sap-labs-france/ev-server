  ...
  public async migrateTenant(tenant: Tenant): Promise<void> {
      ...
      for (const siteArea of siteAreas) {
        ...
        if (foundSite) {
          ...
          updated += await SiteAreaStorage.updateEntitiesWithOrganizationIDs(
            tenant,
            foundSite.companyID.toString(),
            siteArea.siteID.toString(),
            siteArea._id.toString()
          );
        } else {
    ...
    if (updated > 0) {
      await Logging.logDebug({
        ...
        method: 'migrateTenant',
        action: ServerAction.MIGRATION,
        message: `${updated} record(s) have been updated with Organization IDs in Tenant ${Utils.buildTenantName(
          tenant
        )}`,
      });
    }
